...
if(OP_PASS == "") {
 ...
 if(NP_WORD == "") {
    ...
    return false;
	 ...
	 if(NP_WORD.length <= 9) { 
    inlineMsg('NP_WORD','Enter Atleast 10 character As Password',2);
    return false;
    }
...
-->
     ...
     <%
       String USER_ID = (String) session.getAttribute("sOffCd");
		 String msg="";  
 ...
 <p>&nbsp;</p>
  ...
  <th colspan="4"><div align="center"> 
      ...
      <span class="style90 style96"> PASSWORD CHANGE FOR APPROVER AGENCY FORM </span>
...
<th colspan="4"> 
   ...
   <%
       ...
       if (ins.equals("I")) {
		...
		qry.append(" SELECT nvl(COUNT(*),0) FROM  MAS_OFF_MAST WHERE OFF_CD=? AND P_WORD=? AND ACT_YN='Y' AND conv_dltfcyn='Y' AND off_type_cd in(2,5,22,27,3) " );
...
if (cnt.equals("1")) {
		 ...
		 qrysb.append("  UPDATE MAS_OFF_MAST SET P_WORD=? WHERE OFF_CD=? AND ACT_YN='Y'  AND conv_dltfcyn='Y' AND off_type_cd in(2,5,22,27,3)  " );
