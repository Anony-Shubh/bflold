...
qrysb.append("  AD.APP_ID, AD.APP_NAME,CASE WHEN GT.LATITUDE IS NULL AND GT.LONGITUDE IS NULL THEN 'N' ELSE 'Y' END AS GEOYN,"      );
qrysb.append("     RBL.BANK_NAME,"      );
qrysb.append("     RBL.IFSC_CODE,"      );
qrysb.append("     TO_CHAR(BD.MM_REL_DT,'DD-MON-RRRR') AS MM_REL_DT,GT.ACT_ID,"      );
...
qrysb.append(" AND AD.OFF_CD IN (SELECT OFF_CD  FROM MAS_OFF_MAST WHERE OFF_TYPE_CD IN (2,3) AND ACT_YN='Y' AND MIS_OFF_CD=?) "      );
qrysb.append(" UNION ");
...
qrysb.append("   AND AM.IND_GRP_CD   = IGM.IND_GRP_CD"      );
qrysb.append("   AND BD.PACT_ID      = 34 AND GT.MM_ADJUSTMENT_STATUS IS NOT NULL  AND GT.QC_APPRV='Y' "      );
qrysb.append(" AND AD.OFF_CD IN (SELECT OFF_CD  FROM MAS_OFF_MAST WHERE OFF_TYPE_CD IN (2,3) AND ACT_YN='Y' AND MIS_OFF_CD=?)) ORDER BY DATE_OF_VERIFICATION DESC"      );
