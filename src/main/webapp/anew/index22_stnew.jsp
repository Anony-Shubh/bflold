...
<TD><A target="_blank" href="reports/GenerateXLS.jsp?REP_NM=detailed&REPTYPE=1&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>"><%= NO_OF_APP %></A>
...
<TD>
<!--<BUTTON type="button" class="btn btn-primary modalButton" DATA-TOGGLE="modal" data-book-id="<%//= STATE_CD %>">More</BUTTON>	
--> 

<A href="#dynamicModal" class="btn btn-primary" DATA-TOGGLE="modal" DATA-BOOK-ID="<%= STATE_CD %>">More</A>

</TD>	

<TD>
<A href="#dynamicModal2" class="btn btn-primary" DATA-TOGGLE="modal" DATA-BOOK-ID="<%= STATE_CD %>">More</A>
</TD>	
...
<A class="btn btn-primary" href="bankwisedetail.jsp?TYPE=S&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>">View</A></TD>										
<%}%>
...
$(document).ready(function(){
    ...
    $(".modalButton").click(function(){
        var id =$(this).data(id);	
		vstate=document.form.STATE_CD.value;
		alert(vstate);
        $.ajax({
            url:"mmclaimpendingstatus.jsp?STATE_CD="+vstate,
			//url+="?STATE_CD="+vstate;
            method:"post",
            data:{id:id},
...
$('#dynamicModal').on('show.bs.modal', function(e) {
	...
	$.ajax({
            url:"mmclaimpendingstatus.jsp?STATE_CD="+bookId,
			//url+="?STATE_CD="+vstate;
            method:"post",
            //data:{bookId:bookId},
			success:function(response){
...
$('#dynamicModal2').on('show.bs.modal', function(e) {
    var bookId = $(e.relatedTarget).data('book-id');
	...
	$.ajax({
            url:"AgeWiseRepo.jsp?STATE_CD="+bookId,
			//url+="?STATE_CD="+vstate;
            method:"post",
            //data:{bookId:bookId},
			success:function(response){
