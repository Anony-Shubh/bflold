...
qrysb.append("     BBM.BR_NAME,CASE WHEN MS.ZONE_CD <> 'NE' AND ACTIVITY_CD=316 THEN 'N' ELSE 'Y' END AS NEACT_YN,"      );
qrysb.append("    CASE WHEN mminv_bankadmit_sec(AD.APP_ID)<> nvl(BD.mm_claim_amt,0) THEN 'N' ELSE 'Y' END AS MMCLAIMAMTYN,"      );
qrysb.append("     BBM.BRANCH_NAME,"      );
qrysb.append("     TO_CHAR(BD.MM_CLAIM_DT,'DD-MON-RRRR') AS MM_CLAIM_DT,MATCHTRANSACNO(AD.APP_ID,2) AS TRANS_YN,"      );
...
qrysb.append("   AND AD.OFF_CD         = MOM.OFF_CD AND AD.UNIT_DIST_CD=MD.DISTRICT_CD AND MD.STATE_CD=MS.STATE_CD"      );
...
%>
  ...
  <table align="center">
    ...
    <tr valign="middle">
     ...
     <%if ((ACT_ID.equals("30")) || (TRANS_YN.equals("Y")  && (NEACT_YN.equals("Y") ) && (MMCLAIMAMTYN.equals("Y") ) ) ){%> <td colspan="3" nowrap ><input name="ACT_ID" id="ACT_ID" type="radio"  value="32">
	 <span class="GreenLebel">Recommended for MM Disbursement </span> </td><%}%>
    </tr>
    <tr>
      <td colspan="3"><input name="ACT_ID" id="option" type="radio"  value="30">        
  ...
  <%
  String refqry=  " SELECT MRT.CO_REMARKS AS CO_REMARKS1, TO_CHAR(trunc(MRT.PROC_DT),'DD-MON-RRRR') AS PROC_DT1 FROM MM_REMARKS_TRANS_SECOND MRT,BANK_DATAENTRY_SECOND BD WHERE BD.PACT_ID IN (30) ";
...
qryUpdate.append("  INSERT INTO  MM_REMARKS_TRANS_SECOND (REMTR_ID,PROC_DT,USER_TYPE,PACT_ID,CO_REMARKS,APP_ID,IFSC_CODE,USER_ID) ");
...
String Updt= "UPDATE BANK_DATAENTRY_SECOND SET REMTRID=?, PACT_ID=? ,APPRV_YN='S' WHERE APP_ID=? AND IFSC_CODE=? ";
values.add(REMTR_ID); pstm.add ("L");
values.add(ACT_ID); pstm.add ("L");
values.add(APP_ID); pstm.add ("L");
values.add(BR_IFSL_CODE); pstm.add ("L");
