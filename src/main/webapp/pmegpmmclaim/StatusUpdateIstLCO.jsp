...
qrysb.append("     BBM.BRANCH_NAME,CASE WHEN MS.ZONE_CD <> 'NE' AND ACTIVITY_CD=316 THEN 'N' ELSE 'Y' END AS NEACT_YN, "      );
qrysb.append("    CASE WHEN mminv_bankadmit(AD.APP_ID)<> nvl(BD.mm_claim_amt,0) THEN 'N' ELSE 'Y' END AS MMCLAIMAMTYN,"      );
qrysb.append("     TO_CHAR(BD.MM_CLAIM_DT,'DD-MON-RRRR') AS MM_CLAIM_DT,MATCHTRANSACNO(AD.APP_ID,1) AS TRANS_YN,"      );
qrysb.append("     BD.MM_CLAIM_AMT,"      );
qrysb.append("     BD.FB_ACNO,BD.LOAN_DOCFNAME, DECODE(BD.PACT_ID,30,'Reffered Back','Recommended For Disbursement') as PRV_STATUS"      );
...
qrysb.append("   AND BD.IFSC_CODE      = BBM.BR_IFSL_CODE AND AD.UNIT_DIST_CD=MD.DISTRICT_CD AND MD.STATE_CD=MS.STATE_CD "      );
...
PRV_STATUS=rsMain.getString("PRV_STATUS")==null?"":rsMain.getString("PRV_STATUS");
...
%>
  ...
  <table align="center">
    <tr>
      <th colspan="4"><div align="center">PMEGP MARGIN MONEY MM CLAIM PROCESS AT CENTRAL OFFICE
    ...
    <tr valign="middle">
	 ...
	 <tr valign="middle">
      <th colspan="2"><div align="right"><h2>Previous Status:</h2> </div></th>
      <th colspan="2" ><h2><%= PRV_STATUS %></h2></th>
     
    </tr>
    <tr valign="middle">
      <th rowspan="3"><div align="right">Select Current Status: * </div></th>
      <td colspan="3" nowrap ><%if (adhvalid.equals("S") && (TRANS_YN.equals("Y")) && (NEACT_YN.equals("Y")) && (MMCLAIMAMTYN.equals("Y") ) && (BENF_TYPE_CD.equals("1"))){%>
<input name="ACT_ID" id="ACT_ID" type="radio"  value="32" onChange="valid_ts();">
        <span class="GreenLebel">Recommended for MM Disbursement </span> 
		<%} %></td>
    </tr>
    <tr>
      <td colspan="3"><input name="ACT_ID" id="option" type="radio"  value="30" onChange="valid_ts();">        
      <span class="OrangeLebel">To be referred Back</span></td>
    </tr>
    <!--<tr>
      ...
      <th>Referred  Subject to  : </th>
      <td nowrap><input type="checkbox"  name="VALID_RS"  value="EDP">
        EDP <span class="GreenLebel"></span> </td>
      <td nowrap ><input type="checkbox" name="VALID_RS"  value="ADH">
        AADHAR CARD <span class="GreenLebel"></span> </td>
      <td nowrap ><input type="checkbox" name="VALID_RS"   value="EMP">
        EMPLOYEMENT <span class="GreenLebel"></span> </td>
      <td nowrap ><input type="checkbox" name="VALID_RS"   value="OTH">
        OTHERS</td>
		<td nowrap ><input type="checkbox" name="VALID_RS"   value="LOS">
        LOAN STATEMENT</td>
		<td nowrap ><input type="checkbox" name="VALID_RS"   value="SIB">
        SIGNBOARD</td>
    </tr>
  ...
  <%
   //AND MRT.APPRV_YN='R'
  String refqry=  " SELECT MRT.CO_REMARKS AS CO_REMARKS1, TO_CHAR(trunc(MRT.PROC_DT),'DD-MON-RRRR') AS PROC_DT1 FROM MM_REMARKS_TRANS MRT,BANK_DATAENTRY BD WHERE BD.PACT_ID IN (30,32)  ";
...
String fruits[]= request.getParameterValues("VALID_RS");
String vwhercls ="";
if (fruits != null && fruits.length != 0) {
for (int i = 0; i < fruits.length; i++) {
vwhercls=fruits[i]+"||"+vwhercls ; 
}//end of for loop
vwhercls=vwhercls;
}//end of if

StringBuffer qryUpdate = new StringBuffer();
  ResultSet rsMax = db.execSQL(" SELECT MMPROCREMSEQ.NEXTVAL FROM SYS.DUAL ");
  while (rsMax.next()){
  REMTR_ID=rsMax.getString(1);
  }
  rsMax.close();
  
qryUpdate.append("  INSERT INTO  MM_REMARKS_TRANS (REMTR_ID,PROC_DT,USER_TYPE,PACT_ID,CO_REMARKS,APP_ID,IFSC_CODE,USER_ID,VALID_RS,CLIENT_IP) ");
...
String Updt= "UPDATE BANK_DATAENTRY SET REMTRID=?, PACT_ID=? ,APPRV_YN='S' WHERE APP_ID=? AND IFSC_CODE=? AND APP_ID IN ( SELECT APP_ID FROM APP_DETAIL WHERE ACT_YN='Y' AND APP_ID=?) ";
values.add(REMTR_ID); pstm.add ("L");
values.add(ACT_ID); pstm.add ("L");
values.add(APP_ID); pstm.add ("L");
values.add(BR_IFSL_CODE); pstm.add ("L");
