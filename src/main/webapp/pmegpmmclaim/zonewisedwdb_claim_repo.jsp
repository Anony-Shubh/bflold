...
qrysb.append("SELECT  /*+ PARALLEL(AUTO) */ nvl(ZONE_NM,'ZTOTAL') AS ZONE_NM,NVL(OFF_NAME1,'ZTOTAL') AS OFF_NAME1,"      ); 
...
qrysb.append("SUM(CASE WHEN (VUPLOADTIMESTAMP< PROC_DT OR (VUPLOADTIMESTAMP IS NULL OR PROC_DT IS NULL)) AND PACT_ID=30 AND APPRV_YN = 'A' THEN 1 ELSE 0 END)NO_OF_PRJ_RP,"      ); 
qrysb.append("SUM(CASE WHEN (VUPLOADTIMESTAMP< PROC_DT OR (VUPLOADTIMESTAMP IS NULL OR PROC_DT IS NULL)) AND PACT_ID=30 AND APPRV_YN = 'A' THEN ROUND(MM_CLAIM_AMT / 100000, 2) ELSE 0 END) AS MM_CLAIM_AMT_RP,"      ); 
qrysb.append("SUM(CASE WHEN APP_STATUS='CUP' THEN 1 ELSE 0 END) NO_OF_PRJ_CUP,"      ); 
...
qrysb.append("SUM(CASE WHEN (VUPLOADTIMESTAMP< PROC_DT OR (VUPLOADTIMESTAMP IS NULL OR PROC_DT IS NULL)) AND PACT_ID=30 AND APPRV_YN = 'S' THEN 1 ELSE 0 END)NO_OF_PRJ_RP_S,"      );
qrysb.append("SUM(CASE WHEN (VUPLOADTIMESTAMP< PROC_DT OR (VUPLOADTIMESTAMP IS NULL OR PROC_DT IS NULL)) AND PACT_ID=30 AND APPRV_YN = 'S' THEN ROUND(MM_CLAIM_AMT / 100000, 2) ELSE 0 END) AS MM_CLAIM_AMT_RP_S,"      );
qrysb.append("SUM(CASE WHEN NVL(PACT_ID,0) IN (32,30) AND APPRV_YN = 'R' THEN 1 ELSE 0 END) AS RET_RESB,"      );
qrysb.append("SUM(CASE WHEN NVL(PACT_ID,0) IN (32,30) AND APPRV_YN = 'R' THEN ROUND(MM_CLAIM_AMT / 100000, 2) ELSE 0 END) AS RET_RESB_MM "      );
qrysb.append(" FROM (select * from mmclaim_view01 WHERE TRUNC(MM_CLAIM_DT) BETWEEN '"+FROMDT+"' AND '"+TODT+"' )"      ); 

/*qrysb.append(" FROM ( SELECT     BD.APP_ID,"      ); 
qrysb.append("   AD.APP_NAME,"      ); 
...
qrysb.append(" GROUP BY  ROLLUP("      ); 
qrysb.append(" ZONE_NM ,OFF_NAME1) ORDER BY ZONE_NM,OFF_NAME1 "      ); 
...
String NO_OF_PRJ_RP_S="";
String MM_CLAIM_AMT_RP_S="";
String RET_RESB="";
String RET_RESB_MM="";
DBCon db= new DBCon();
db.connect();
...
%>
<center>
  <h1> Margin Money (MM) Claim Status Zone Wise <%=FROMDT%> to <%= TODT %>
<a href="mmclaimProcHomepage.jsp" class="button" >
 ...
 <thead>
  <tr>
	...
	<th colspan="2" rowspan="2"><div align="center">Referred to Bank and pending for retification </div></th>
	<th colspan="2" rowspan="2"><div align="center">Referred to Bank and pending for Approval </div></th>
	<th colspan="2" rowspan="2"><div align="center">Returned Back For Resubmission </div></th>
	<th colspan="2" rowspan="2"><div align="center">Recommended For MM Release and Approved </div></th>  
    <th rowspan="2" colspan="2"><div align="center">Recommended Claims Pending for Approval </div></th>
    <th colspan="2" rowspan="2"><div align="center">MM Released </div></th>
    </tr>
  <tr>
...
PEND_APPRV_MM=rsMain.getString("PEND_APPRV_MM")==null?"":rsMain.getString("PEND_APPRV_MM");
 NO_OF_PRJ_RP_S=rsMain.getString("NO_OF_PRJ_RP_S")==null?"":rsMain.getString("NO_OF_PRJ_RP_S");
 MM_CLAIM_AMT_RP_S=rsMain.getString("MM_CLAIM_AMT_RP_S")==null?"":rsMain.getString("MM_CLAIM_AMT_RP_S");
 RET_RESB=rsMain.getString("RET_RESB")==null?"":rsMain.getString("RET_RESB");
 RET_RESB_MM=rsMain.getString("RET_RESB_MM")==null?"":rsMain.getString("RET_RESB_MM");
...
<tbody>
  ...
  <tr>
	...
	<td><%=ZONE_NM %></td>
    <td><%=OFF_NAME1 %></td>	
    <td><a href="appZonewisedwdb.jsp?ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%= TOT_MM_CLAIM_PRJ %></a></div></td>
    <td><%= TOT_MM_CLAIM_MM %></td>
    <td><a href="appZonewisedwdb.jsp?APP_STATUS=CUP&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>"><%=NO_OF_PRJ_CUP %></a></div></td>
    <td><%=MM_CLAIM_AMT_CUP %></td>	
	<td><a href="appZonewisedwdb.jsp?APP_STATUS=RC&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=NO_OF_PRJ_RC %></a></div></td>
	<td><%=MM_CLAIM_AMT_RC %></td>
	<td><a href="appZonewisedwdb.jsp?APP_STATUS=RP&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=NO_OF_PRJ_RP %></a></div></td>
	<td><%= MM_CLAIM_AMT_RP%></td>
	<td><a href="appZonewisedwdb.jsp?APPS_STATUS=RPS&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=NO_OF_PRJ_RP_S %></a></div></td>
	<td><%= MM_CLAIM_AMT_RP_S%></td>
	<td><a href="appZonewisedwdb.jsp?APPT_STATUS=RET&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=RET_RESB %></a></div></td>
	<td><%= RET_RESB_MM%></td>
	<td><a href="appZonewisedwdb.jsp?APP_STATUS=RMR&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=NO_OF_PRJ_RMR %></a></div></td>
	<td><%=MM_CLAIM_AMT_RMR %></td>
	<td><a href="appZonewisedwdb.jsp?APPR_STATUS=A&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%= PEND_APPRV %></a></div></td>
	<td><%=PEND_APPRV_MM%></td>
	<td><a href="appZonewisedwdb.jsp?APP_STATUS=MR&ZONE_NM=<%=ZONE_NM %>&OFF_NAME=<%=OFF_NAME1 %>&FROMDT=<%=FROMDT%>&TODT=<%=TODT%>" ><%=NO_OF_PRJ_MR %></a></div></td>
