...
function ShowNewPage(param){
 ...
 function SetState(){
 var callbackFunctionArray = new Array(EnrollLater);
 modalWin.ShowURL('StatePop.jsp?',600,900,'Filter',null,callbackFunctionArray);
 }
 function CearAll(){
 var callbackFunctionArray = new Array(EnrollLater);
 modalWin.ShowURL('clearFilter.jsp?',600,900,'Filter',null,callbackFunctionArray);
 }
...
String STATE="";
  ...
  String STATUS="";
  String vWcls="";
  String vStatus=(String) session.getAttribute("STATUS")==null?"32":(String)session.getAttribute("STATUS");
   out.print(vSTATE);
    out.print(vStatus);
   ...
   if (vStatus.equals("31")){
	   vWcls=vWcls+"  AND MRT.PACT_ID in (30,32)";   
   }
   else if (vStatus.equals("")){
	   vWcls=vWcls+"  AND MRT.PACT_ID =32";  
   }
   else{
	    vWcls=vWcls+"  AND MRT.PACT_ID ="+vStatus+" "; 
   }
 ...
 vWherecls=vWherecls+" AND (MS.STATE_CD ) LIKE  ('AP')  ";
 ...
 <div>
  <div align="center">
  <input name="btnDocument" type="button" class="buttonGray" id="btnDocument" onClick="SetState();" value="Filter">
  <input name="btnDocument" type="button" class="buttonGray" id="btnDocument" onClick="CearAll();" value="Clear All">
...
qrysb.append("   AND BD.MM_CLAIM_AMT   > 0 AND AD.UNIT_DIST_CD=MD.DISTRICT_CD AND MD.STATE_CD=MS.STATE_CD AND AD.ACT_YN='Y' "      );
qrysb.append("   AND BD.LOAN_DOCFNAME IS NOT NULL"      );
qrysb.append("   AND BD.APPRV_YN      IS NOT NULL AND BD.APPRV_YN='S' "+vWherecls+" "+vWcls+" "      );
