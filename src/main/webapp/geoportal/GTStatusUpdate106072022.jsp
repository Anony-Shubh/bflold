...
function chMd()
 {	
	...
	//alert(actid);
     ...
     if( actid == "RM")
     {
		document.forms[0].MAX_AMT.style.backgroundColor="#FFFFFF";
        document.forms[0].MAX_AMT.disabled=false;
		document.forms[0].MAX_AMT.value=document.form.MM_REL_AMT.value;
                  
     }
	 else{
	 
	    document.forms[0].MAX_AMT.style.backgroundColor="#FFFFFF";
        document.forms[0].MAX_AMT.disabled=true;
		document.forms[0].MAX_AMT.value=0;
...
var MM_ADJUSTMENT_STATUS = document.form.MM_ADJUSTMENT_STATUS.value;
   var MM_ADJUSTMENT_AMT_S = document.form.MM_REL_AMT.value;
...
<!--PERCAPITAEMPREQ-->
	...
	<TR >
	  ...
	  <TD ><%=LOAN_SANC_DT%></TD>
        ...
        <INPUT name="MM_REL_AMT" type="hidden" id="MM_REL_AMT"  size="10" maxlength="7"  value="<%= MAX_AMT %>" >
...
rsValidate.close();
    ...
    <TD colspan="3"><DIV align="left"><STRONG>MM Adjustment Status </STRONG>*</DIV></TD>
	 <TD colspan="2">
	  ...
	  <INPUT type="radio" id="opt-1" value="RM"   name="MM_ADJUSTMENT_STATUS" disabled>
	   ...
	   <%} else{%>
	    <INPUT type="radio" id="opt-1" value="RM"   name="MM_ADJUSTMENT_STATUS" <% if (MM_ADJUSTMENT_STATUS.equals("RM")){out.print("checked");}%> onChange="chMd();">
	   <SPAN class="GreenLebel">Recommended For MM Adjustment (Rs. <%=MAX_AMT%>)</SPAN>	 
	   <%}%>
	    </STRONG> </TD>
     <TD  colspan="4">
	  <STRONG>
	  <INPUT type="radio" id="opt-1" value="RU"   name="MM_ADJUSTMENT_STATUS" <% if (MM_ADJUSTMENT_STATUS.equals("RU")){out.print("checked");}%> onChange="chMd();">
  ...
  <TR >
    ...
    <TD ><DIV align="left"><INPUT name="MAX_AMT" type="text" id="MAX_AMT"  size="10" maxlength="7"  value="<%= MAX_AMT %>" disabled="disabled" ></DIV></TD>
...
MM_ADJUSTMENT_AMT_S=(String) request.getParameter("MAX_AMT")==null?"0":(String) request.getParameter("MAX_AMT").trim();
...
int j=Integer.parseInt(MM_REL_AMT); 

 if(MM_ADJUSTMENT_STATUS.equals("RM")){
 
 try {
  if (i>j) {
   aerror.add("MM Amt cannot be greater than :"+j);
...
qryUpdate.append("MM_ADJUSTMENT_STATUS=?,ACT_ID=NULL,REMTRID=null,MM_ADJ_DATE=SYSDATE,LAST_UPDATE_DT=SYSDATE ");
