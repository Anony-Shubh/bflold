...
<form method="post"  name="form" id="form" class="form" enctype="multipart/form-data" >
	  ...
	  <p>
        ...
        DBCon db= new DBCon();
        db.connect();
ResultSet rsMANU= db.execSQL("SELECT substr(A.App_id,instr(A.App_id,'-')+1,length(A.app_id))  AS ID FROM APP_DETAIL A WHERE APP_ID='"+APP_ID+"' union SELECT substr(A.App_id,instr(A.App_id,'-')+1,length(A.app_id))  AS ID FROM APP_DETAIL_SECOND A WHERE APP_ID='"+APP_ID+"' ");
		   while (rsMANU.next()) {  //3
...
String a=sm.getStorage("GEOTAGUPLOAD/AGENCY/"); 
   ...
   <jsp:setProperty name="upBean" property="whitelist" value="*.pdf,*.jpg,*jpeg,*.png" />
	 ...
	 try {
      if (MultipartFormDataRequest.isMultipartFormData(request))
      {
		 ...
		 sbY.append(" UPDATE GEOTAG_TRANS SET AGENCY_UPLOAD=? WHERE APP_ID=? AND PMEGPGEOTAGID=?");  
  values.add(frenNm);pstm.add("D");
  values.add(APP_ID);pstm.add("D");
  values.add(PMEGPGEOTAGID);pstm.add("D");
try{
