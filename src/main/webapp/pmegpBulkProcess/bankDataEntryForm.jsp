<%@ page session="true" buffer="1000kb" import="java.sql.*,javax.sql.*,javax.naming.*,java.util.*,java.text.*,java.util.*,pkgPmegpNew.DBCon"  %>
...
append(" OWNCONT_PER(B.APP_ID) AS OWN_CONT_PER,MM_PER(B.APP_ID) AS MM_ELIG_PER,MMINV_BANKADMIT(a.APP_ID) as DMMELGB,").
...
append ( " to_char(a.tdr_adj_dt,'DD-MON-YYYY') as tdr_adj_dt, a.tdr_adj_amt,to_char(sysdate,'DD-MON-YYYY') as TODAYDT,BBM.CIRCLE_IFSC AS IFSC_CODE, " ).
 append ( " a.bank_comment,  to_char(a.time_stamp,'DD-MON-YYYY:HH:MM:SS') as time_stamp ").
  append (" FROM bank_dataentry a, app_detail b,BANK_BR_MAST BBM,BENF_CATEGORY_MAST C,education_mast d , ACTIVITY_MAST AM,M_DISTRICT MD,M_STATE MS WHERE B.ACTIVITY_CD=AM.ACTIVITY_CD AND bbm.br_ifsl_code=b.ifsc_code AND  b.APP_ID=? AND bbm.CIRCLE_IFSC=?").
...
rsTrno= db.executeSQL("SELECT MAX(decode(CIRCLE_NEFT,null,circle_ifsc,CIRCLE_NEFT) ) AS  NEFT_IFSC,MAX(CIRCLE_TRANS_ACNO) TRANS_ACNO FROM BANK_BR_MAST  WHERE CIRCLE_IFSC=? UNION SELECT BTM.NEFT_IFSC_CODE|| '('||BTM.AC_DESC||')' AS NEFT_IFSC, BTM.TRAC_NO  AS TRANS_ACNO FROM BANK_TRANS_ACMAST BTM WHERE ACT_YN='Y' AND  BTM.NEFT_IFSC_CODE=?",values);
//SELECT NVL(NEFT_IFSC,CIRCLE_IFSC) AS NEFT_IFSC,TRANS_ACNO FROM BANK_BR_MAST  WHERE CIRCLE_IFSC=? UNION SELECT BTM.NEFT_IFSC_CODE|| '('||BTM.AC_DESC||')' AS NEFT_IFSC, BTM.TRAC_NO  AS TRANS_ACNO FROM BANK_TRANS_ACMAST BTM WHERE ACT_YN='Y' AND  BTM.NEFT_IFSC_CODE=?",values);
values.clear();
...
<%}%>
  ...
  <TABLE class="pmegpTable">
    ...
    <TR>
      ...
      <TD colspan="3">
<INPUT name="BANK_REC_DT" id="BANK_REC_DT" type="text"  tabindex="0" value="<%= BANK_REC_DT %>" onClick="javascript:NewCssCal('BANK_REC_DT','ddMMMyyyy')" size="12" PLACEHOLDER="dd-mom-yyyy" AUTOCOMPLETE="off" >
</TD>
    ...
    <TR>
	  ...
	  <INPUT name="REJ_ID" type="hidden" id="REJ_ID" value="<%= REJ_ID %>" size="50" maxlength="50" >
        <INPUT name="btnREJL" type="button" class="buttonBlue" id="btnREJL" style="color:#000000"; value="Select Rejection Reason" onClick="ShowRejList();">
    ...
    <TR>
	  ...
	  <INPUT name="OFF_CD" type="hidden" id="OFF_CD" value="<%=OFF_CD%>" size="50" maxlength="50" >
        <INPUT name="btnEDP" type="button" class="buttonBlue" id="btnEDP" value="Select  EDP Centers" style="color:#000000;" onClick="ShowEdpList();"></TD>
    ...
    <TR>
      ...
      <TD colspan="2"><INPUT name="Button" type="button" class="buttonMerun" style="color:#000000;" onClick="clearEDP();" value="CLEAR EDP DATA" title="After Clear Data Please Click on Save Button"></TD>
 ...
 <span class="redLebel">Aadhaar Details are invalid! Please ensure Aadhaar No, Name, Date Of Birth , Gender and mobile No Details are as per Aadhaar Card....... ! Kindly Reject/Cancel the MM Claim and Contact Concern Agency to Update Correct  Details in Applicant Data form </span>
		<%} else{%>
		<INPUT name="btnClaim" type="button" class="buttonBlue" id="btnClaim" style="color:#000000;" value=" Submit  Online Govt Subsidy (MM) Claim" onClick="openMM('<%= APP_ID %>');">
