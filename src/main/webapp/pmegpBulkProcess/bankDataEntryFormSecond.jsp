...
<script language="javascript" src="../js/bankModuleSecond.js"></script>
<script language="javascript" src="../js/inlineMsgMain.js"></script>
<script src="../js/ModalPopupWindow.js" type="text/javascript"></script>

</head>
<body onLoad="loan_calc();Status_change();">
<form  method="post"  name="form1" id="form1" action="bankDataEntryFormSecond.jsp" >
...
String IFSC_CODE = (String)session.getAttribute("SIFSC_CODE");
...
av.append("SELECT a.tr_id, b.app_id,a.act_id, to_char(a.bank_rec_dt,'DD-MON-YYYY') AS  bank_rec_dt, MS.STATE_CD,bank_remarks,AM.activity_name,100-10 AS BANK_LOAN_PER,a.RINT,a.LOAN_PERIOD, ").
...
append(" 10 OWN_CONT_PER,MM_PER_SEC(B.APP_ID) AS MM_ELIG_PER,").
...
append ( " to_char(a.tdr_adj_dt,'DD-MON-YYYY') as tdr_adj_dt, a.tdr_adj_amt,to_char(sysdate,'DD-MON-YYYY') as TODAYDT,B.IFSC_CODE, " ).
 append ( " a.bank_comment,  to_char(a.time_stamp,'DD-MON-YYYY:HH:MM:SS') as time_stamp ").
  append (" FROM bank_dataentry_second a, app_detail_second b,BENF_CATEGORY_MAST C, ACTIVITY_MAST AM,M_DISTRICT MD,M_STATE MS WHERE B.ACTIVITY_CD=AM.ACTIVITY_CD AND  b.APP_ID=? AND B.IFSC_CODE=?").
  append ("  AND B.UNIT_DIST_CD=MD.DISTRICT_CD AND MD.STATE_CD=MS.STATE_CD and b.app_id=a.app_id(+) and b.BENF_SPECAT_CD=C.BENF_CATEGORY_CD  ");
...
rsTrno= db.executeSQL("SELECT CIRCLE_NEFT AS NEFT_IFSC, circle_trans_acno AS TRANS_ACNO FROM BANK_BR_MAST  WHERE BR_IFSL_CODE=? ", values);
/*
...
if (rsApp.getString("TR_ID") != null){
	...
	if (request.getParameter("ACT_ID") != null){
		...
		String validateqry="SELECT * FROM TABLE(GET_PMEGP_BANKERROR_SEC(?,?,?,?,?,?,?,?,?,?,?,? ,?,?,?,? ,?,?,?,?,?,?,?,?, ?,?,?,?,?) )";
...
CallableStatement cs = conn.prepareCall("{call  PMEGPNEW_UPDATE_BANKDATA_SEC (?,?,?,?,?,?,?,? ,?,?,?,? ,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?)}");
...
<%
if (MMCLAIM.equals("Y")){
response.sendRedirect("bankfinalsubmissionClaimSec.jsp");
}
...
<input name="BANK_REC_DT" id="BANK_REC_DT" type="text"  tabindex="0" value="<%= BANK_REC_DT %>"  onClick="javascript:NewCssCal('BANK_REC_DT','ddMMMyyyy')" size="12" placeholder="dd-mom-yyyy" autocomplete="off"  ></td>
    ...
    <tr>
      ...
      <td colspan="2" nowrap><div align="center">
        <input name="MACHINARY_COST" id="MACHINARY_COST" type="text"  tabindex="6"      value="<%= MACHINARY_COST %>" size="12" maxlength="8" onChange="loan_calc();">
      </div></td>
      <td nowrap><div align="center">
        <input name="CE_SANC_FB" id="CE_SANC_FB" type="text"  tabindex="6"      value="<%= CE_SANC_FB %>" size="12" maxlength="8" onChange="loan_sanc();" >
      </div></td>
    <tr>
      <th>&nbsp;</th>
      <th>(b) Working Capital/Cash Credit (in Rs.) </th>
      <td colspan="2"><div align="center">
        <input name="WORKING_CAPITAL" id="WORKING_CAPITAL" type="text"  tabindex="8"     value="<%= WORKING_CAPITAL %>" size="12" maxlength="8" onChange="loan_calc();">
      </div></td>
      <td><div align="center">
        <input name="WC_SANC_FB" id="WC_SANC_FB" type="text"  tabindex="8"     value="<%= WC_SANC_FB %>" size="12" maxlength="8" onChange="loan_sanc();" >
      </div></td>
    <tr>
      <th>&nbsp;</th>
    ...
    <tr>
      ...
      <td colspan="3"><input name="LOAN_SANC_DT" type="text"  id="LOAN_SANC_DT" placeholder="dd-mom-yyyy"  value="<%= LOAN_SANC_DT %>" onClick="javascript:NewCssCal('LOAN_SANC_DT','ddMMMyyyy')" size="12" maxlength="11" autocomplete="off"></td>
    ...
    <tr>
      <th>(10)</th>
      <th>Deposit date: </th>
      <td colspan="3"><input name="OWN_CONT_DT" type="text"  id="OWN_CONT_DT" tabindex="9" placeholder="dd-mom-yyyy"  onClick="javascript:NewCssCal('OWN_CONT_DT','ddMMMyyyy')"  value="<%=OWN_CONT_DT %>" size="12" maxlength="11"></td>
    ...
    <tr>
      ...
      <td colspan="3"><input name="IST_LOAN_DT" id="IST_LOAN_DT" type="text" onClick="javascript:NewCssCal('IST_LOAN_DT','ddMMMyyyy')"  placeholder="dd-mom-yyyy" value="<%= IST_LOAN_DT %>" size="12" maxlength="12"  autocomplete="off"></td>
    ...
    <tr>
      <th>(18)</th>
      <th>&nbsp;</th>
      <th colspan="3"><input name="btnClaim" type="button" class="buttonBlue" style="color:#000000"; id="btnClaim" value=" Submit  Online Govt Subsidy (MM) Claim" onClick="openMM('<%= APP_ID %>');"></th>
    ...
    <tr>
      ...
      <td colspan="3"><input name="TDR_ADJ_DT" type="text" id="TDR_ADJ_DT"  placeholder="DD-MON-YYYY" value="<%= TDR_ADJ_DT %>" onClick="javascript:NewCssCal('TDR_ADJ_DT','ddMMMyyyy')" size="20" maxlength="20" readonly="true"></td>
    ...
    <tr>
      ...
      <td colspan="3"><input name="MM_ADJ_DT" type="text" id="MM_ADJ_DT"  value="<%= MM_ADJ_DT%>" onClick="javascript:NewCssCal('MM_ADJ_DT','ddMMMyyyy')" size="12" maxlength="11" readonly="true"></td>
