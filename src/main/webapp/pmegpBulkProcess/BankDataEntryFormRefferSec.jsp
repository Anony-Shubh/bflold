...
append ( " to_char(a.tdr_adj_dt,'DD-MON-YYYY') as tdr_adj_dt, a.tdr_adj_amt,to_char(sysdate,'DD-MON-YYYY') as TODAYDT,B.IFSC_CODE, " ).
 append ( " a.bank_comment,  to_char(a.time_stamp,'DD-MON-YYYY:HH:MM:SS') as time_stamp ").
  append (" FROM bank_dataentry_second a, app_detail_second b,BANK_BR_MAST BBM,BENF_CATEGORY_MAST C, ACTIVITY_MAST AM WHERE B.ACTIVITY_CD=AM.ACTIVITY_CD AND bbm.br_ifsl_code=b.ifsc_code AND  b.APP_ID=? AND BBM.CIRCLE_IFSC=?").
...
values.clear();
 ...
 values.add (APP_ID); 
	 ...
	 ResultSet rsEDPDOC= db.executeSQL("SELECT UP_ID,DOC_NAME FROM APP_UPLOAD_TRANS_SECOND WHERE UP_ID IN (SELECT MAX(UP_ID) FROM  APP_UPLOAD_TRANS_SECOND WHERE APP_ID=?  AND UPM_CD=8) AND DOC_ACTYN='Y'",values);
	 values.clear();
	 ...
	 ResultSet rsADDDOC= db.executeSQL("SELECT UP_ID,DOC_NAME FROM APP_UPLOAD_TRANS_SECOND WHERE UP_ID IN (SELECT MAX(UP_ID) FROM  APP_UPLOAD_TRANS_SECOND WHERE APP_ID=?  AND UPM_CD=21) AND DOC_ACTYN='Y'",values);
			values.clear();
 ...
 <table align="center" class="pmegpTable">
    ...
    <tr>
	 ...
	 <%  if (!DOC_NAME_ADD.equals("")) { %>
      ...
      <td colspan="2"><div align="center">
        <input name="btnAdd" type="button" class="buttonGreen" style="color:#000000"; id="btnAdd" value="UPLOAD" onClick="uploadAdd('<%= APP_ID %>')"> 
        </div></td>
    <tr>
      <th colspan="5">&nbsp;</th>
    <tr>
...
( Non customer A/c)	</td>
    ...
    <tr>
      ...
      <th colspan="4"><div align="center">
        <input name="btnClaim" type="submit" class="buttonBlue" id="btnClaim" style="color:#000000"; value="Final MM Claim Submission">
      </div></th>
  </table>
