...
<br> 
 <form name="form" id="form"  class="form" method="post">
   ...
   qrysb.append("  RBL.BANK_NAME,   RBL.BRANCHNAME, BD.MM_CLAIM_AMT,TO_CHAR(BD.MM_CLAIM_DT,'DD-MON-YYYY') AS MM_CLAIM_DT  , BD.PBATCH_ID,TO_CHAR( BD.BATCH_DATE,'DD-MON-YYYY') AS  BATCH_DATE  ,  BD.UTRN_NO,BBM.BR_UPDATE_YN," );
   ...
   qrysb.append("  FROM APP_DETAIL_SECOND AD,  BANK_DATAENTRY_SECOND BD,  MM_REMARKS_TRANS_SECOND MRT,  MAS_OFF_MAST MOM,  M_DISTRICT MD,  RBIBANKLIST RBL,BANK_BR_MAST BBM" );
   ...
   qrysb.append("  AND BD.MM_CLAIM_DT IS NOT NULL AND BD.LOAN_DOCFNAME IS NOT NULL AND BBM.CIRCLE_IFSC='"+IFS_CODE+"' " );
...
String TDR_YN="";
  ...
  <table border="1" align="center" cellpadding="1" class="pmegpTable" cellspacing="1" bordercolor="#666666" class="bordered" id="example">
    <thead>
    <tr>
      <th>Sr No </th>
  ...
  {	 
	  ...
	  <input name="btnBank1" id="btnBank1" type="button" class="button" style="color:#000000"; onClick="ShowNewPage('<%=APP_ID%>','bankDetailUpdationsecond.jsp');"  value="UPDATE TDR DETAIL" />
	<%} else if ( PACT_ID.equals("30") && APPRV_YN.equals("A")  ) {%>
	<a href="appidSnRefSec.jsp?APP_ID=<%= APP_ID %>&IFSC_CODE=<%=IFS_CODE %>" class="buttonGray"> Update  Referred Bank MM Claim For Second Loan
	 </a>
	...
	<%}%>
      ...
      <td><div align="left"><%if ( PACT_ID.equals("30") && APPRV_YN.equals("A") ){%><%= CO_REMARKS %><%}%></div></td>	  
