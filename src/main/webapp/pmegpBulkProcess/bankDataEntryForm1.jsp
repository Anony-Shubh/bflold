...
append(" CASE WHEN NVL(B.DLTFC_CE,0)+ NVL(B.DLTFC_WC,0)>0 THEN NVL(B.DLTFC_CE,0) ELSE nvl(B.machinary_cost,0) END AS Proj_cost_CE,B.MOB_NO1,B.E_MAIL, ").
append(" CASE WHEN NVL(B.DLTFC_CE,0)+ NVL(B.DLTFC_WC,0)>0 THEN NVL(B.DLTFC_WC,0) ELSE nvl(B.working_capital,0) END AS Proj_cost_WC,  ").
append(" CASE WHEN NVL(B.DLTFC_CE,0)+ NVL(B.DLTFC_WC,0)>0 THEN NVL(B.DLTFC_WC,0)+NVL(B.DLTFC_CE,0) ELSE nvl(B.working_capital,0)+nvl(B.machinary_cost,0) END AS Proj_cost,  ").
...
values.clear();
values.add (IFSC_CODE); 
ResultSet rsTrno= db.executeSQL("SELECT NVL(NEFT_IFSC,BR_IFSL_CODE) AS NEFT_IFSC,TRANS_ACNO FROM BANK_BR_MAST WHERE BR_IFSL_CODE=?",values);
values.clear();
...
<input name="BANK_REC_DT" id="BANK_REC_DT" type="text"  tabindex="0" value="<%= BANK_REC_DT %>"  size="12" placeholder="dd-mom-yyyy" autocomplete="off"  onClick="popUpCalendar(this, this.form.BANK_REC_DT,'dd-mmm-yyyy');return false;">
    ...
    <tr>
      <th rowspan="2">(3)</th>
      <th rowspan="2">Remarks ( Rejection / Under Process ) :</th>
    ...
    <tr>
	  ...
	  <option value="<%= TRANS_ACNO %>"<% if (FB_ACNO.equals(TRANS_ACNO)){out.print("selected");}%>>IFS CODE:<%= NEFT_IFSC %>  Ac No: <%= TRANS_ACNO %> </option>
	  </select>
