<%@ page session="true" import="java.sql.*,java.util.*,java.io.*,java.text.*,java.util.Date,java.text.SimpleDateFormat,pkgPmegpNew.DBCon"  %>
...
<body> 
 <form name="form" id="form"  class="form" >
		...
		String MAINIFSC = request.getParameter("MAINIFSC")==null?"":(String) request.getParameter("MAINIFSC");
...
ResultSet rsMax= db.execSQL(" SELECT SEQRRBCODE.NEXTVAL FROM DUAL");	
while (rsMax.next()) {
	BR_CODE=rsMax.getString("NEXTVAL");
}
rsMax.close();

IFSC_CODE=BR_CODE+MAINIFSC.substring(4,11);
...
ResultSet rsSTATE=db.execSQL("SELECT STATE_CD,STATE_NM FROM M_STATE ");		
  ...
  <tr>
    <th>RRB-IFS CODE </th>
    <td ><input name="IFSC_CODE" type="text" id="IFSC_CODE"  value="<%=IFSC_CODE%>" size="20" maxlength="20" readonly="true"></td>
    <th><input name="BR_CODE" type="hidden" id="BR_CODE"  value="" >
      NEFT_IFSC</th>
    <td><input name="NEFT_IFSC" type="text" id="NEFT_IFSC"  value="<%=MAINIFSC%>" size="20" maxlength="20" readonly="true"></td>  
  </tr>
   <tr>
    <th >BANK_NAME</th>
   ...
   <tr valign="baseline" class="style60">
     <th>BRANCHNAME</th>
     <td colspan="3"><input name="BRANCHNAME" type="text" id="BRANCHNAME" value="<%=BRANCHNAME %>" size="30" maxlength="30">
      <input name="MAINIFSC" type="hidden" id="MAINIFSC" value="<%= MAINIFSC %>"></td>
   </tr>
   <tr valign="baseline" class="style60">
...
%>
  ...
  <th>DISTRICT </th>
      <td><input name="DISTRICT_NAME" type="text" id="DISTRICT_NAME" value="<%=DISTRICT %>"  size="35" maxlength="40">
  ...
  <tr>
    <th >EMAIL ADDRESS</th>
    <td><input name="EMAIL" type="text" id="EMAIL" value="<%=EMAIL%>" size="50" maxlength="40">
    </td>
  ...
  <td>  <input name="MOBNO" type="text" id="MOBNO" value="<%=MOBNO %>" maxlength="15">    </td>  
...
<%
	 ...
	 // BR_CODE=request.getParameter("BR_CODE")==null?"":request.getParameter("BR_CODE");	
	  //IFSC_CODE=request.getParameter("IFSC_CODE")==null?"":request.getParameter("IFSC_CODE");
	  MICR_CODE=request.getParameter("MICR_CODE")==null?"":request.getParameter("MICR_CODE");
	  BANK_CODE=request.getParameter("BANK_CODE")==null?"":request.getParameter("BANK_CODE");
	  IFSC_ID=request.getParameter("IFSC_ID")==null?"":request.getParameter("IFSC_ID");	  
	  NEFT_IFSC=request.getParameter("NEFT_IFSC")==null?"":request.getParameter("NEFT_IFSC");	  
 ...
 sb.append("  INSERT INTO RBIBANKLIST_RRB( IFSC_CODE,MICR_CODE,BANK_CODE,  ");
 sb.append("  IFSCID,NEFT_IFSC,BANK_NAME,BRANCHNAME,ADDRESS,CENTRE,STATE_CD,DISTRICT,DISTRICT_CD,BANK_PINCD,  "); 
 sb.append("  TEL_NO,FAX_NO,EMAIL,CONT_NAME,MOBNO,DESIG,BRCODE) VALUES ( ?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,? )"); 


db.setSqlValue(sb.toString());

 values.add (IFSC_CODE);pstm.add ("D");
 values.add (MICR_CODE);pstm.add ("D");
 values.add (BANK_CODE);pstm.add ("D");
 values.add (IFSC_ID);pstm.add ("D");
 values.add (NEFT_IFSC);pstm.add ("D");
