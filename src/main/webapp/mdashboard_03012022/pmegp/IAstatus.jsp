...
String wcls="";
String srno ="";
String rFromdt=request.getParameter("FROMDT")==null?"":(String)request.getParameter("FROMDT");
String rTodt=request.getParameter("TODT")==null?"":(String)request.getParameter("TODT");
...
qrysb.append(" SELECT  nvl(mom.agency_type,'TOTAL') agency_type,COUNT(*), "      );
...
qrysb.append(" FROM APP_DETAIL AD,MAS_OFF_MAST MOM WHERE AD.OFF_CD=MOM.OFF_CD "      );
qrysb.append(" AND TRUNC(ad.online_subdt) BETWEEN '"+rFromdt+"' AND '"+rTodt+"'  GROUP BY ROLLUP(mom.agency_type) ORDER BY COUNT(*) ASC "      );
...
ResultSet rsMain = db.execSQL(qrysb.toString());
 %>
<CENTER>
  <H1>Application Status with Implementing Agency  for the period from <%= rFromdt %> to <%= rTodt %> ( <%= YR %> )
...
%>
  <TR>
    <TD><DIV align="center"><%= cnt %></DIV></TD>
     <TD><DIV align="left"><A href="statewiseIA.jsp?Agency=<%=STATE_NM%>&YR=<%=YR%>" target="_parent"><%= STATE_NM %></A></DIV></TD>
