...
qrysb.append("   SELECT NVL(STATE_NM,'TOTAL') AS AGENCY_TYPE, "      );
...
qrysb.append("     FROM app_detail ad,rbibanklist rbi,M_STATE MS,M_DISTRICT MD,"      );
qrysb.append("       bank_dataentry bd,MAS_OFF_MAST MOM "      );
...
ResultSet rsMain = db.execSQL(qrysb.toString());
 %>
<CENTER>
  <H2>For the Application handled by the bank for the period from <%= rFromdt %> to <%= rTodt %> ( <%= YR %> ) for <%=AGENCY%> (<%=AGN%> Agency)
...
<TABLE align="center" cellpadding="2" cellspacing="10" id="customers">
 <THEAD>
    ...
    <TR>
      <TH bgcolor="#FCDCF4"><DIV align="center"><%=rFromdt%></DIV></TH>
      <TH bgcolor="#FCDCF4"><DIV align="center">To</DIV></TH>
      <TH bgcolor="#FCDCF4"><DIV align="center"><%=rTodt%></DIV></TH>
...
%>
  <TR>
    <TD><DIV align="center"><%= cnt %></DIV></TD>
     <TD><DIV align="left"><A href="DistrictBankDetail.jsp?Agency=<%=STATE_NM%>&YR=<%=YR%>&Org=<%=AGENCY%>&AGN=<%=AGN%>" target="_parent"><%= STATE_NM %></A></DIV></TD>
