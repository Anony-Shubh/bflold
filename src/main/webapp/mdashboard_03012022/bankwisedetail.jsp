...
//////////////////////////////////////////agency
sb.append("   and  MOM.AGENCY_TYPE like '%"+AGENTYPE+"%'"      );
...
sb.append("      ) ORDER BY BANK_FORD DESC"      );
...
<IMG src="img/banner3.jpg" >   <BR>
	...
	<A class="btn btn-primary" href="pmegp/index.jsp">Pending Reports</A>
	 <%if(TYPE.equals("A")){%>
	<A hidden class="btn btn-primary" target="_blank" href="reports/dashboardrep.jsp?REP_NM=bankwise&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>">Download PDF</A>
	<A hidden class="btn btn-primary" href="reports/GenerateXLS.jsp?REP_NM=bankwise&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>" >Download Excel</A>
	<%}%>
	 <%if(TYPE.equals("S")){%>
	<A hidden class="btn btn-primary" target="_blank" href="reports/dashboardrep.jsp?REP_NM=bankwisestate&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>">Download PDF</A>
	<A hidden class="btn btn-primary" href="reports/GenerateXLS.jsp?REP_NM=bankwisestate&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>" >Download Excel</A>
	<%}%>
	 <%if(TYPE.equals("D")){%>
	<A hidden class="btn btn-primary" target="_blank" href="reports/dashboardrep.jsp?REP_NM=bankwisedistrict&STATE_NM=<%= STATE_NM %>&DISTRICT_CD=<%= DISTRICT_CD %>&DISTRICT_NAME=<%= DISTRICT_NAME %>&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>">Download PDF</A>
	<A hidden class="btn btn-primary" href="reports/GenerateXLS.jsp?REP_NM=bankwisedistrict&STATE_NM=<%= STATE_NM %>&DISTRICT_CD=<%= DISTRICT_CD %>&DISTRICT_NAME=<%= DISTRICT_NAME %>&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>" >Download Excel</A>
	<%}%>
...
<INPUT type="hidden" name="TYPE" id="TYPE" value="<%= TYPE %>">  
   ...
   <%if(AGENTYPE.equals("")){%>
	 ...
	 <%=AGENTYPE%>)</H3></DIV>
							  <%}%>
							  <%if(TYPE.equals("S")){%>
							<DIV align="center"><H3>Bankwise Details for <%= STATE_NM %> (Data from <%= FROMDT %> TO <%= TODT %>)(Agency: 
   <%if(AGENTYPE.equals("")){%>
	 All
	 <%}%>
	 <%=AGENTYPE%>)</H3></DIV>
                            <%}%>
							  ...
							  <%if(TYPE.equals("D")){%>
							<DIV align="center"><H3>Bankwise Details for <%= DISTRICT_NAME %>, <%= STATE_NM %> (Data from <%= FROMDT %> TO <%= TODT %>) (Agency: 
   <%if(AGENTYPE.equals("")){%>
	 All
	 <%}%>
	 <%=AGENTYPE%>)</H3></DIV>
                                    ...
                                    <TFOOT>
                                        <TR>
										  ...
										  <TH>Bank Name</TH>
                                            ...
                                            <TH>Bank Forwarded </TH>
                                            <TH>Loan Sanctioned </TH>
                                            <TH>Claim by Bank (No.)</TH>
                                            <TH>Claim by Bank (MM. in Cr.)</TH>
                                            <TH>MM Released (No.)</TH>
                                            <TH>MM Released (Amt. in Cr.)</TH>
                                            <TH>Pending for Credit Decision by Bank</TH>
                                            <TH>Rejected by Bank</TH>
...
%>									
                                        ...
                                        <TR>
										  ...
										  <TH><%= BANK_NAME %></TH>
                                            ...
                                            <TD><DIV align="right"><%= CLAIM_BY_BANK_NO %></DIV></TD>
                                            <TD><DIV align="right"><%= CLAIM_BY_BANK_MM %></DIV></TD>
                                            <TD><DIV align="right"><%= MM_REL_NO %></DIV></TD>
                                            <TD><DIV align="right"><%= MM_REL_AMT %></DIV></TD>
                                            <TD><DIV align="right"><%= FOR_DECISION %></DIV></TD>
                                            <TD><DIV align="right"><%= BANK_REJ %></DIV></TD>
                                            <TD hidden><A href="mmclaimpendingstatus.jsp?STATE_CD=<%= STATE_CD %>" class="btn btn-primary">More</A></TD>
											
											<%if(!BANK_NAME.equals(" TOTAL")){%>
											
											     <TD>
											<%if(DLEVEL.equals("1")){%>
											<A href="AppStatusRepoBank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=1&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>
											 <%if(DLEVEL.equals("2")){%>
											<A href="AppStatusRepoBank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=2&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>
											 <%if(DLEVEL.equals("3")){%>
											<A href="AppStatusRepoBank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=3&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>											</TD>
											
                                            <TD>
											<%if(DLEVEL.equals("1")){%>
											<A href="AgeWiseRepobank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=1&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>
											 <%if(DLEVEL.equals("2")){%>
											<A href="AgeWiseRepobank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=2&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>
											 <%if(DLEVEL.equals("3")){%>
											<A href="AgeWiseRepobank.jsp?STATE_CD=<%= STATE_CD %>&DLEVEL=3&BANK_NAME=<%= BANK_NAME %>" class="btn btn-primary">More</A>
											 <%}%>											</TD>
  <TD>
  
  <%if(DLEVEL.equals("1")){%>
  <A href="rejectiondetail_bankwise.jsp?BANK_NAME=<%= BANK_NAME %>&DLEVEL=1" class="btn btn-primary">More</A>
  <%}%>
    <%if(DLEVEL.equals("2")){%>
  <A href="rejectiondetail_bankwise.jsp?BANK_NAME=<%= BANK_NAME %>&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>&DLEVEL=2" class="btn btn-primary">More</A>
  <%}%>
    <%if(DLEVEL.equals("3")){%>
  <A href="rejectiondetail_bankwise.jsp?BANK_NAME=<%= BANK_NAME %>&DLEVEL=3&STATE_CD=<%= STATE_CD %>&STATE_NM=<%= STATE_NM %>&DISTRICT_CD=<%= DISTRICT_CD %>&DISTRICT_NAME=<%= DISTRICT_NAME %>" class="btn btn-primary">More</A>
  <%}%>  </TD>
											<%}else{%>
											<TD>-</TD>
                                            <TD>-</TD>
