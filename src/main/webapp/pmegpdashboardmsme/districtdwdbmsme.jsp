...
String VSTATECD=request.getParameter("STATECD")==null?"NO":(String) request.getParameter("STATECD");
...
qrysb.append("       district_name as state_nm, MAX(sq.state_nm) AS STATE_CD,"      );
...
qrysb.append("               AND ad.app_id = bd.app_id (+) AND MD.STATE_CD='"+VSTATECD+"' "      );
qrysb.append("       ) sq"      );
qrysb.append("       GROUP BY  ROLLUP(sq.district_name)"      );
qrysb.append("       ORDER BY district_name"      );
...
%>
  <tr>
    <td><div align="center"><%= cnt %></div></td>
     <td><div align="left"><a href="statewisedwdb_claim_dw.jsp?bankname=&YR=<%=YR%>" target="_parent"></a><%= STATE_NM %></div></td>
