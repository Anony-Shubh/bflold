...
function makerCheckerField()
{
    ...
    var role=document.form.DT_LOCK1.value;

    if ((role === "0") || (role === "1"))   {
        $('#PROD_DESC').attr('readonly', 'readonly');
    } else {
        $('#PROD_DESC').removeAttr('readonly');
    }
} 
</script>
<%!
...
<form name="form"  id="form" method="post"  >
  ...
  <tr bgcolor="#D1A476">
    <td colspan="7">
      <h3 align="center">PMEGP APPLICANT DATA </h3>
    </td>
  </tr>
  <tr>
    <th width="26">(1)</th>
...
%>
  ...
  <tr>
    ...
    <td colspan="2"><input name="APP_DIST_CD" type="hidden" id="APP_DIST_CD" value="<%=APP_DIST_CD  %>"/>
      District*:</td>
    <td><input name="APP_DIST_NM" type="text" id="APP_DIST_NM"  style="background-color: #FFCCCC" readonly="true" value="<%= APP_DIST_NM %>" /></td>
    <td>Pin*:</td>
    <td colspan="2"><input name="APP_PIN" type="text" id="APP_PIN"  onkeypress="return numbersonly(event);" size="10" maxlength="6" value="<%=APP_PIN  %>"/></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
 ...
 ResultSet rslock = db.execSQL("SELECT COUNT(APP_ID) as DT_LOCK FROM BANK_DATAENTRY WHERE APP_ID='"+APP_ID+"' AND NVL(MM_CLAIM_AMT,0) >0    ");
	  ...
	  %>
	 <input type="hidden" name="DT_LOCK1" id="DT_LOCK1" value="<%=DT_LOCK1%>" /> 
  <tr>
