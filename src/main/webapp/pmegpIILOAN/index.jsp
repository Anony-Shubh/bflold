...
var xmlHttp   					//index
 ...
 var xmlHttp   					//MUDRADETAILS
      var xmlHttp 
      function mudra(str1,PAN_NO,STATE,UAD,DISTRICT,SCHEME_NM){ 
	  try
    {
    xmlHttp=new XMLHttpRequest();
    }
    ...
    try
	 ...
	 document.getElementById('MUDRA').innerHTML = '';
      ...
      var url="mudraDetails.jsp"; 
       url +="?state="+STATE+"&aadhar="+str1+"&pan="+PAN_NO+"&appid="+appid+"&UAD="+UAD+"&dis="+DISTRICT+"&schemenm="+SCHEME_NM;
...
return false;	
	...
	if (vstatex.indexOf(STATE) >-1) {
	 ...
	 if(DISTRICT == "-1") {
	  ...
	  /*if(appid.length < 10  && appid.length !="" && SCHEME_NM == "PMEGP") {
				inlineMsg('APP_ID','Please Enter Full Application ID',2);
				return false;
			  }*/
	...
	mudra(str1,PAN_NO,STATE,UAD,DISTRICT,SCHEME_NM);	
	document.getElementById("REC").style.backgroundColor  = "GREEN";	
}
...
String UPDATE= (String) session.getAttribute("UPDATE")==null?"":(String)session.getAttribute("UPDATE");
String SESSION_ID=(String) session.getAttribute("SESSION_ID")==null?"":(String)session.getAttribute("SESSION_ID");
if ((SESSION_ID.equals("")) && (UPDATE.equals(""))){
...
<BR><table align="center">
  <tr><input name="FLAG" type="hidden" id="FLAG" value="1">
    <td colspan="3"><div align="left">
      ...
      <input name="Button"  type="button" disabled="true"  class="button" id="CHR"   value="Personal Details" />
      <input name="Button2" type="button" disabled="true" class="button" id="REC"   value="Previous Sanction Details" />
      <input name="Button3" type="button" disabled="true" class="button" id="CON"   value="Apply Online" />
     <input name="Button3" type="button" disabled="true" class="button" id="CAL"   value="Upload Documents" />
	 <input name="Button3" type="button" disabled="true" class="button" id="FS"   value="Final Submit" />
...
ResultSet rsRd = db.execSQL("select round(dbms_random.value(1000000,9999999),0) SECL_ID from SYS.dual ");
...
if(MOB_NO1.length() == 10){
	// String sms_string="Dear  "+APP_NAME+ ",Your PMEGP e-Tracking Application is registered. User  ID is "+tempUN+" and password is: "+APP_PWD+".Please Login for Upload //Documents and Final Submission. After Final Submission You can Print Application form. -KVIC ";
	 String sms_string="Dear  "+APP_NAME+ " ,Your PMEGP  Portal Application is registered. User  ID is "+tempUN+" and password is "+APP_PWD+". Please Login for Upload Documents and Final Submission. After Final Submission You can Print Application form. -KVIC ";
	 SMSServices sms1=new SMSServices();
	try {
     String smsresponse=sms1.sendSingleSMS("KVICDIT", "Panda@1974",sms_string, "KVICIT",MOB_NO1,"2740939a-a79d-42ba-984c-02fb4fa0e10d","1007053449928293814");	
}catch (Exception e) {}
