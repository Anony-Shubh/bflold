...
String Appid="";
String S=session.getAttribute("sAPP_ID")==null?"":(String) session.getAttribute("sAPP_ID");
//out.print(S);
String DIS = request.getParameter("DIS")==null?"N":(String) request.getParameter("DIS");
if(DIS.equals("N")){
Appid=S;
}else{
Appid=session.getAttribute("sAPP_ID")==null?"":(String) session.getAttribute("sAPP_ID");
}
...
qrysb.append("   SELECT AD.OFF_CD,to_char(AD.UNIT_ESTB_YR,'DD-MON-RRRR') AS UNIT_ESTB_YR," );
...
qrysb.append("     AD.APP_POST_ADDR,TRUNC((SYSDATE-AD.DOB)/365) AS AGE,TO_CHAR(AD.DOB,'DD-MM-RRRR') AS DOB," );
...
qrysb.append("     AD.UNIT_TALUK_BLOCK,AD.BENF_CATAGORY_CD," );
...
qrysb.append("   FROM APP_DETAIL_ONLINE_SECOND AD," );
...
<BR><table align="center">
  <tr><input name="FLAG" type="hidden" id="FLAG" value="1">
    <td colspan="3"><div align="left">
	  ...
	  <input name="Button3" type="button" disabled="true" class="buttonGreen" id="EDIT"  onclick="finalCalc();" value="Edit Application" />
...
<%}%>
  ...
  <tr bgcolor="#D1A476">
    <td colspan="7">
      <h3>ONLINE APPLICATION FOR SECOND LOAN SUBSIDY FOR UPGRADING OF EXISTING UNIT </h3>    </td>
  </tr> <input name="DIS" type="hidden" id="DIS" value="<%=DIS%>" />
  <input name="SESSION_ID" type="hidden" id="SESSION_ID" value="<%=SID%>" />
  <tr>
    <th>(1)</th>
  ...
  <tr>
    ...
    <td colspan="4"><select name="BENF_CATAGORY_CD" class="button" id="BENF_CATAGORY_CD" >
      <option value="" selected="selected">-Social Category-</option>
  ...
  <tr>
    ...
    <th nowrap> Capital Expenditure ( In Rs.) </th>
    <th nowrap>Working Capital<strong> (In Rs.) </strong></th>
    <th nowrap><strong>Total</strong></th>
    <th nowrap>Employment</th>
  </tr>
  <tr>
    <td>&nbsp;</td>  <input type="hidden" name="ins" id="ins" value="" />
    </th>( In rupees) 
    <td colspan="2">&nbsp;</td>
    <td><div align="center">
    </div>      
    ...
    <td><input name="WORKING_CAPITAL" type="text" id="WORKING_CAPITAL"  onchange="proj_cal();" onkeypress="return numbersonly(event);" size="8" maxlength="7"  value="<%=WORKING_CAPITAL  %>" /></td>
 ...
 rs = db.execSQL("SELECT  DISTINCT(TRIM(UPPER(A.BANK_NAME))) AS BANK_NAME FROM rbibanklist A");
  ...
  <tr>
    <!--<th>(21)</th>
    <th colspan="2" nowrap>2nd Financing Branch IFS Code-->
      <input type="hidden" name="BANK_TYPE" id="BANK_TYPE" value="1" />
        <input type="hidden" name="ins" id="ins" value="" /></th>
    <td colspan="4"><input name="IFSC_CODE2" type="hidden" id="IFSC_CODE2" maxlength="11" value="<%=IFSC_CODE2  %>" /></td>
  </tr>
  <tr>
...
UNIT_ESTB_YR=(String) request.getParameter("UNIT_ESTB_YR")==null?"":(String) request.getParameter("UNIT_ESTB_YR").trim();
...
BENF_CATAGORY_CD=(String) request.getParameter("BENF_CATAGORY_CD")==null?"NA":(String) request.getParameter("BENF_CATAGORY_CD").trim();
...
qryUpdate.append("PAN_NO=?, BENF_CATAGORY_CD=?,");
...
if (DIS.equals("Y")){
response.sendRedirect("editpmegpapplication.jsp?DIS=Y&app_id="+APP_ID);
}
else{
response.sendRedirect("validateBeforeSubmit.jsp?DIS=N&app_id="+APP_ID);
}
