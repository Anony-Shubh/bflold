<%@ page buffer="1000kb" session="true" import="java.sql.*,java.util.*,java.text.*,java.util.Date,java.text.SimpleDateFormat,pkgPmegpNew.DBCon,java.math.BigDecimal"  %><head>
   ...
   <SCRIPT>
    function view()
  {
  var MONTH_DT= document.getElementById("INVDT").value;
if(MONTH_DT == "") {
...
window.location.href = 'GenerateExcel.jsp?REPNM=samrecomamount&MONTH='+MONTH_DT;
...
BigDecimal noapp=new BigDecimal("0");
  ...
  %>
   ...
   <label></label>
    ...
    <td><input name="Submit3" type="submit"  value="FIND"  class="login100-form-btn"  > </td> 
  <td> <input type="button" class="login100-form-btn" onClick="view();" value="Download Report" > </td> </tr>
  </table>
  </center>
 ...
 <% if (sInvDt.equals("")) { out.print("<center><h2>Please Select Invoice Month</h2></center>"); }else{%>
  ...
  <table id="example" class="display" style="width:100%">
    <thead>
      <tr>
        <th width="18%">State Name</th>
        <th width="29%">Office Name</th>
        <th width="16%"> Date</th>
        <th width="6%">KVIC</th>
        <th width="11%">KVIB </th>
        <th width="8%">DIC</th>
        ...
        <th width="12%">Upload Invoice</th>
        <th>Invoice No </th>
		 <th>Invoice Date </th>
		   <th>Amount</th>
		   <th>Download </th>
		   <th>Delete</th>
		   <th>Forward To Kvic</th>
      </tr>
    </thead>
    <tbody>
      <%
...
qrysb.append("    sq.inv_no, sq.updoc_edp,sq.total_amt, TO_CHAR(sq.inv_date,'DD-MON-YYYY')inv_date, sq.forward_dt,sq.FOPAY_DT,sq.FOPAY_AMT  "      );
...
qrysb.append("    ) mq,( SELECT  boi.off_cd,boi.inv_no,boi.updoc_edp,boi.total_amt,boi.inv_date,boi.FOPAY_DT,boi.FOPAY_AMT,"      );
...
FOPAY_DT=rsMain.getString("FOPAY_DT")==null?"0":rsMain.getString("FOPAY_DT");
FOPAY_AMT=rsMain.getString("FOPAY_AMT")==null?"0":rsMain.getString("FOPAY_AMT");
...
%>
      <tr>
        ...
        <a href="otherdocu/<%=updoc_edp%>" class="login100-form-btn">Download</a></button>
	       ...
	       <td>
		    <%if((FOPAY_AMT.equals("0")) && (!invupid.equals("0"))){%>
			
			 <input type="button" name="Submit" class="login100-form-btn" 
		  onClick="Del('<%=off_cd%>','<%=inv_no%>','<%=invupid%>');" value="Delete"/>
...
modalWin.ShowURL("UploadEDP.jsp?off_cd="+off_cd+"&invupid="+dt+"&no_ofapp="+no_ofapp,700,700,'CERTIFICATE',callbackFunctionArray,null);
 ...
 function Del(off_cd,dt,invupid){
//alert (off_cd+dt+no_ofapp);
var callbackFunctionArray = new Array(refreshpage);
modalWin.ShowURL("DelEDP.jsp?off_cd="+off_cd+"&invno="+dt+"&invupid="+invupid,700,700,'CERTIFICATE',callbackFunctionArray,null);
 } 
