...
String OFF_EMAIL_ADDR="";
    ...
    if (!CITY.equals("")){
       vWherecls=vWherecls+" AND TRIM(UPPER(OFF_ADDR2)) ||' '||TRIM(UPPER(MOM.CITY))||' '||TRIM(UPPER(MD.DISTRICT_NAME)) ||' '||MOM.PINCODE||' '||TRIM(UPPER(OFF_LONG_DESC))||' '||TRIM(UPPER(MS.STATE_NM))||' '||TRIM(UPPER(MOM.OFF_NAME))||' '||TRIM(UPPER(MOM.OFF_CD))  LIKE  UPPER('%"+CITY+"%')  ";
   }
...
qrysb.append(" SELECT MOM.OFF_CD,MOTC.OFF_TYPE_SHORT_DESC, MOM.OFF_EMAIL_ADDR,MOM.OFF_LONG_DESC,MOM.OFF_NAME,MOM.CITY,MD.DISTRICT_NAME|| ' , '|| MS.STATE_NM AS DISTRICT_NAME,  " );
qrysb.append(" MD.DISTRICT_CD,MOM.PINCODE,MS.STATE_NM FROM MAS_OFF_MAST MOM,  " );
qrysb.append(" M_DISTRICT MD,M_STATE MS,MAS_OFF_TYPE_CDMAST MOTC WHERE MOM.OFF_TYPE_CD IN (26,17,18,19,28,29,30,31,32,33,34)  AND MOM.OFF_TYPE_CD=MOTC.OFF_TYPE_CD " );
...
// ResultSet rsDist=db.execSQL(" SELECT MD.DISTRICT_CD,MD.DISTRICT_NAME FROM M_DISTRICT MD WHERE MD.DISTRICT_STATUS='A'  ORDER BY DISTRICT_NAME");
 ...
 %> 
    ...
    <p><span style="font-weight: bold">Note:Accrediated/Approved EDP Training Centers By Central Office
    </span> </p>
  </div>
  <table align="center" class="bordered">
    ...
    <tr>
      <td> Search </td>
      <td><input name="CITY" type="CITY" size="20" maxlength="20" value="<%= CITY %>" placeholder="Search"></td>
...
<div id = "view"  >
  <table id="example" align="center" class="bordered">
    ...
    <tr>
	  ...
	  <th><div align="center">Officail Mail ID </div></th>
	  ...
	  <th><div align="center">UPDATE DETAIL</div></th>
	   <th><div align="center">RESEND PASSWORD</div></th>
    </tr>
...
edpcount=edpcount+1;
	...
	<tr>
	 ...
	 <td>	
			...
			<td>	
		    <div align="center">
			<% if (OFF_EMAIL_ADDR.equals("") || OFF_EMAIL_ADDR.equals("0")) {%>
			Please Update Officail Mail Id
			<%} else{%>
			     <input name="btnBank3" type="button" class="button" id="btnBank3" onClick="ShowNewPage('<%= OFF_CD %>','sendMail.jsp');"  value="Send Email" /> 
				 <%}%>
            </div></td>
    </tr>
	<% 
...
function sendemail(vOFFCD,filename){
var urlpar=filename+'?OFF_CD='+vOFFCD;
 var callbackFunctionArray = new Array(null, null);
 modalWin.ShowURL(urlpar,600,1000,'Send Email',callbackFunctionArray,null);
 }
