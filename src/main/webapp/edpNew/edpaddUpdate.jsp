...
String AGENCY_TYPE="";
  ...
  {			
	  ...
	  AGENCY_TYPE=rsMain.getString("AGENCY_TYPE")==null?"":rsMain.getString("AGENCY_TYPE");
			...
			if (!OFF_CD.equals("0")){
       vWherecls=vWherecls+" where  district_cd="+DISTRICT_CD+" ";
   }
   else{
   vWherecls=vWherecls+" where  district_cd LIKE '%%' ";
   }
  ...
  <table align="center">
  <th colspan="4" bgcolor="#FF9E5E"><div align="center"> 
...
<tr bgcolor="#FF9E5E" >
 ...
 <tr>
   <th colspan="4" class="redLebel"><div align="center"></div></th>
   </tr>
 <tr>
 <th>EDP Center ID: </th>
...
%>
    ...
    <tr valign="baseline" class="style60">
      ...
      <td colspan="3"><input name="OFF_LONG_DESC" type="text" id="OFF_LONG_DESC" value="<%=OFF_LONG_DESC %>" size="70" maxlength="70"></td>
    </tr>
    <tr valign="baseline" class="style60">
	...
	<th align="right" nowrap>City/Town:</th>
      <td><input name="CITY" type="text" id="CITY" value="<%=CITY %>" size="40" maxlength="40"></td>
	  <td>STATE</td>
      ...
      <% 
	   String vSTATE_CD= "";
          ...
          ResultSet rsState= db.execSQL("SELECT STATE_CD,STATE_NM FROM M_STATE WHERE STATE_CD in (SELECT STATE_CD FROM M_DISTRICT  "+vWherecls+" ) ");
		 
		  while (rsState.next()){
	      vSTATE_CD=rsState.getString("STATE_CD")==null?"":rsState.getString("STATE_CD");
		  vSTATE_NAME =rsState.getString("STATE_NM")==null?"":rsState.getString("STATE_NM");
		 
  %>
     <option value="<%=vSTATE_CD %>" ><%=vSTATE_NAME%></option>
<% }
rsState.close();
%>
     </select></td>
    ...
    <td><select name="DISTRICT_CD"  id="DISTRICT_CD">
      ...
      <% 
	   String vDISTRICT_CD = "";
          ...
          ResultSet rs= db.execSQL("SELECT DISTRICT_CD,DISTRICT_NAME FROM M_DISTRICT WHERE STATE_CD in (SELECT STATE_CD FROM M_DISTRICT "+vWherecls+") ");
...
%>
  ...
  <tr>
  <th >ACTIVE:</th>
  <td colspan="3"> <select name="ACT_YN" id="ACT_YN">
    <option value="Y" <% if (ACT_YN.equals("Y")){out.print("selected");} %>>Yes</option>
	 <option value="N" <% if (ACT_YN.equals("N")){out.print("selected");} %>>No</option>
  </select> </td>
  </tr>
  </table>
...
<%
	  ...
	  ACT_YN=request.getParameter("ACT_YN")==null?"":request.getParameter("ACT_YN");
...
rsMax.close();
 ...
 sb.append("  CONT_PER1,CONT_PER2,PER1_NO,PER2_NO,PER1_EMAIL,PER2_EMAIL,JURIS_DIST_CD,LATITUDE,LONGITUDE,ACT_YN,TIME_STAMP,OFF_CD) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSDATE,?)"); 
}
else
{
...
sb.append("   CONT_PER2=?,PER1_NO=?,PER2_NO=?,PER1_EMAIL=?,PER2_EMAIL=?,JURIS_DIST_CD=?,LATITUDE=?,LONGITUDE=?,ACT_YN=?,TIME_STAMP=SYSDATE ");
