...
String vfromdt=request.getParameter("FROMDT")==null?"":(String) js.getOnlyTextspc(request.getParameter("FROMDT"),12);
String vtodt=request.getParameter("TODT")==null?"":(String) js.getOnlyTextspc(request.getParameter("TODT"),12);;
String STATE=request.getParameter("STATE")==null?"":(String) js.getOnlyTextspc(request.getParameter("STATE"),3);
String BANK_NAME=request.getParameter("BANK_NAME")== null?"":(String) js.getOnlyTextspc(request.getParameter("BANK_NAME"),80);
...
Wcls=Wcls+" AND AD.BANK_NAME ='"+BANK_NAME+"' ";
}

else {
	Wcls=Wcls+" AND AD.BANK_NAME LIKE '%%' ";
}
...
qrysb.append("   SELECT NO_APP_FORWD ,"      );
...
qrysb.append("     WHERE AD.APP_ID = BD.APP_ID(+) AND AD.UNIT_DIST_CD=MD.DISTRICT_CD AND MD.STATE_CD=MS.STATE_CD "+vWherecls+" "      );
qrysb.append("     AND AD.OFF_CD   = MOM.OFF_CD AND TRUNC(AD.BANK_F_DATE) BETWEEN '"+vfromdt+"' AND '"+vtodt+"' "      );
qrysb.append("     AND AD.ACT_ID=5 AND AD.ACT_YN='Y' "+Wcls+" "      );
qrysb.append("     ) "      );
...
%>
<center>
  <h1> Rejection By Bank Status Report Of <%=BANK_NAME%> </h1>
</center>
...
<div id="piechart_3d"></div>
 ...
 <table width="80%" align="center" class="bordered" id="tableid">
  ...
  <tr>
	...
	<tr><td>1</td>
	  <td>Total No of Application Forwarded</td>
	  <td><%=NO_APP_FORWD%></td>
	   <td>-</td></tr>
	   <tr>
	     <td>2</td>
