...
sb.append("     NVL(DISTRICT_NAME,'ALL TOTAL') AS DISTRICT_NAME,MAX(DISTRICT_CD) as DISTRICT_CD,"      );
...
sb.append("     FROM"      );
sb.append("       (SELECT (MD.DISTRICT_CD) AS DISTRICT_CD,"      );
sb.append("         MD.DISTRICT_NAME,"      );
sb.append("         SUM("      );
sb.append("         CASE"      );
...
sb.append("       GROUP BY (MD.DISTRICT_NAME, MD.DISTRICT_CD)"      );
sb.append("       )"      );
sb.append("     UNION ALL"      );
sb.append("       (SELECT MD.DISTRICT_CD,"      );
sb.append("         MD.DISTRICT_NAME,"      );
sb.append("         0,"      );
sb.append("         0,"      );
sb.append("         0,"      );
...
sb.append("        and AD.BENF_CATAGORY_CD IN ('ST')  "      );
sb.append("       AND MS.STATE_CD='"+STATE_CD+"'"      );
sb.append("       "      );
sb.append("       GROUP BY MD.DISTRICT_CD,"      );
sb.append("         MD.DISTRICT_NAME"      );
sb.append("       )"      );
sb.append("     )GROUP BY ROLLUP(DISTRICT_NAME)"      );
sb.append("     ORDER BY DISTRICT_NAME"      );
sb.append("      "      );
sb.append("      "      );
sb.append("      "      );
sb.append("      "      );
	...
	<A class="btn btn-primary" href="index.jsp">Home</A>
                    ...
                    <DIV class="card shadow mb-4">
                        ...
                        <DIV class="card-body">
							...
							<DIV align="center"><H3>PMEGP Portal online Performance Report for ST<br><%= STATE_NM %>  Data from <%= FROMDT %> TO <%= TODT %> (Agency: 
   <%if(AGENTYPE.equals("")){%>
	 All
	 <%}%>
	 <%=AGENTYPE%>)
...
<%if(DISTRICT_NAME.equals("ALL TOTAL")){%>	
...
<TD><DIV align="right"><A target="_blank" href="reports/GenerateXLS.jsp?AGENTYPE=<%= AGENTYPE %>&TYPE=D&REP_NM=detailed_pend&REPTYPE=4&DISTRICT_CD=<%= DISTRICT_CD %>&DISTRICT_NAME=<%= DISTRICT_NAME %>&STATE_NM=<%= STATE_NM %>&FROMDT=<%= FROMDT %>&TODT=<%= TODT %>"><%= PENDING_AGENCY %></A></DIV></TD>
