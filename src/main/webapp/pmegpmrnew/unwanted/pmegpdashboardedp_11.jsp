...
qrysb.append("   0 PREONSAN_EDP, "      );
qrysb.append("   0 AS POSONSAN_EDP,0 AS PREOFSAN_EDP,"      );
qrysb.append("   0 AS POSOFSAN_EDP, 0 AS PREON_EDP,"      );
qrysb.append("   0 AS POSTON_EDP, 0 AS TOTON_EDP,0 AS PREOF_EDP,"      );
qrysb.append("   0 AS POSTOF_EDP, 0 AS TOTOF_EDP, 0 AS TOT_EDP,0 AS COMPREON_EDP,"      );
qrysb.append("   0 COMPOSTON_EDP, 0 AS COMPREOF_EDP, 0 AS COMPOSTOF_EDP, "      );
qrysb.append("   0 AS PENPREON_EDP,0 AS PENPOSTON_EDP, 0 AS PENPREOF_EDP,"      );
qrysb.append("   0 AS PENPOSTOF_EDP"      );
qrysb.append("   FROM APP_DETAIL ado, BANK_DATAENTRY bd, m_district DM, m_state ms, mas_off_mast mom, ZONE_MAST ZM "      );
qrysb.append("   WHERE ado.APP_ID = bd.APP_ID(+) AND ado.UNIT_DIST_CD = DM.DISTRICT_CD(+) AND DM.STATE_CD = ms.STATE_CD(+) "      );
qrysb.append("    AND ado.OFF_CD = mom.OFF_CD(+) AND ms.ZONE_CD = ZM.ZONE_CD "      );
...
<td  ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=6">	<%=TOTOF_EDP  %></td>
<td  ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=7"><%=TOT_EDP  %></a></td>
<td  ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=16"><%=BANK_FORW  %></a></td>
<td colspan="2"  ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=17"><%=PREONSAN_EDP  %></td>
<td colspan="2"  ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=18">	<%=PREOFSAN_EDP  %></td>
...
<!--<td ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=15"><%=PENPOSTOF_EDP  %></a></td>-->
<!--<td nowrap="nowrap"><a href="TrainingEdp.jsp<%=urlrep+p2+vnmdw+p1+firstcolold%>">view</a></td>-->
<td nowrap="nowrap"><a href="centerWiseEdp.jsp<%=urlrep+p2+vnmdw+p1+firstcolold%>">view</a></td>
<% } else{%>
...
<td  ><a href="<%=repnmE+urlrep+p3%>&QRYCODE=16&REPTP=4"><%=BANK_FORW  %></a></td>
...
<a href="centerWiseEdp.jsp?<%=p3+vFurl%>">view</a>
<%}%></td>
