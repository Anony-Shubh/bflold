...
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS not NULL   and qc_apprv='Y' THEN 1 ELSE 0 END) AS QC_PEN,  ");
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS NOT  NULL AND ACT_ID=1  and qc_apprv='Y'  THEN 1 ELSE 0 END) AS QC_COMP,  ");
...
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND    GT.MM_ADJUSTMENT_STATUS='RM' AND ((NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> 0) AND NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> NVL(A.MM_REL_AMT,0)) THEN 1 ELSE 0 END) AS PARDIS_NO,  ");
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND  GT.MM_ADJUSTMENT_STATUS='RM' AND  ((NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> 0) AND NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> NVL(A.MM_REL_AMT,0)) THEN  ROUND(MM_ADJUSTMENT_AMT_S / 100000, 2) ELSE 0 END) AS PARDIS_MM, ");
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND   GT.MM_ADJUSTMENT_STATUS='RM' AND ((NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> 0) AND NVL(GT.MM_ADJUSTMENT_AMT_S,0) <> NVL(A.MM_REL_AMT,0)) THEN  ROUND((NVL(A.MM_REL_AMT,0)-GEOALLOWAMT(GT.PMEGPGEOTAGID)) / 100000, 2) ELSE 0 END) AS CALL_BCK, ");
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS  NULL   and qc_apprv='Y' THEN 1 ELSE 0 END) AS AGEN_PEN,   ");
qrysb.append(" SUM(CASE WHEN gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS NOT  NULL AND NVL(ACT_ID,0) IN (0,2) and qc_apprv='Y'  THEN 1 ELSE 0 END) AS NAGEN_PEN ");
...
%>
       ...
       <td ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=2"><%=COM  %></a></td>
        ...
        <td ><a href="<%=repnmE+urlrep+p2+vnmdw+p1+firstcolold%>&QRYCODE=10"><%=TOTOF_EDP  %></a></td>
