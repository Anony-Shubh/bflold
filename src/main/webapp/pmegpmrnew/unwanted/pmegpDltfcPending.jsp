...
String ReportDesc=" STATUS OF APPLICATION WITH DLTFC FOR APPROVAL PERIOD from "+FromDt +" to "+ToDt+" " ; 
...
append(" SELECT"+qryField+"").
 ...
 append(" ,COUNT(AD.APP_ID) as APP_REC ").
  ...
  append("  ,sum( nvl(case when NVL(AD.MTG_ID,0)>0 then 1 end ,0)) as dtfc_place ").
 ...
 append("  ,sum( nvl(case when (ad.act_id<>7 AND AD.ACT_ID>=4) then 1 end ,0)) as dtfc_sanction ").
  append(" ,sum( DECODE(ad.act_id,5, 1,0)) as bank_ford ").
 ...
 append("  ,sum( nvl(case when BD.ACT_ID>=11 then  ROUND( MM_AMT_BANKSANC(BD.APP_ID)/100000,2) end ,0)) as bank_sanc_mm ").
 append(" ,sum( nvl(case when bd.MM_CLAIM_AMT>0 AND BD.LOAN_DOCFNAME IS NOT NULL and bd.act_id>=11 then 1 end ,0)) as mm_claim ").
  append("  ,sum( nvl(case when bd.MM_CLAIM_AMT>0 AND BD.LOAN_DOCFNAME IS NOT NULL  and bd.act_id>=11  then round(bd.mm_claim_amt/100000,2) end ,0)) as mm_claim_amt ").
 append(" ,sum( nvl(case when bd.MM_REL_DT IS NOT NULL and bd.act_id>=11 then 1 end ,0)) as mm_rel ").
   append(" ,sum( nvl(case when MM_REL_DT IS NOT NULL and bd.act_id>=11 then round(bd.mm_rel_amt/100000,2) end ,0)) as mm_rel_amt ").
 append("  ,sum( nvl(case when bd.edp_cert_dt IS NOT NULL and bd.act_id=11 then 1 end ,0)) as edp_cert_iss ").
  append(" ,sum( nvl(case when  bd.act_id=3 then 1 end ,0)) as bank_reject ").
  append(" ,sum( nvl(case when  bd.ACT_ID not in (11,3) and ad.act_id=5 then 1 end ,0)) as pend_bank_sanc ").
   append(",sum( nvl(case when bd.ACT_ID not in (11,3) and ad.act_id=5 then round(mminvolve(bd.app_id)/100000,2) end ,0)) as pend_bank_sanc_mm ").
    append(" ,sum( nvl(case when bd.mm_claim_dt IS NOT NULL AND bd.MM_REL_AMT IS NULL and bd.act_id>=11 then round(bd.mm_claim_amt/100000,2) end ,0)) as Pend_mmrel_amt ").
   append("  ,sum( nvl(case when bd.mm_claim_dt IS NOT NULL AND bd.MM_REL_AMT IS NULL and bd.act_id>=11  then 1 end ,0)) as Pend_mmrel ").
   append("  ,sum( nvl(case when bd.pv_dt IS NOT NULL AND BD.ACT_ID>=11 then 1 end ,0)) as pv_no ").
...
<td>
    <div align="right">
	<a href="dwstatewise.jsp?<%=p3+"&QRYCODE=3"%>">
...
<td><div align="right">
<a href="dwstatewise.jsp?<%=p3+"&QRYCODE=10"%>">
...
<td><div align="right">
<a href="dwstatewise.jsp?<%=p3+"&QRYCODE=15"%>">
...
<td>
 <div align="right"><a href="dwstatewise.jsp?<%=p3+"&QRYCODE=13"%>">
<% out.print(dfd.format(rsinst.getDouble("pv_no")));%></a> </div></td>
