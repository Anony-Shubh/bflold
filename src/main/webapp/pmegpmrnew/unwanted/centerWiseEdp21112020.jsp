...
String repnm="centerWiseEdp.jsp";
 ...
 urlrepE ="edpJasperRepo.jsp?TODT="+ToDt+"&FROMDT="+FromDt+"&AGENCY_TYPE="+vAgency+"&ZONE_NM="+vZonecd+"&STATE_NM="+vStatecd+"&OFF_NAME1=";
 String EDPOFF_CD="";
String STATE_NM="";
String OFF_NAME="";
String PREON_EDP="";
...
qrysb.append("   SELECT A.district_name as zone_nm , nvl(A.OFF_NAME,'Total')  as off_name1 , "      );
...
qrysb.append("   sum(A.PENPOSTOF_EDP) AS PENPOSTOF_EDP FROM (SELECT DM.district_name , mom.OFF_NAME,"      );
qrysb.append("   0 AS BANK_FORW,0 AS PREONSAN_EDP,0 AS POSONSAN_EDP,0 AS PREOFSAN_EDP,0 AS POSOFSAN_EDP, "      );
qrysb.append("   SUM( CASE WHEN ed.EDP_TYPE = 1 AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PREON_EDP, SUM( CASE WHEN ed.EDP_TYPE = 2 AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS POSTON_EDP,"      );
qrysb.append("   SUM( CASE WHEN ed.EDP_TYPE IN (1, 2) AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS TOTON_EDP, SUM( CASE WHEN ed.EDP_TYPE = 3 AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE = 4 AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS POSTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3, 4) AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS TOTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1, 2, 3, 4) AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS TOT_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1) AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS COMPREON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (2) AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS COMPOSTON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3) AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS COMPREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (4) AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS COMPOSTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1) AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PENPREON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (2) AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PENPOSTON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3) AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PENPREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (4) AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ed.ACT_YN = 'Y' THEN 1 ELSE 0 END) AS PENPOSTOF_EDP FROM app_detail_online ado, edp_conduct_trans ed,"      );
qrysb.append("   m_district DM, m_state ms, mas_off_mast mom, mas_off_mast mo, ZONE_MAST ZM WHERE ado.APP_ID = ed.APP_ID "      );
qrysb.append("   AND ado.UNIT_DIST_CD = DM.DISTRICT_CD(+) AND DM.STATE_CD = ms.STATE_CD(+)"      );
qrysb.append("   AND ado.APP_ID NOT IN (SELECT APP_ID FROM APP_DETAIL WHERE ACT_ID > =0 AND APP_ID=ado.APP_ID) and ado.act_id=-1   AND ed.EDPOFF_CD     = mom.OFF_CD(+)"      );
qrysb.append("   AND ado.OFF_CD = mo.OFF_CD(+) AND ms.ZONE_CD = ZM.ZONE_CD "+txtSearch+" group by mom.OFF_NAME,(DM.district_name) "      );

qrysb.append("   UNION ALL "      );
qrysb.append("   SELECT DM.district_name,mo.OFF_NAME,"      );
qrysb.append("   SUM( CASE WHEN TRUNC(ado.BANK_F_DATE) BETWEEN '"+FromDt+"' AND '"+ToDt+"' AND ado.ACT_ID = 5 THEN 1 ELSE 0 END) AS BANK_FORW, "      );
qrysb.append("   SUM( CASE WHEN ed.EDP_TYPE = 1 AND bd.act_id=11 and ado.act_id=5 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PREONSAN_EDP, SUM( CASE WHEN ed.EDP_TYPE =2 AND bd.act_id=11 and ado.act_id=5 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS POSONSAN_EDP, SUM( CASE WHEN ed.EDP_TYPE =3 AND bd.act_id=11 and ado.act_id=5 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PREOFSAN_EDP, SUM( CASE WHEN ed.EDP_TYPE =4 AND bd.act_id=11 and ado.act_id=5 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS POSOFSAN_EDP, SUM( CASE WHEN ed.EDP_TYPE = 1 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PREON_EDP, SUM( CASE WHEN ed.EDP_TYPE = 2 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS POSTON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1, 2) AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS TOTON_EDP, SUM( CASE WHEN ed.EDP_TYPE = 3 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE = 4 AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS POSTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3,4) AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS TOTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1, 2, 3, 4) AND ed.ACT_YN = 'Y' AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS TOT_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS COMPREON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (2) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS COMPOSTON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS COMPREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (4) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NOT NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS COMPOSTOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (1) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PENPREON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (2) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PENPOSTON_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (3) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PENPREOF_EDP, SUM( CASE WHEN ed.EDP_TYPE IN (4) AND ed.ACT_YN = 'Y' AND ed.EDP_CERT_DT IS NULL AND TRUNC(ed.EDPTIMESTAMP) BETWEEN '"+FromDt+"' AND '"+ToDt+"' THEN 1 ELSE 0 END) AS PENPOSTOF_EDP FROM APP_DETAIL ado, BANK_DATAENTRY bd, edp_conduct_trans ed, m_district DM, m_state ms, mas_off_mast mom,MAS_OFF_MAST MO, ZONE_MAST ZM WHERE ado.APP_ID = bd.APP_ID(+) AND ado.APP_ID = ed.APP_ID (+) AND ado.UNIT_DIST_CD = DM.DISTRICT_CD(+) AND DM.STATE_CD = ms.STATE_CD(+)   AND ed.EDPOFF_CD     = mo.OFF_CD(+) AND ado.OFF_CD = mom.OFF_CD(+) AND ms.ZONE_CD = ZM.ZONE_CD"      );
qrysb.append("  "+txtSearch+"    "  );
qrysb.append("    group by mo.OFF_NAME,(DM.district_name) )"      );
qrysb.append("   A  where A.OFF_NAME is not null group by A.district_name, ROLLUP(A.OFF_NAME)"      );
qrysb.append("   "      );
...
<tbody>
 <tr>
 <th  colspan="25"><div align="center"></div>   <%=firstcolold%></span></th>
 </tr>
	 <%}%>
...
<a href="<%=urlrepE+vnmdw%>">DOWNLOAD</a>
<%}%>
</th>
