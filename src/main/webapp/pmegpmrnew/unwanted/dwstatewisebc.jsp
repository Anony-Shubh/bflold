...
// UNDER PROCESS AT AGENCY
  ...
  txtSearch = txtSearch + " and NVL(AD.MTG_ID,0)>0 ";
 ...
 // Rejected by Bank
  txtSearch = txtSearch + " and  bd.act_id=3  ";
 }else if (vQryCode.equals("8")){
 // Own Contribution Deposited
  txtSearch = txtSearch + " and   bd.OWN_CONT_DT between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
 ...
 // EDP TRANING COMPLETED
  txtSearch = txtSearch + " and bd.EDP_CERT_DT between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
 ...
 // Pending for Bank Sanction
  txtSearch = txtSearch + " and ad.bank_f_date between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=8  ";
 }else if (vQryCode.equals("16")){
 // Pending for MM Release
  txtSearch = txtSearch + " and bd.mm_claim_dt between '"+FromDt+"' and '"+ToDt+"' and bd.mm_rel_dt is null and bd.act_id=11  ";
...
sb.append(" SELECT ad.app_id,ad.app_name,UPPER(ad.fh_name) AS FH_NAME,mom.agency_type, mom.off_name, NVL(ad.remarks,'') AS Remraks_up_rej ,").
append(" ms.state_nm,ad.MOB_NO2 AS TEL_NO,ad.E_MAIL,ad.AADHAR_NO,TO_CHAR(AD.ONLINE_SUBDT,'DD-MON-RRRR') ONLINESUBDT, ").
append(" ad.app_post_addr||' , '||ad.app_taluk_block||' '|| ' Dist:'||bend.district_name||', ").
append(" Pin:'||ad.app_pin as app_addr,ad.MOB_NO1 AS ONLINEMOBNO , ").
...
append(" (bd.workshed_cost+bd.machinary_cost+ bd.pre_ope_cost) AS ").
append(" CE,bd.working_capital,(bd.workshed_cost+bd.machinary_cost+bd.pre_ope_cost+bd.working_capital) ").
append(" As total_sanc, bd.own_contribution, 0  AS bank_loan, bd.MM_CLAIM_AMT,edpom.off_name as ").
append(" edp_trg_nm, to_char(bd.edp_fr_dt,'dd-mon-RRRR') as edp_fr_dt, to_char(bd.edp_to_dt,'dd-mon-RRRR') ").
append(" as edp_to_dt, to_char(bd.EDP_CERT_DT,'dd-mon-RRRR') as ").
append(" EDP_CERT_DT,'' as Nodal_bank, ").
...
append(" as mm_claim_dt, bd.mm_claim_amt, bd.mm_rel_amt,to_char(bd.MM_REL_dt,'dd-mon-RRRR') AS mm_rel_dt, bd.emp, ").
...
append(" mas_off_mast edpom, ").
append(" zone_mast zm where ms.zone_cd=zm.zone_cd and  ").
...
append(" AND AD.IFSC_CODE LIKE UPPER('"+vBankName+"') ").
...
%>
  ...
  <tr>
    <th colspan="52" align="left" nowrap="nowrap"><div align="left">PMEGP Detail Report for the period from <%= FromDt %> to <%= ToDt %>of Agency :<%= vAgency %></div></th>
    </tr>
  <tr>
   <th rowspan="2" align="left">     srno</th>
    ...
    <th rowspan="2" align="left">Date of Application Received at Bank </th>
    <th rowspan="2" align="left">Sanction Date </th>
    <th colspan="3" align="left">Bank Sanction Detail </th>
