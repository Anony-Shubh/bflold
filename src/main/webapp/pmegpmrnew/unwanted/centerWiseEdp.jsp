...
String   PENPOSTOF_EDP="";  
 ...
 txtSearch=txtSearch+" AND MOM.AGENCY_TYPE LIKE '"+vAgency+"' AND ZM.ZONE_NM LIKE '"+vZonecd+"' AND MS.STATE_NM LIKE '"+vStatecd+"' AND DM.DISTRICT_NAME LIKE '"+vDist+"' AND MOM.OFF_NAME1 LIKE '"+vOffice+"' AND MO.OFF_TYPE_CD="+OFF_TYPE_CD+"  "; 
...
*/
qrysb.append("   SELECT nvl(A.OFF_NAME,'Total')  as off_name1 ,max(A.off_cd ) as off_cd,max(EDP_STATE) as EDP_STATE,SUM(A.COMPTOT_EDP) AS COMPTOT_EDP,"      );
qrysb.append("   sum(A.BANK_FORW) AS BANK_FORW, sum(A.PREONSAN_EDP) AS PREONSAN_EDP, SUM(A.PENPREON_EDP+A.PENPREOF_EDP) AS TOT_PEN_EDP, "      );
...
qrysb.append("   (SELECT DM.district_name AS EDP_STATE,mo.OFF_NAME,mo.off_cd,SM.STATE_NM ,"      );
qrysb.append("       SUM("      );
qrysb.append("       CASE"      );
qrysb.append("         WHEN ado.ACT_ID = 5"      );
qrysb.append("        "      );
...
qrysb.append("  FROM APP_DETAIL ado, BANK_DATAENTRY bd, edp_conduct_trans ed, m_district DM, m_state ms, mas_off_mast mom,MAS_OFF_MAST MO, ZONE_MAST ZM ,M_DISTRICT UD,M_STATE SM");
 ...
 qrysb.append("  AND ed.EDPOFF_CD = mo.OFF_CD(+) AND ado.OFF_CD = mom.OFF_CD(+) AND ms.ZONE_CD = ZM.ZONE_CD AND MO.DISTRICT_CD=UD.DISTRICT_CD AND UD.STATE_CD=SM.STATE_CD "      );
qrysb.append("  "+txtSearch+"    "  );
qrysb.append("    group by mo.OFF_NAME,mo.off_cd,SM.STATE_NM,(DM.district_name) )"      );
qrysb.append("   A WHERE ( A.PENPREON_EDP > 0 OR A.PENPREOF_EDP >0 OR A.COMPTOT_EDP >0) group by ROLLUP(A.OFF_NAME)"      );
...
<tr>
  ...
  <th rowspan="3"  > <p>Pre EDP </p>
   <p>(Before Sanction) </p></th>
...
<td >
<% if (!vnmdw.equals("Total")){%>
<a href="<%=urlrepE+off_cd+"&DISTCD="+vDist%>&QRYCODE=1&REPTP=5">	<%=PENPREON_EDP  %></a>
<%} else{%>
<%=PENPREON_EDP  %>
<%}%></td>
<td ><% if (!vnmdw.equals("Total")){%>
<a href="<%=urlrepE+off_cd+"&DISTCD="+vDist%>&QRYCODE=2&REPTP=5">	<%=PENPREOF_EDP  %></a>
<%} else{%>
<%=PENPREOF_EDP  %>
<%}%></td>
<td ><%=TOT_PEN_EDP%></td>
<th nowrap="nowrap">
<% if ((!vnmdw.equals("Total")) && (!COMPTOT_EDP.equals("0"))){%>
<a href="<%=urlrepE+off_cd+"&DISTCD="+vDist%>">DOWNLOAD</a>
<%}%></th>
