...
txtSearch=" and upper(A.zone_nm) like upper('"+vZoneCD+"') and upper(A.state_nm) like upper('"+vStateCd+"') and A.AGENCY_TYPE like '"+vAgency+"' and upper(A.UNIT_DIST) like upper('"+vDist+"') ";
 
 if (vQryCode.equals("1")){

 txtSearch = txtSearch ;
} else if (vQryCode.equals("2")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND UPPER(VERIFICATION_STATUS)='WORKING' " ;

}
else if (vQryCode.equals("3")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND UPPER(VERIFICATION_STATUS)='DEFUNCT' " ;

}

else if (vQryCode.equals("4")){
 txtSearch = txtSearch + "  and gt.pmegpgeotagid IS NOT NULL AND UPPER(VERIFICATION_STATUS)='NON-TRACEABLE' " ;

}

else if (vQryCode.equals("5")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL  " ;

}

else if (vQryCode.equals("6")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS not NULL   and qc_apprv='Y' " ;

}

else if (vQryCode.equals("7")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS  NULL   and qc_apprv='Y'  " ;

}
else if (vQryCode.equals("8")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS NOT  NULL AND GT.ACT_ID=1  and qc_apprv='Y'  " ;

}

else if (vQryCode.equals("9")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND MM_ADJUSTMENT_STATUS IS NOT  NULL AND NVL(GT.ACT_ID,0) IN (0,2) and qc_apprv='Y' " ;

}
else if (vQryCode.equals("10")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND GT.MM_ADJUSTMENT_STATUS IS NOT NULL    " ;

}

else if (vQryCode.equals("11")){
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND GT.MM_ADJUSTMENT_STATUS IS NOT NULL AND GT.AGENCY_UPLOAD IS NOT NULL " ;
 ...
 txtSearch = txtSearch + " and gt.pmegpgeotagid IS NOT NULL AND GT.ACT_ID=1 AND  GT.MM_ADJUSTMENT_STATUS='RU' " ;
