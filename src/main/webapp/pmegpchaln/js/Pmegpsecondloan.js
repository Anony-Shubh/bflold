...
function validate(form) {
   ...
   var DISTRICT = document.form.DISTRICT.value; 
	...
	var  WORKING_CAPITAL = document.form.WORKING_CAPITAL.value;
	...
	var BENF_SPECAT_CD = document.form.BENF_SPECAT_CD.value;	
     var UNIT_ESTB_YR= document.form.UNIT_ESTB_YR.value;	
 ...
 if(BENF_CATAGORY_CD == ""){
	   inlineMsg('BENF_CATAGORY_CD','Please Select Social Category',2);
    return false;
  }
  
  if(BENF_SPECAT_CD == "") {
    inlineMsg('BENF_SPECAT_CD','Please Select Special Category',2);
    return false;
  }
...
if(APP_TALUK_BLOCK == "") {
  ...
  if(!numberOnly.test(MOB_NO1))
	...
	if(MOB_NO1.length !=10)
    {
	  ...
	  if(UNIT_ESTB_YR=="") {
    inlineMsg('UNIT_ESTB_YR','Select Unit Establishment Date',2);
    return false;
  }
  
   if (UNIT_ESTB_YR != "") {
	if  (!dtRegex.test(UNIT_ESTB_YR)){
	inlineMsg('UNIT_ESTB_YR',"WRONG DATE OR DATE FORMAT SHOULD BE (DD-MON-YYYY)",2);
    return false;
...
if ((STATE == 'AR') || (STATE ==  'AS')||(STATE == 'MN')||(STATE ==  'MG') ||(STATE == 'SK')||(STATE == 'TR')||(STATE == 'NG')|| (STATE == 'MZ'))
	{
		var EMP_NS= (MACHINARY_COST%150000===0)?Math.round(MACHINARY_COST/150000):Math.round(( Number(MACHINARY_COST)+(150000-  (Number(MACHINARY_COST)   %Number  (150000))    ) )/150000) ;
		}
	...
	{
	var EMP_NS= (MACHINARY_COST%100000===0)?Math.round(MACHINARY_COST/100000):Math.round(( Number(MACHINARY_COST)+(100000-  (Number(MACHINARY_COST)   %Number  (100000))    ) )/100000);
	}
...
// qualification validation
	// Under 8th 
 if (it=='M'  && Number(tprj)>10000000){
	inlineMsg('TOTAL_PRJ','Total Project Cost  for <Strong>Manufacturing Industry </Strong> should not exceed 1 cr',2);
	return (false);
}

 if (it=='S' && Number(tprj)>2500000){
	inlineMsg('TOTAL_PRJ','Total Project Cost for for <Strong>Service/Trading Industry</Strong> should not exceed 25 lakhs',2);
	return (false);
}
if (it=='T' && Number(tprj)>2500000){
	inlineMsg('TOTAL_PRJ','Total Project Cost for for <Strong>Service/Trading Industry</Strong> should not exceed 25 lakhs',2);
	return (false);
}
