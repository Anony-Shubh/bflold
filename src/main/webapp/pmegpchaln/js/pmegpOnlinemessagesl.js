...
function validate(form) {
  ...
  //var BANK_TYPE = document.form.BANK_TYPE.value;
	...
	var EDP_INST_NM =document.form.EDP_INST_NM.value;                              
	var BENF_CATAGORY_CD =document.form.BENF_CATAGORY_CD.value;  
...
if(ACTIVITY_CD == "" ) {
  ...
  if(EDP_YN=="Y") {
	  ...
	  if(EDP_INST_NM=="") {
		  inlineMsg('EDP_INST_NM','Enter EDP Training Institution Name',2);
    return false;
	  }
  }//edp trg main
...
return true;
 ...
 if(IFSC_CODE == "" ) {
    inlineMsg('IFSC_CODE',' Enter IFSC code, if IFSC code not known then select from list. ',2);
    return false;
  }
