...
<br>
<br>
<div align="center"><a target="_blank" href="PVPDF/Indian_Bank_NEFT_Challan12.doc" class="greenButtonRound">New Challan Format </a></div>

<div align="center"><a target="_blank" href="PVPDF/4479_Download_EFTnet for Merchant V1.2.pdf" class="greenButtonRound">User Manual </a></div>
<div align="center"><a target="_blank" href="PVPDF/Indian_Bank_NEFT_Challan12.pdf" class="greenButtonRound">New Challan format in PDF </a></div>
...
String IFSC_CODE="IDIB000N149";//"JAKA0BBHARA";//(String)session.getAttribute("bank_id");
...
qrysb.append("  SELECT p.*,(select  count(*) from indbank_request where substr(txnid,3,length(txnid))=substr(p.app_id,instr(p.app_id,'-')+1,length(p.app_id)) and trunc(SYSTIMESTAMP) BETWEEN trunc(rtimestamp_req) AND trunc(rtimestamp_req)+30) as verifyapp_cnt FROM ( SELECT GT.PMEGPGEOTAGID,"      );
...
qrysb.append("   AND UPPER(AD.APP_ID"      );
qrysb.append("     || ' '"      );
qrysb.append("     || TRIM(AD.APP_NAME)) LIKE TRIM(UPPER(?)) AND  RBL.IFSC_CODE=?  )p  ORDER BY DATE_OF_VERIFICATION DESC"      );
...
//out.print(AGENCY_UPLOAD);
    ...
    %>
     <INPUT name="APP_ID" type="hidden" id="BANKCODE" value="<%=APP_ID %>">
	  ...
	  <DIV align="center"><A target="_blank" href="payment_gateway_t.jsp?APP_ID=<%= APP_ID %>&IFSC_CODE=<%= IFSC_CODE %>" class="greenButtonRound">Generate</A></DIV>	
	  <%} else{%>
	  <DIV align="center"><A target="_blank" href="payment_verify_status.jsp?APP_ID=<%= APP_ID %>" class="greenButtonRound">Verify Payment Status</A></DIV>	
	  <%}%>
