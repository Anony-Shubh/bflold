...
<%@ page  session="true" buffer="500kb" import="java.util.*" %>
...
<tr>
  <td>5): Then Save Challan as pdf or print</td>
    ...
    <%
   String APP_ID = request.getParameter("APP_ID")==null?"DIJK18196317-8206420":(String) request.getParameter("APP_ID").trim();

	//String APP_ID =session.getAttribute("SAPPID")==null?"": (String)session.getAttribute("SAPPID");
    String IFSC_CODE=request.getParameter("IFSC_CODE")==null?"JAKA0BBHARA":(String) request.getParameter("IFSC_CODE").trim();
...
%>
    <CENTER>
    <input name="APP_ID" type="HIDDEN" id="APP_ID" value="<%= APP_ID %>" size="50" maxlength="30">
    <BR>
...
qrysb.append("   SELECT p.*,"      );
qrysb.append("     (SELECT lpad(NVL(COUNT(*), 0) + 1, 2, 0)"      );
qrysb.append("     FROM indbank_request"      );
qrysb.append("     WHERE SUBSTR(indbank_request.TXNID, 3, LENGTH(indbank_request.TXNID)) = p.appid_str"      );
qrysb.append("     )"      );
qrysb.append("     || p.appid_str txnid"      );
qrysb.append("   FROM"      );
qrysb.append("     (SELECT bd.APP_ID,"      );
qrysb.append("       SUBSTR(bd.APP_ID, instr(bd.APP_ID, '-') + 1, LENGTH(bd.APP_ID)) appid_str,"      );
qrysb.append("       gt.AGN_CALLBACK mm_rel_amt,"      );
qrysb.append("       ap.APP_NAME,"      );
qrysb.append("       bd.IFSC_CODE,"      );
qrysb.append("       rb.BANK_NAME,"      );
qrysb.append("     rb.BRANCHNAME  BRANCHNAME,"      );
qrysb.append("       bbm.EMAIL AS BRANCHEMAIL"      );
qrysb.append("     FROM bank_dataentry bd,"      );
qrysb.append("       app_detail ap,"      );
qrysb.append("       rbibanklist rb,"      );
qrysb.append("       geotag_trans gt,"      );
qrysb.append("       bank_br_mast bbm"      );
qrysb.append("     WHERE gt.APP_ID             = ap.APP_ID"      );
qrysb.append("     AND bd.APP_ID               = ap.APP_ID"      );
qrysb.append("     AND rb.IFSC_CODE            = bd.IFSC_CODE"      );
qrysb.append("     AND rb.IFSC_CODE            = bbm.BR_IFSL_CODE"      );
qrysb.append("     AND (bd.APP_ID              = ?"      );
qrysb.append("     AND bd.IFSC_CODE            = ?"      );
qrysb.append("     AND bd.PACT_ID              = 34"     );
qrysb.append("     AND gt.ACT_ID               = 1"      );
qrysb.append("     AND NVL(gt.AGN_CALLBACK, 0) > 0"      );
qrysb.append("     AND gt.AGENCY_UPLOAD       IS NOT NULL)"      );
qrysb.append("     UNION"      );
qrysb.append("     SELECT bd.APP_ID,"      );
qrysb.append("       SUBSTR(bd.APP_ID, instr(bd.APP_ID, '-') + 1, LENGTH(bd.APP_ID)) appid_str,"      );
qrysb.append("       gt.AGN_CALLBACK mm_rel_amt,"      );
...
qrysb.append("       app_detail_SECOND ap,"      );
qrysb.append("       rbibanklist rb,"      );
qrysb.append("       geotag_trans gt,"      );
qrysb.append("       bank_br_mast bbm"      );
qrysb.append("     WHERE gt.APP_ID             = ap.APP_ID"      );
...
//ResultSet Mainrs=db.executeSQL("   ) p ",values);
...
qryins.append("  length(bd.app_id)) appid_str, gt.agn_callback as mm_rel_amt ,ap.app_name,bd.ifsc_code,    "      );
qryins.append("  rb.bank_name,rb.branchname from bank_dataentry bd, app_detail ap,rbibanklist rb,geotag_trans gt     "      );
qryins.append("  where pact_id=34  and bd.app_id=ap.app_id  and gt.app_id=ap.app_id    "      );
qryins.append("  and rb.ifsc_code=bd.ifsc_code and bd.app_id=? and gt.act_id=1 and nvl(agn_callback,0)>0  and  gt.AGENCY_UPLOAD IS NOT NULL ");
qryins.append("  UNION  ");
qryins.append("    select bd.app_id,  substr(bd.app_id,instr(bd.app_id,'-')+1,  ");  
qryins.append("   length(bd.app_id)) appid_str, gt.agn_callback as mm_rel_amt ,ap.app_name,bd.ifsc_code, ");   
qryins.append("   rb.bank_name,rb.branchname from bank_dataentry_SECOND bd, app_detail_SECOND ap,rbibanklist rb,geotag_trans gt  ");   
qryins.append("   where pact_id=34  and bd.app_id=ap.app_id  and gt.app_id=ap.app_id    ");
qryins.append("   and rb.ifsc_code=bd.ifsc_code and bd.app_id=? and gt.act_id=1 and nvl(agn_callback,0)>0  and  ");
qryins.append(" gt.AGENCY_UPLOAD IS NOT NULL  ");
...
//out.print (hash);
response.sendRedirect("payment_gateway_conf_t.jsp?APP_ID="+APP_ID+"&IFSC_CODE="+IFSC_CODE+"&hash="+hash);
}
 ...
 %>
  ...
  <tr>
    ...
    <th>Applicant's  Name :
	  ...
	  <td ><input name="firstname" id="firstname" type="hidden" value="<%= APP_NAME %>"  size="50" readonly="true" /><%= APP_NAME %></td>
  </tr>
  <tr>
  ...
  <tr>
    <th colspan="4" ><div align="center">
          <input type="radio" id="RADIO" name="RADIO" value="C" checked="checked"  /> 
      Margin Money Callback    
      <!--<input type="radio" id="RADIO" name="RADIO" value="R"  onclick="checkradio('R');"  onchange="showqry();">  
      Margin Money Refund -->
      <input name="productinfo" id="productinfo" type="HIDDEN" value="Margin Money Callback" />
    </div></th>
    </tr>
