...
<tr>
 ...
 //String APP_ID = request.getParameter("APP_ID")==null?"KBHY18196222-8509080":(String) request.getParameter("APP_ID").trim();
...
qrysb.append("       bd.IFSC_CODE,bd.MM_REL_AMT,"      );
...
qrysb.append("     WHERE      " );
qrysb.append("      bd.APP_ID               = ap.APP_ID"      );
...
String  hash= sh.sha512 (hash1);
 ...
 try{
qryins.append("  insert into indbank_request (FIELD5,rtimestamp_req,  ");  
...
qryins.append("  UDF5,hash) ");    
 qryins.append(" select 'By KVIC',sysdate,?,?,app_name,'BHUZlo','',?,?, (select lpad(nvl(count(*),0)+1,2,0) from indbank_request ");     
...
qryins.append("  length(bd.app_id)) appid_str,ap.app_name,bd.ifsc_code,    "); 
...
db.close();
 %>
  ...
  <tr>
    ...
    <td ><input type="text" name="amount" id="amount" value="<%= MM_REL_AMT %>"   size="50"  /></td>
  ...
  <tr>
    <th>Generator Mobile No. *:</th>
    <td ><input type="text" id="phone" name="phone" value=""   size="50" maxlength="10" /></td>
	  <th>Branch Manager Email Id *: </th>
	  <td ><input type="text" name="email" id="email" value="<%=BRANCHEMAIL%>"  readonly="true" size="50" maxlength="50" /></td>
  </tr>
