...
<table align="center" class="pmegpTable">
    <tr>
      <td><div align="center"> <h4>
	  <% if (!vOFF_TYPE_CD.equals("17")){%>
	  <a href="edpRepGenerate.jsp?REPNM=EDPTRAINING&GRV_TYPE=0" class="buttonMerun" >Download Applicant with contact details</a>
	  <% } else{%>
	   <a href="edpRepGenerate.jsp?REPNM=EDPTRAINING&GRV_TYPE=-1" class="buttonMerun" >Download Applicant with contact details</a>
	  <%}%>
...
qrysb.append("   SELECT TO_CHAR(AD.ONLINE_SUBDT, 'DD-MON-RRRR') AS ONLINE_SUBDT,"      );
qrysb.append("     ED.DOC_NAME,"      );
...
qrysb.append("       AND NVL(BD.ACT_ID, 0) = 0"      );
qrysb.append("       THEN ASM.ACT_DESC"      );
qrysb.append("       WHEN BD.ACT_ID > 0"      );
qrysb.append("       THEN BAS.ACT_DESC"      );
qrysb.append("     END ACT_DESC,"      );
qrysb.append("     AD.APP_ID,"      );
qrysb.append("     NVL(AD.MACHINARY_COST, 0) + NVL(AD.WORKING_CAPITAL, 0) AS PROJECT_COST,"      );
qrysb.append("     CASE"      );
qrysb.append("       WHEN AD.IND_TYPE = 'M'"      );
...
qrysb.append("       AND ED.EDP_TYPE  = 4)"      );
qrysb.append("       OR (ED.EDP_TYPE  = 3))"      );
qrysb.append("       AND ED.DOC_NAME IS NULL"      );
qrysb.append("       THEN 'Y'"      );
qrysb.append("       ELSE 'N'"      );
qrysb.append("     END AS UPLOAD_YN,"      );
qrysb.append("     ED.ED_ID"      );
...
EDP_TYPE=rsMain.getString("EDP_TYPE")==null?"":rsMain.getString("EDP_TYPE");
  ...
  %>
	...
	<%}else if  (!DOC_NAME.equals("")){%>
	 <input type="button" name="button3" id="button3" class="button" value="View EDP Certificate" onClick="viewdocument('<%= DOC_NAME %>','<%= APP_ID%>');">
...
var callbackFunctionArray = new Array(refreshpage);
 modalWin.ShowURL("../jsp/doctransfertoNas.jsp?docname="+vDOC_NAME+"&APP_ID="+vAPPID,500,800,'View Document',null,null);
 }
