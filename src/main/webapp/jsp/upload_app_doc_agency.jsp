...
qryUp.append(" (SELECT * FROM TABLE(FUPLOAD_DOC(?))) ");
	...
	values.clear();	
	  ...
	  <table align="center" class="pmegpTable">
        ...
        <tr>
          <th colspan="5" nowrap class="redLebel"><div align="center">Note: After Forward to Bank No document Can be Deleted but Revised Document/EDP Certificate/ Addl. Document Can be uploaded </div></th>
        </tr>
        <tr>
          <th nowrap>SrNo</th>
  ...
  %>
        <tr>
		 ...
		 <% if (!UP_ID.equals("0") && (!ACT_ID.equals("5"))) {%>
		  
		  <input name="btnUploadDel" type="button" class="button"  onClick="ShowNewPageDel('<%= UP_ID %>','<%= APP_ID %>');" value="Delete">
		  <% } else if (ACT_ID.equals("5") && (!UP_ID.equals("0")) ){%>
		   <input name="btnUpload" type="button" class="button"  onClick="ShowNewPage('<%= UPM_CD %>','<%= APP_ID %>');" value="UPLOAD (Revised Doc./ EDP /Addl. Doc) ">
		  <%}%> 	  	  
		</td>
        </tr>
        <%
...
var urlpar='AppUplodadByAgency.jsp?UPM_CD='+UPM_CD+'&APP_ID='+APP_ID;
...
var urlpar='upload_app_doc_delbyAgency.jsp?UP_ID='+UP_ID+'&APP_ID='+APP_ID;
 var callbackFunctionArray = new Array(EnrollNow, EnrollLater);
 modalWin.ShowURL(urlpar,600,1000,'Upload Document',null,callbackFunctionArray);
 }//E
