...
<INPUT name="TOTAL_SCORE" type="hidden" id="TOTAL_SCORE" value="<%= TOTAL_SCORE %>">
  ...
  <TABLE align="center">
    ...
    <TR >
      ...
      Under Process
		...
		<%if(SCOREVALID.equals("Y")){%>
		 <INPUT name="ACT_ID"  id="ACT_ID" type="radio" value="4" <% if (ACT_ID.equals("4")){out.print("Checked");}%>> 
		  <SPAN class="GreenLebel">Score Card Verified and to be forwarded to Bank</SPAN>
		 <%}else {%>
		  <INPUT name="ACT_ID"  id="ACT_ID" type="radio" disabled> 
    ...
    <TR >
      ...
      <TD colspan="3" ><INPUT name="REMARKS" type="text" id="REMARKS" onClick="Status_change();" value="<%= REMARKS %>" size="50" maxlength="100">      </TD>
    </TR>
    <TR>
...
//qryUpdate.append("  ACT_ID=?,FORD_DT_DLTFC=? ,MTG_ID=? WHERE APP_ID=? AND ACT_ID IN (0,1,3,2,4) AND MTG_ID IS NULL AND DLTFC_PROC_DT IS NULL");

qryUpdate.append(" UPDATE APP_DETAIL SET REMARKS=?,APP_REC_DATE=SYSDATE,");
qryUpdate.append("  ACT_ID=?,FORD_DT_DLTFC=? ,MTG_ID=? WHERE APP_ID=? AND  ACT_ID NOT IN (7,5) ");
