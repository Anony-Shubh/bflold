...
<form  action="AgnedpDetailENBN.jsp" method="post" name="form" id="form" >
...
qrysb.append(" SELECT AD.APP_ID,AD.APP_NAME,AD.APP_DIST_CD,ED.ED_ID,CASE WHEN ED.EDP_TYPE IN (1,2) THEN 'ON' WHEN ED.EDP_TYPE IN (3,4) THEN 'OF' END AS EDP,  "      );
qrysb.append("   ED.EDP_TYPE,CASE WHEN ED.EDP_TYPE IN (1,2) THEN '' ELSE  ED.EDP_TRG_NAME END AS  EDP_TRG_NAME"      );
qrysb.append("   ,ED.EDPOFF_CD FROM app_detail_online AD,  EDP_CONDUCT_TRANS ED WHERE AD.APP_ID = ED.APP_ID(+)  AND AD.APP_ID='"+APP_ID+"' ");
...
<tr>
  <th colspan="2"><div align="center">Applicant EDP Detail </div></th>
    ...
    <th> Applicant ID:  
		...
		<input name="h" type="hidden" id="h" value="<%=EDP%>">
		</th>
  <td><%= APP_ID %></td>
</tr>  
  <tr>
 ...
 <td> 
   <input name="EDP" type="radio" id="ONP"  value="ON" <% if (EDP.equals("ON")) {out.print("CHECKED");} %>   />
   Online  EDP    </td>
</tr>
  <tr id="EDP_TY6">
...
values.clear();
pstm.clear();
String iqry=" UPDATE EDP_CONDUCT_TRANS SET ACT_YN='N' WHERE  ED_ID="+ED_ID+"  ";
db.setSqlValue( iqry);
db.setValues(values,pstm);
db.executeUpdate();
db.close();
