...
String fileRedirectname="appUpdateRejectedbyBank.jsp?APP_ID="+APP_ID;
String AADHAR_NO ="";
String PAN_NO="";
String DOB="";
String PROJ_COST="0";
		   ...
		   qry.append("  TO_CHAR(A.ONLINE_SUBDT, 'DD-MON-RRRR HH:MI:SS') AS ONLINE_SUB_DT, TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE),TRUNC(A.DOB))/12) AS AGE,");
		    qry.append("  A.MOB_NO1,upper(A.PAN_NO) as PAN_NO,MS.STATE_CD, MS.STATE_NM, UPPER(AM.ACTIVITY_NAME) AS ACTIVITY_NAME, ");
			 ...
			 qry.append(" CASE WHEN MS.STATE_CD IN ('AR','AS','MN','MG','SK','TR','NG','MZ','JK') THEN 'Y' ELSE 'N' END NER_YN ");
			  qry.append(" FROM APP_DETAIL A , M_STATE MS, M_DISTRICT MD, ACTIVITY_MAST AM ");
...
aerror.add("Applicant Name Should not be Blank and Name Should be As per Identity Proof..");
  ...
  StringBuffer qrydup = new StringBuffer();
   ...
   try {
  if (adharvalid) {
 qrydup.append(" SELECT APP_ID,TO_CHAR(AADHAR_NO) AS PER_IDENTITY FROM APP_DETAIL_ONLINE WHERE AADHAR_NO="+AADHAR_NO+" ");
...
qrydup.append(" SELECT APP_ID,PAN_NO FROM APP_DETAIL WHERE TRIM(UPPER(PAN_NO))=TRIM(UPPER('"+PAN_NO+"')) ");
 ...
 if (DOB.equals("")) {
  ...
  if (Integer.parseInt(PROJ_COST)<1000) {
   aerror.add("Total Projest Cost Mentioned is Too Low ( should be in Rs.):"+PROJ_COST);
...
<table align="center" class="pmegpTable">
  ...
  <tr>
    <td colspan="4" class="warning" scope="row"><h2 class="error"> :Incomplete Application:</h2></td>
  </tr>
...
response.sendRedirect(fileRedirectname);
 }
 }
 rsApp.close();
 db.close(); 
