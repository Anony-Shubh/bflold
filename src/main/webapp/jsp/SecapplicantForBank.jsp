...
String VALID_DATA="Y";
 ...
 ArrayList aerror = new ArrayList();
        ...
        db.connect();
         ...
         qry.append(" SELECT A.APP_ID,MD.STATE_CD,A.BENF_SPECAT_CD, ");
		   ...
		   qry.append("  TO_CHAR(A.ONLINE_SUBDT, 'DD-MON-RRRR HH:MI:SS') AS ONLINE_SUB_DT,");
		    qry.append("  A.MOB_NO1,upper(A.PAN_NO) as PAN_NO,MS.STATE_CD, MS.STATE_NM, UPPER(AM.ACTIVITY_NAME) AS ACTIVITY_NAME, ");
			 ...
			 qry.append(" CASE WHEN MS.STATE_CD IN ('AR','AS','MN','MG','SK','TR','NG','MZ','JK') THEN 'Y' ELSE 'N' END NER_YN ");
			  qry.append(" FROM APP_DETAIL_SECOND A , M_STATE MS, M_DISTRICT MD, ACTIVITY_MAST AM ");
...
if (BENF_SPECAT_CD.equals("NA")){
	 aerror.add("Please select  proper Special Cateogry...Your State belong to Either North East Or Hill States ");  
} 
  ...
  // END OF NER
	...
	if (BENF_SPECAT_CD.equals("NE") || BENF_SPECAT_CD.equals("HS")){
	 aerror.add("Please select  proper Special Cateogry............State is not declared as Hill states or North East States");  
}   
...
qryupdt.append("    SELECT AUM.UPM_CD,");
 qryupdt.append(" AUM.DOC_DESC FROM APP_UPLOAD_MAST_second AUM WHERE AUM.USR_TYPE='I' AND AUM.UPM_CD<>8  ");
...
<%			
 ...
 response.sendRedirect("secStatusBankForward.jsp?APP_ID="+APP_ID);
 }
