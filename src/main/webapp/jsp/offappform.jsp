...
String Agency=""; 
 DBCon db= new DBCon();
db.connect();
String statecd=(String) session.getAttribute("state_cd");
...
String APP_NAME = "";
String FH_NAME = "";
String sDISTRICTNAME=(String) session.getAttribute("sDISTRICTNAME");
...
ResultSet rsBenfType = db.execSQL("SELECT A.AADHAR_NO,A.APP_NAME,A.GENDER,to_char(A.DOB,'DD-MM-RRRR') as DOB,A.MOB_NO,A.OFF_CD,A.DIST_CD,MOM.ORG_CD  FROM AADHAR_VERI_OFFLINE A,MAS_OFF_MAST MOM  WHERE A.OFF_CD=MOM.OFF_CD AND OFF_ID= "+SOFFID+"");
...
ResultSet rsDist = db.execSQL("SELECT DISTRICT_CD,DISTRICT_NAME FROM M_DISTRICT WHERE STATE_CD='"+statecd+"'  ");
	%>

</HEAD>

<BODY onLoad="showOffice(this.value);copy_dist();">
...
<FORM name="form"  id="form" method="post"  >
  ...
  <TR>
    ...
    <TD colspan="2"><SELECT name="STATE" disabled="disabled" class="button"  id="STATE" onBlur="showOffice(this.value);" onChange="showDistrict(this.value);">
      ...
      <%
qrystr="SELECT  ms.state_cd, initCap(ms.state_nm) as state_nm FROM m_state ms where ms.state_cd='"+statecd+"' ";
       ResultSet rs = db.execSQL(qrystr);
	   while(rs.next()){ 
	   state=rs.getString("state_cd");
	   %>
      <OPTION value="<%=rs.getString("state_cd")%>"  <% if (state.equals(statecd)){out.print("selected");}%> ><%=rs.getString("state_nm")%></OPTION>
      <% } 
	  rs.close();
	   %>
    </SELECT></TD>
...
%>
  ...
  <TR>
    ...
    <TD colspan="2"><INPUT name="APP_DIST_CD" type="hidden" id="APP_DIST_CD" value=""/>
      District*:</TD>
    <TD colspan="2"><INPUT name="APP_DIST_NM" type="text" id="APP_DIST_NM"  style="background-color: #FFCCCC" readonly="true" value=""  />	  <INPUT name="btnComaddr" id="btnComAddr" type="button" class="button" onClick="changeComDistrict();"  value="Change Communication District" /></TD>
    <TD>Pin*:</TD>
  ...
  <TR>
    ...
    <TD colspan="4"><INPUT name="UNIT_DIST_NM" type="text" id="UNIT_DIST_NM" style="background-color: #FFCCCC"  readonly="true" value="" /></TD>
  </TR>
  <TR>
    <TD>&nbsp;</TD>
    <TD colspan="2">Pin*</TD>
  ...
  <TR>
	...
	<INPUT name="b1" type="hidden" id="b1" size="1" maxlength="1" >
...
qryUpdate.append("ONLINE_SUBDT,ATRID,RET_YN ) ");
...
response.sendRedirect("offlineDataexist.jsp");
}catch(Exception e){
