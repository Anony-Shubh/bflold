...
<form method="post" action="upload_app_doc_puf_test.jsp" name="form" id="form" class="form" enctype="multipart/form-data" >
	  ...
	  <p>
        ...
        db.connect();
          ...
          String docStr="SELECT UPM_CD,DOC_DESC FROM APP_UPLOAD_MAST WHERE  UPM_CD="+UPM_CD+"";
		  
		    ResultSet rsDoc = db.execSQL(docStr);

           StringBuffer qry = new StringBuffer();
         qry.append(" SELECT A.APP_ID, ");
		  qry.append(" UPPER(A.APP_NAME) AS APP_NAME, TO_CHAR(SYSDATE,'RRRR')||'/'||TO_CHAR(SYSDATE,'MON') ");
			qry.append(" ||'/'|| TO_CHAR(SYSDATE,'DD') ||'/'||substr(app_id,instr(app_id,'-')+1,length(app_id))||'/'   AS TDIR,   ");
...
//out.print(qry.toString());
	  ...
	  MOB_NO1=rsApp.getString("MOB_NO1")==null?"":rsApp.getString("MOB_NO1");
		...
		TDIR=rsApp.getString("TDIR")==null?"":rsApp.getString("TDIR");
        }
			rsApp.close();
