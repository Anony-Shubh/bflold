...
//out.print(a);
%>
<form method="post" action="upload_app_doc_mmSec.jsp" name="form" id="form" class="form" enctype="multipart/form-data" >
   ...
   <jsp:setProperty name="upBean" property="whitelist" value="*.pdf,*.jpg,*.jpeg,*.png" />
	  ...
	  <p>
        ...
        db.connect();
          ...
          String docStr="SELECT UPM_CD,DOC_DESC FROM APP_UPLOAD_MAST_second WHERE  UPM_CD IN (17,18,19) AND UPM_CD NOT IN (SELECT UPM_CD FROM APP_UPLOAD_TRANS_second WHERE APP_ID='"+APP_ID+"'  AND UPM_CD<>19)";
		  ...
		  qry.append("UPPER(A.APP_NAME) AS APP_NAME,  ");
		   qry.append("  TO_CHAR(A.ONLINE_SUBDT, 'DD-MON-RRRR HH:MI:SS') AS ONLINE_SUB_DT, ");
		    qry.append("  A.MOB_NO1, ");
			 qry.append("  A.E_MAIL,A.AADHAR_NO ");
			  qry.append(" FROM APP_DETAIL_SECOND A ");
			  qry.append("WHERE A.APP_ID = '"+APP_ID+"' ");

        ResultSet rsApp = db.execSQL(qry.toString());
		
		ResultSet rsAppDoc=db.execSQL("SELECT AUT.DOC_NAME, AUT.DOC_REMARKS,AUM.DOC_DESC, TO_CHAR(TRUNC(AUT.UPLOAD_TIMESTAMP),'DD-MON-RRRR') AS UPLOAD_TIMESTAMP FROM APP_UPLOAD_TRANS_SECOND AUT, APP_UPLOAD_MAST_SECOND AUM WHERE AUM.UPM_CD = AUT.UPM_CD AND  AUM.UPM_CD IN (17,18,19) AND AUT.APP_ID='"+APP_ID+"' ORDER BY AUT.UPLOAD_TIMESTAMP DESC ");
	...
	ResultSet rsUp = db.execSQL(qryUp.toString());		
	  ...
	  <table align="center">
        ...
        <tr>
          <td colspan="2" scope="col">
            <span class="redLebel">(Note  Scanned Copy can be uploaded and file size should not be more then 1 mb and format should be in jpg/pdf only)</span> </td>
        </tr>
        
        <tr>
          <td colspan="2" scope="col"><div align="center" class="mtextarea">( Only those documents as per the remarks of PMEGP,KVIC, Mumbai needs to be uploaded) </div></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="2">UPLOADED DOCUMENTS FOR REFERRED CASE: </td>
        </tr>
	  ...
	  <% 
	   String vDOC_NAME="";
	  String vDOC_DESC="";
	  String vUPLOAD_TIMESTAMP="";
	  String vDOC_REMARKS="";
	  int cnt =0;
	  while (rsAppDoc.next()) {
	  cnt=cnt+1;
	  vDOC_NAME=rsAppDoc.getString("DOC_NAME")==null?"":rsAppDoc.getString("DOC_NAME");
	  vDOC_DESC=rsAppDoc.getString("DOC_DESC")==null?"":rsAppDoc.getString("DOC_DESC");
	  vUPLOAD_TIMESTAMP=rsAppDoc.getString("UPLOAD_TIMESTAMP")==null?"":rsAppDoc.getString("UPLOAD_TIMESTAMP");
	  vDOC_REMARKS=rsAppDoc.getString("DOC_REMARKS")==null?"":rsAppDoc.getString("DOC_REMARKS");
	  
	   %>
        <tr>
          <td>(<%= cnt %>) <%= vDOC_DESC %></td>
          <td><%= vDOC_REMARKS %> (<%= vUPLOAD_TIMESTAMP %>)</td>
        </tr>
		 <% }
		 rsAppDoc.close();
		  %>
                ...
                if ( (files != null) && (!files.isEmpty()))
                {
					...
					if (ext.equals("jpg")||ext.equals("jpeg")||ext.equals("pdf")) {
					 upBean.store(mrequest, "uploadfile");
						...
						qryUpdate.append(" INSERT INTO APP_UPLOAD_TRANS_SECOND ( UP_ID,");
						...
						pstm.clear();
							values.clear();	
							db.close();
							response.sendRedirect("upload_app_doc_mmSec.jsp?APP_ID="+APP_ID);
