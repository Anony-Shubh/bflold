...
<form  action="AgnedpDetail.jsp" method="post" name="form" id="form" >
...
String EDP_TRG_NM="";
String OFF_CD="";
String APP_ID=(String) session.getAttribute("APP_ID")==null?"":(String)session.getAttribute("APP_ID");
String APP_NAME="";
String EDP_YN="";
String EDP="";
String EDP_TRG_NAME = "";
...
qrysb.append(" SELECT AD.APP_ID,AD.APP_NAME,AD.APP_DIST_CD,ED.ED_ID,CASE WHEN ED.EDP_TYPE IN (1,2) THEN 'ON' WHEN ED.EDP_TYPE IN (3,4) THEN 'OF' END AS EDP,(SELECT count(*) AS APPC from EDP_CONDUCT_TRANS where APP_ID=ad.app_id AND ACT_YN='Y')  APPC, "      );
qrysb.append("   ED.EDP_TYPE,CASE WHEN ED.EDP_TYPE IN (1,2) THEN '' ELSE  ED.EDP_TRG_NAME END AS  EDP_TRG_NAME"      );
qrysb.append("   ,ED.EDPOFF_CD FROM app_detail_online AD,  EDP_CONDUCT_TRANS ED WHERE AD.APP_ID = ED.APP_ID(+) AND ED.ACT_YN='Y' AND AD.APP_ID='"+APP_ID+"' ");
...
APP_NAME=rsMain.getString("APP_NAME")==null?"1":rsMain.getString("APP_NAME");
APPC=rsMain.getString("APPC")==null?"1":rsMain.getString("APPC");
}
rsMain.close();
...
<tr>
  <th colspan="2"><div align="center">Applicant EDP Detail </div></th>
    ...
    <th> Applicant ID:  
		...
		<input name="h" type="hidden" id="h" value="<%=EDP%>"><input name="APPC" type="hidden" id="APPC" value="<%=APPC%>">
		</th>
  <td><%= APP_ID %></td>
</tr>  
  <tr>
...
<tr id="EDP_TY5">
  ...
  <input name="EDP_TRG_NAME" id="EDP_TRG_NAME" type="text" size="50" maxlength="50" value="<%= EDP_TRG_NM %>" readonly="true">
      ...
      <!--  <a href="edpD.jsp?DIST_CD=<%= DISTRICT_CD %>" class="buttonBlue">Select  EDP Centers</a> -->
 ...
 <input name="OLD_OFF_CD" id="OLD_OFF_CD" type="HIDDEN" size="50" maxlength="50" value="<%= OFF_CD %>">
</tr>  
...
OLD_OFF_CD=(String) request.getParameter("OLD_OFF_CD")==null?"0":(String) request.getParameter("OLD_OFF_CD").trim();
...
EDP_TRG_NAME=EDP_TRG_NAME;
 ...
 StringBuffer qryUpdate= new StringBuffer();
  StringBuffer vqry= new StringBuffer();


String ED1_ID="";
if (!OLD_OFF_CD.equals(OPT_TRG_CENTER)) {

StringBuffer qryEXR = new StringBuffer();

qryEXR.append(" INSERT INTO edp_conduct_trans_LOG (APP_ID,EDPOFF_CD,EDP_TRG_NAME,EDP_FR_DT,EDP_TO_DT,EDP_CERT_DT,ACT_YN,ED_ID,EDPTIMESTAMP,DOC_NAME,DOC_REMARKS,EDP_TYPE,AGN_APPRV,ENB_YN,DEL_USERID,DELTIMESTAMP)SELECT E.APP_ID,  E.EDPOFF_CD,E.EDP_TRG_NAME,E.EDP_FR_DT,E.EDP_TO_DT,E.EDP_CERT_DT,E.ACT_YN,E.ED_ID,E.EDPTIMESTAMP,E.DOC_NAME,E.DOC_REMARKS,E.EDP_TYPE,E.AGN_APPRV,E.ENB_YN,? AS DEL_USERID,SYSDATE AS DELTIMESTAMP FROM edp_conduct_trans E WHERE ED_ID=? AND APP_ID=? ");
...
StringBuffer qryu= new StringBuffer();
		qryu.append("DELETE FROM  EDP_CONDUCT_TRANS  WHERE APP_ID=? AND ED_ID=? ");
		values.clear();
        pstm.clear();
		values.add(APP_ID); pstm.add ("L");
		values.add(ED_ID); pstm.add ("L");
...
values.clear();
 ...
 ResultSet rsMax= db.execSQL("SELECT APPUPLOAD_SEQ.NEXTVAL AS MAX_ID FROM DUAL");
     while (rsMax.next()) {
     ED1_ID=rsMax.getString(1);
     }
rsMax.close();

//out.print("OLD"+ED_ID);
//out.print("NEW"+ED1_ID);
...
qryUpdate.append(" UPDATE EDP_CONDUCT_TRANS SET APP_ID=?,EDPOFF_CD=?, EDP_TRG_NAME=?, ACT_YN=?,EDP_TYPE=?, AGN_APPRV=SYSDATE WHERE ED_ID=?   ");
values.add(APP_ID); pstm.add ("L");
values.add(OPT_TRG_CENTER); pstm.add ("L");
values.add(EDP_TRG_NAME); pstm.add ("L");
values.add(ACT_YN); pstm.add ("L");
