...
String EMAIL = "";
String DLTFC_TOTAL="";
String DAYS_PENDINGWITH_AGENCY="";
String DAYS_PENDINGWITH_DLTFC="";
...
qrysb.append("     AD.REMARKS         AS AGENCY_REMARKS,"      );
qrysb.append("     AM.ACTIVITY_NAME,"      );
qrysb.append("     AD.PROD_DESC,"      );
qrysb.append("     AD.DLTFC_REMARK,"      );
qrysb.append("     NVL(AD.MACHINARY_COST, 0)                              AS PROP_CE,"      );
qrysb.append("     NVL(AD.WORKING_CAPITAL, 0)                             AS PROP_WC,"      );
qrysb.append("     NVL(AD.MACHINARY_COST, 0) + NVL(AD.WORKING_CAPITAL, 0) AS PROP_PROJ_TOTAL,"      );
qrysb.append("     TO_CHAR(AD.ONLINE_SUBDT, 'dd-mon-yyyy')                AS ONLINE_SUBDT,"      );
qrysb.append("     TO_CHAR(AD.FORD_DT_DLTFC, 'dd-mon-yyyy')               AS FORD_DT_DLTFC,"      );
qrysb.append("      TRUNC(SYSDATE)-TRUNC(AD.ONLINE_SUBDT) AS DAYS_PENDINGWITH_AGENCY,"      );
qrysb.append("      TRUNC(SYSDATE)-TRUNC(AD.FORD_DT_DLTFC) AS DAYS_PENDINGWITH_DLTFC,"      );
qrysb.append("     AD.BANK_DISTRICT,"      );
qrysb.append("     AD.BRANCH_NAME,"      );
qrysb.append("     AD.BANK_F_DATE,"      );
qrysb.append("     AD.BANK_NAME,"      );
...
qrysb.append("     NVL(AD.DLTFC_CE, 0)                       AS DLTFC_CE,"      );
qrysb.append("     NVL(AD.DLTFC_WC, 0)                       AS DLTFC_WC,"      );
qrysb.append("     NVL(AD.DLTFC_CE, 0) + NVL(AD.DLTFC_WC, 0) AS DLTFC_TOTAL,"      );
qrysb.append("     CASE WHEN AD.ACT_ID IN (0,1) AND TRUNC(SYSDATE)-TRUNC(AD.ONLINE_SUBDT)>45 THEN"      );
qrysb.append("   '45 days Pending With Agency'"      );
qrysb.append("   WHEN AD.ACT_ID=2 AND TRUNC(SYSDATE)-TRUNC(AD.FORD_DT_DLTFC)>45 THEN"      );
qrysb.append("   '45 days Pending With DLTFC'"      );
qrysb.append("   WHEN AD.ACT_ID=4 THEN "      );
qrysb.append("   'Approved by DLTFC and Yet to be Forward to Bank'"      );
qrysb.append("   ELSE"      );
qrysb.append("   'NA'"      );
qrysb.append("   END PENDING_STATUS"      );
qrysb.append("   "      );
qrysb.append("   FROM RBIBANKLIST RBL,"      );
qrysb.append("     APP_DETAIL AD,"      );
qrysb.append("     APP_STATUS_MAST ASM,"      );
qrysb.append("     M_DISTRICT UMD,"      );
qrysb.append("     ACTIVITY_MAST AM"      );
qrysb.append("   WHERE AD.ACTIVITY_CD = AM.ACTIVITY_CD"      );
qrysb.append("   AND AD.IFSC_CODE     = RBL.IFSC_CODE"      );
qrysb.append("   AND AD.ACT_ID        = ASM.ACT_ID AND AD.OFF_CD=? "      );
qrysb.append("   AND AD.UNIT_DIST_CD  = UMD.DISTRICT_CD"      );
qrysb.append("   AND AD.APP_ID        = ?) WHERE PENDING_STATUS <>'NA'"      );
...
DAYS_PENDINGWITH_AGENCY=rsApp.getString("DAYS_PENDINGWITH_AGENCY")==null?"":rsApp.getString("DAYS_PENDINGWITH_AGENCY");
DAYS_PENDINGWITH_DLTFC=rsApp.getString("DAYS_PENDINGWITH_DLTFC")==null?"":rsApp.getString("DAYS_PENDINGWITH_DLTFC");
PENDING_STATUS=rsApp.getString("PENDING_STATUS")==null?"":rsApp.getString("PENDING_STATUS");
...
%>
  ...
  <table align="center">
    <tr>
      <th colspan="4"><div align="center" class="mtextarea">Foward the Applications which were 45 days Pending with Agency or DLTFC</div></th>
    </tr>
    <tr>
      <th>PENDING STATUS: </th>
      <th colspan="3"><H1 class="warning"><%= PENDING_STATUS %></H1></th>
    </tr>
    <tr>
    ...
    <tr >
      ...
      <th><%=  ONLINE_SUBDT %>(Days:<%= DAYS_PENDINGWITH_AGENCY %>) </th>
    </tr>
...
db.executeUpdate() ;
db.close();
response.sendRedirect("savemsgbforward.jsp");
}
