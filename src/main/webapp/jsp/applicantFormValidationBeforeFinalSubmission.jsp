...
qry.append(" SELECT A.APP_ID, A.GENDER,CASE WHEN A.IND_TYPE='M' AND A.ACTIVITY_CD IN (312,313,314,315,316) THEN 'N' ELSE 'Y' END AS ACT_VALID, FUNCAVALID_APP(A.APP_ID)  AS ADHVAL, MD.STATE_CD,A.BENF_TYPE_CD,NVL(A.MACHINARY_COST, 0) MACHINARY_COST,A.EDP_YN,(SELECT COUNT(*) AS EDP_C FROM EDP_CONDUCT_TRANS WHERE APP_ID=A.APP_ID) AS EDP_C,A.ACTIVITY_CD,case when MD.DISTRICT_CD in (600,118) then 'N' else 'Y' end as dist_subyn, ");
		 ...
		 qry.append("  TO_CHAR(A.ONLINE_SUBDT, 'DD-MON-RRRR HH:MI:SS') AS ONLINE_SUB_DT, TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE),TRUNC(A.DOB))/12) AS AGE,MS.ZONE_CD,");
  ...
  ZONE_CD=rsApp.getString("ZONE_CD")==null?"0":rsApp.getString("ZONE_CD");
  ...
  ACTIVITY_CD=rsApp.getString("ACTIVITY_CD")==null?"0":rsApp.getString("ACTIVITY_CD");
  try {
...
aerror.add("For Unit Location in Urban Area, Select sponsoring Agency DIC");
  ...
  if (AADHAR_NO.equals("")) {
   ...
   if (!AADHAR_NO.equals("")) {
    adharvalid = vaf.validateAadharNumber(AADHAR_NO);
   if (!adharvalid) { 
    aerror.add( "Invalid Aadhaar Number:");
  ...
  if (!PAN_NO.equals("")) {
    apan = vaf.validatePAN(PAN_NO);
   if (!apan) {  aerror.add( "Invalid PAN Number:");
...
qrydup.append(" SELECT * FROM (  SELECT APP_ID,PAN_NO,ACT_YN FROM APP_DETAIL WHERE TRIM(UPPER(PAN_NO))=TRIM(UPPER('"+PAN_NO+"')) ");
 ...
 if (!BENF_TYPE_CD.equals("1")) {
  ...
  if ((!ZONE_CD.equals("NE")) && (ACTIVITY_CD.equals("316") )){
   aerror.add(" This Activity Belongs to North East Region.......");
  }
...
qryupdt.append("   SELECT AUM.UPM_CD, ");
 qryupdt.append(" AUM.DOC_DESC FROM APP_UPLOAD_MAST AUM WHERE ACT_YN='Y' AND ");
