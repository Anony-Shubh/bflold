...
<form method="post" action="app_uploadView.jsp" name="form" id="form" class="form">
		  ...
		  qry.append("UPPER(A.APP_NAME) AS APP_NAME,  ");
		   qry.append("  TO_CHAR(A.ONLINE_SUBDT, 'DD-MON-RRRR HH:MI:SS') AS ONLINE_SUB_DT, ");
		    qry.append("  A.MOB_NO1, ");
			 qry.append(" A.DOB,  A.E_MAIL,A.AADHAR_NO,A.ACT_ID ");
	...
	ResultSet rsUp = db.execSQL(qryUp.toString());		
	  ...
	  <table align="center">
        ...
        <tr>
		  ...
		  <a href="../pmegpIILOAN/PMEGPGenAppForm.jsp?APP_ID=<%= APP_ID %>" class="button">View Applicantion</a>
		  
		   <% if (ACT_ID.equals("5")){ %>
		  <a href="../pmegpIILOAN/PMEGPGenAppForm.jsp?RNAME=SecondBankForwardingLetter&APP_ID=<%= APP_ID %>" class="button">Download Bank Forwarding Letter</a>
		  <%}%></th>
        </tr>
        <tr>
          <th nowrap>SrNo
  ...
  %>
        <tr>
          ...
          <td><%= vDOC_DESC %></td>
          <td><%= vDOC_REMARKS %></td>
          <td><a href="docView.jsp?docname=<%= vDOC_NAME %>" class="greenButtonRound">view Document </a> </td>
        </tr>
        <%
  }
  rsUp.close();
