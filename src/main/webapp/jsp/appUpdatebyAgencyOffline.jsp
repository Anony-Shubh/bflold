...
qrysb.append("   SELECT AD.OFF_CD,AD.TECH_QUALI,AD.BLDG_TYPE, " );
...
ResultSet rsDist = db.execSQL("SELECT DISTRICT_CD,DISTRICT_NAME FROM M_DISTRICT WHERE STATE_CD='"+STATE_CD+"' AND DISTRICT_STATUS='A' ");
...
<form name="form"  id="form" method="post"  >
  ...
  <tr bgcolor="#D1A476">
    <td colspan="7">
      <h3>PMEGP OFFLINE APPLICATION  FOR INDIVIDUAL APPLICANT </h3>
	  <div align="right">
	   <a href="AppSearchLOVOFF.jsp" class="greenButtonRound">Back</a>
	    <a href="upload_app_doc_agency_offline.jsp?APP_ID=<%= APP_ID %>" class="greenButtonRound">Upload</a>
		 <a href="appngenupdateoffline.jsp?APP_ID=<%= APP_ID %>&TYPE=2" class="greenButtonRound">Score Card </a></div>
    </td>
  </tr>
  <tr>
    <th>(1)</th>
    <th colspan="2" nowrap><strong>AADHAAR Card No: </strong></th>
    <td colspan="4"><input name="AADHAR_NO" type="text" id="AADHAR_NO" value="<%= AADHAR_NO %>" readonly="true" size="17" maxlength="12" />   (Not Editable)</td>
  </tr>
  <tr>
    <th>(2)</th>
    <th colspan="2"><strong>Name of Applicant:*</strong></th>
    <td colspan="4"><select name="APP_PRIX">
      ...
      <option value="M/s." <% if (APP_PRIX.equals("M/s.")){out.print("selected");}%>>M/s.</option>
        ...
        <input name="APP_NAME" type="text"  id="APP_NAME"  value="<%=APP_NAME  %>"/> 
        </td>
  </tr>
  ...
  <tr>
    ...
    <td colspan="4"><select name="AGENCY" class="button" id="AGENCY" disabled="disabled"  onchange="showOffice(this.value);" >
  ...
  <tr>
    ...
    <td colspan="4"><select name="STATE" class="button"  disabled="disabled" id="STATE" onBlur="showOffice(this.value);" onChange="showDistrict(this.value);">
...
%>
  ...
  <tr>
    ...
    <td colspan="4"><select name="IND_TYPE" class="button" id="IND_TYPE" >
	  ...
	  <input name="btnActList" type="button" class="button" id="btnActList"  onClick="activity();"  value="Select Industry / Activity" />
...
qryUpdate.append("BENF_SPECAT_CD=?, APP_NAME=?, ");
qryUpdate.append(" ONLINE_SUBDT=SYSDATE,TECH_QUALI=?,BLDG_TYPE=? WHERE APP_ID=?  ");
