...
if (state.equals("")||state.equals("-1") ){
 ...
 String off_desc=orgcd.equals("KV")?"Khadi and Village Industries Commission":"";
  DBCon db= new DBCon();
db.connect();
 ResultSet rs = db.execSQL("SELECT nvl(OFF_TEL_NO,'--') as OFF_TEL_NO, nvl(OFF_EMAIL_ADDR,'--') as OFF_EMAIL_ADDR, mom.off_cd,motc.off_type_long_desc,mom.off_addr1||','||mom.off_addr2 as addr, mom.city ,mom.pincode ,initCap(md.district_name) as dist,initCap(ms.state_nm)  as state FROM mas_off_mast mom ,mas_off_type_cdmast motc ,m_district md, m_state ms where   mom.off_type_cd=motc.off_type_cd and mom.off_type_cd in (3,2,5,22,27) and mom.district_cd=md.district_cd and md.state_cd=ms.state_cd  and mom.org_cd='"+orgcd+"' and md.state_cd='"+state+"' AND MD.DISTRICT_CD="+distcd+" AND MOM.ACT_YN='Y' ");  
