<%@ page contentType="text/html; charset=iso-8859-1" buffer="200kb" language="java" import="java.sql.*,pkgPmegpNew.DBCon,kvic.inputvalid.*" %>
...
<form name="form" id="form" class="form" method="post">
 ...
 jspinputvalid js = new jspinputvalid();
  ...
  String BANK_NAME =request.getParameter("BANK_NAME")==null?"Please Select Bank": js.getOnlyText( (String)request.getParameter("BANK_NAME"),70); 
 // out.print(BANK_NAME);
 // BANK_NAME=sanrem(BANK_NAME);
  String SDISTRICT=request.getParameter("SDISTRICT")==null?"0": js.getOnlyText( (String)request.getParameter("SDISTRICT"),30); 
    String BRIFSC=request.getParameter("BRIFSC")==null?"": js.getOnlyText((String)request.getParameter("BRIFSC"),11); 
  String BRADDR=request.getParameter("BRADDR")==null?"": js.getOnlyText( (String)request.getParameter("BRADDR"),30); 

    String vstate =request.getParameter("state")==null?"": js.getOnlyText( (String)request.getParameter("state"),2); 
	
	String vStateqry="";
	
	if (vstate.equals("TG")||vstate.equals("AP")) {
  vStateqry="  RBI.STATE_CD IN ('TG','AP')";
  }else if (vstate.equals("JK")||vstate.equals("LK")) {
  vStateqry="  RBI.STATE_CD IN ('JK','LK')";
  }else if (vstate.equals("TN")||vstate.equals("PY")) {
  vStateqry="  RBI.STATE_CD IN ('TN','PY')";
  }else{
...
qrysb.append(" WHERE RBI.ACT_YN='Y' AND TRIM(UPPER( REGEXP_REPLACE(BANK_NAME,'[^a-zA-Z0-9\\s+]','')   ) ) LIKE TRIM(UPPER(  REGEXP_REPLACE('"+BANK_NAME+"','[^a-zA-Z0-9\\s+]','') ) ) AND "+vStateqry+"   "+vWherecls+"  " );
