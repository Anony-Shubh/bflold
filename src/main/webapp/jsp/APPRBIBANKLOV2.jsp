    ...
    $('#example tbody').on( 'click', 'tr', function () {
        if ( $(this).hasClass('selected') ) {
            ...
            $(this).addClass('selected');
			 ...
			 window.parent.getBankLOV2(IFSC);
...
qrysb.append(" WHERE RBI.ACT_YN='Y' AND UPPER(BANK_NAME) LIKE '"+BANK_NAME+"'  AND "+vStateqry+"   "+vWherecls+"  " );
//qrysb.append(" WHERE RBI.ACT_YN='Y' AND BANK_NAME LIKE '"+BANK_NAME+"'  AND RBI.STATE_CD='"+vstate+"'  "+vWherecls+"  " );
 ...
 ResultSet rsDist=db.execSQL(" SELECT DISTRICT_CD,DISTRICT_NAME,MS.STATE_NM FROM M_DISTRICT MD,M_STATE MS WHERE MD.DISTRICT_STATUS='A' AND MD.STATE_CD=MS.STATE_CD AND MS.STATE_CD='"+vstate+"' ORDER BY DISTRICT_NAME");
  ...
  <table align="center" class="bordered">
    ...
    <tr>
	 ...
	 String vstatenm="";
	   while (rsDist.next()) { %>
        <option value="<%= rsDist.getString(1) %>"><%= rsDist.getString(2) %></option>
		<%
