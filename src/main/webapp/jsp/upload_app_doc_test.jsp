...
qryUp.append(" (SELECT * FROM TABLE(FUPLOAD_DOC(?))) ");
  ...
  %>
        <tr>
		  ...
		  <input type="button" name="button" id="button" value="View" onClick="viewdocument('<%= vDOC_NAME %>');">
		<!--  <a href="doctransfertoNas.jsp?docname=<%//= vDOC_NAME %>" class="greenButtonRound"  onClick="javascript:Popup(this.href, 'View Document', 800, 350);return (false);">view </a>  -->		  
		  
		  <% }else{%>
		  <a href="upload_app_doc_puf_test.jsp?UPM_CD=<%= UPM_CD %>" class="greenButtonRound" onClick="javascript:Popup(this.href, 'Upload Document', 800, 350);return (false);">Upload </a>
		  <% } %>
		   </td>
          <td>
		  
		  
		 <% if (!UP_ID.equals("0") && (!ACT_ID.equals("0"))) {%>
		  <a href="upload_app_doc_del.jsp?UP_ID=<%= UP_ID %>" class="greenButtonRound" onClick="javascript:Popup(this.href, 'Upload Document', 800, 500);return (false);">delete</a>
		 <% } else if (ACT_ID.equals("0") && (!UP_ID.equals("0")) ) {%>
		  Data Lock
		  <%}%>
		</td>
        </tr>
