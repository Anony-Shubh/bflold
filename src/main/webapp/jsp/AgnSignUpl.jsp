...
String SIGNC="";
 ...
 ResultSet rsMANU= db.execSQL("SELECT AD.APP_ID,CASE WHEN TRUNC(AD.ONLINE_SUBDT) > '31-MAY-2022' AND trunc(IST_LOAN_DT) IS NOT NULL AND BD.ACT_ID=11 THEN 'Y' ELSE 'N' END AS DY_N,(SELECT COUNT(*) FROM APP_UPLOAD_TRANS WHERE APP_ID=AD.APP_ID and upm_cd=16) AS SIGNC FROM APP_DETAIL  AD,BANK_DATAENTRY BD  WHERE AD.APP_ID=BD.APP_ID(+)  AND AD.APP_ID='"+vAppid+"' ");
		   while (rsMANU.next()) {  //3
	 ...
	 if (DY_N.equals("Y") && (SIGNC.equals("0"))){
	  db.close();	 
	  response.sendRedirect ("../bankModule/PmegpBoardUpload.jsp?APP_ID="+vAppid);	  
	  }	  
	  else {	  
