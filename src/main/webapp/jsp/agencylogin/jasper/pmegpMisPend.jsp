...
String vFname=request.getParameter("REPNM")==null?"pendingMISBank":request.getParameter("REPNM");
	...
	if(QRYCODE.equals("1"))
	{
		vcls=vcls + " AND MOM.OFF_CD="+vooff_cd+" AND RBI.BANK_NAME='"+vBANK_NAME+"' AND  NVL(BD.ACT_ID, 0) IN (0, 8) AND TRUNC(AD.BANK_F_DATE) BETWEEN '"+FromDt+"' AND '"+ToDt+"' ";
	}
	else{
		vcls=vcls + "  AND MOM.OFF_CD="+vooff_cd+"  AND RBI.BANK_NAME='"+vBANK_NAME+"' AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NULL  AND BD.MM_CLAIM_DT IS NULL AND TRUNC(BD.LOAN_SANC_DT) BETWEEN '"+FromDt+"' AND '"+ToDt+"' ";
	}
