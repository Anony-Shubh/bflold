...
String vdist_nm=session.getAttribute("sdistrict_cd")=="ALL"?"%%":(String) session.getAttribute("sdistrict_cd");

String vfldQryDesc="";
String vCurStatus="";
//String addUrl="AGENCY="+vAgency+"&ZONECD="+vZoneCD+"&STATECD="+ vStateCd+"&OFFNAMECD="+vOffname+"&DISTCD="+vDist+"&FROMDT="+FromDt+"&TODT="+ToDt;
...
// UNDER PROCESS AT AGENCY
  ...
  txtSearch = txtSearch + " and (ad.FORD_DT_DLTFC IS NOT NULL OR ad.MTG_ID IS NOT NULL)";
 ...
 // Sanctioned by DLTFC COMMITTEE
  txtSearch = txtSearch + " and TRUNC(ad.online_subdt) between '"+FromDt+"' and '"+ToDt+"' and ( ad.act_id<>7 AND AD.ACT_ID>=4) ";
 
  }else if (vQryCode.equals("5")){
 // Forwarded to Bank
  txtSearch = txtSearch + " and TRUNC(ad.online_subdt) < '"+FromDt+"'  and ad.act_id=5  and NVL(BD.ACT_ID,0) NOT IN (11,3)";
  }else if (vQryCode.equals("6")){
 // Sanctioned by Bank 
  txtSearch = txtSearch + " and TRUNC(ad.online_subdt) between '"+FromDt+"' AND '"+ToDt+"' and ad.act_id=5 ";
}

else if (vQryCode.equals("8")){
 // Rejected by Bank
  txtSearch = txtSearch + " and TRUNC(AD.ONLINE_SUBDT) < '"+FromDt+"' and AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NULL AND BD.MM_CLAIM_DT IS NULL   ";
 }else if (vQryCode.equals("9")){
 // Own Contribution Deposited
  txtSearch = txtSearch + " and   TRUNC(AD.ONLINE_SUBDT) between '"+FromDt+"' AND '"+ToDt+"' and AD.ACT_ID=5 AND BD.ACT_ID=11 ";
 }
 
 
 else if (vQryCode.equals("11")){
 //  LOAN RELEASED
  txtSearch = txtSearch + " and TRUNC(BD.MM_CLAIM_DT) < '"+FromDt+"'  and AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND NVL(BD.PACT_ID,0) <> 34  ";
  }else if (vQryCode.equals("12")){
 // EDP TRANING COMPLETED
  txtSearch = txtSearch + " and TRUNC(BD.MM_CLAIM_DT) between '"+FromDt+"' AND '"+ToDt+"' and  AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND NVL(BD.PACT_ID,0) <> 34 ";
 }
 
 
 else if (vQryCode.equals("14")){
 // MM CLAIMED
  txtSearch = txtSearch + " and AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND NVL(BD.PACT_ID,0) = 34  AND TRUNC(BD.MM_REL_DT)  between '"+FromDt+"' AND '"+ToDt+"'   ";
 }else if (vQryCode.equals("1")){
 // MM RELEASED
  txtSearch = txtSearch + " and TRUNC(BD.MM_CLAIM_DT) between '"+FromDt+"' AND '"+ToDt+"' and AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND BD.PACT_ID NOT IN(34,30) AND BD.APPRV_YN='A'  ";
 ...
 // Pending for MM Release
  txtSearch = txtSearch + " and TRUNC(ad.online_subdt)  between '"+FromDt+"' and '"+ToDt+"' and ad.act_id=3  ";
   ...
   txtSearch = txtSearch + " and TRUNC(BD.MM_CLAIM_DT) between '"+FromDt+"' and '"+ToDt+"' AND  AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND BD.MM_CLAIM_AMT >0 AND  BD.PACT_ID IS NULL ";
 }else if (vQryCode.equals("19")){
  txtSearch = txtSearch + " and TRUNC(BD.MM_CLAIM_DT) between '"+FromDt+"' and '"+ToDt+"' and AD.ACT_ID=5 AND BD.ACT_ID=11 AND BD.LOAN_DOCFNAME IS NOT NULL AND BD.MM_CLAIM_DT IS NOT NULL AND BD.MM_CLAIM_AMT >0 AND BD.PACT_ID =30   ";
 }
...
sb.append(" SELECT ad.app_id,ad.app_name,UPPER(ad.fh_name) AS FH_NAME,mom.agency_type, mom.off_name,ad.bank_name, NVL(ad.remarks,'') AS Remraks_up_rej ,AD.UNIT_LOC,").
append(" ms.state_nm,ad.mob_no2 AS TEL_NO,ad.E_MAIL, ad.aadhar_no  AADHAR_NO,TO_CHAR(AD.ONLINE_SUBDT,'DD-MON-RRRR') ONLINESUBDT,  AD.BENF_SPECAT_CD,BD.PAY_STATUS,BD.UTRN_NO,BD.FAIL_REASON, ").
...
append(" and bd.tr_id=edpom.off_cd(+)  and MOM.OFF_CD="+vooff_cd+"  ").
append(" "+txtSearch+" ").
append("  ").
...
//response.setHeader("Content-Disposition", "inline; filename=" + "excel.xls"); 
  ...
  <tr>
   ...
   <% while (rsinst.next()) {
  srn=srn+1;
  p3="&BANKNAME="+rsinst.getString("BANK_NAME")+"&QRYCODE="+vQryCode;
   %>
  <tr>
