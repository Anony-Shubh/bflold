...
//out.print(a);
%>
<form method="post" action="upload_app_doc_mm.jsp" name="form" id="form" class="form" enctype="multipart/form-data" >
   ...
   <jsp:setProperty name="upBean" property="whitelist" value="*.pdf,*.jpg,*.jpeg,*.png" />
	  ...
	  <p>
        ...
        db.connect();
          ...
          String docStr="SELECT UPM_CD,DOC_DESC FROM APP_UPLOAD_MAST WHERE  UPM_CD IN (8,17,18,19) AND UPM_CD NOT IN (SELECT UPM_CD FROM APP_UPLOAD_TRANS WHERE APP_ID='"+APP_ID+"' AND UPM_CD<>8 AND UPM_CD<>19)";
		...
		ResultSet rsAppDoc=db.execSQL("SELECT AUT.DOC_NAME, AUT.DOC_REMARKS,AUM.DOC_DESC, TO_CHAR(TRUNC(AUT.UPLOAD_TIMESTAMP),'DD-MON-RRRR') AS UPLOAD_TIMESTAMP FROM APP_UPLOAD_TRANS AUT, APP_UPLOAD_MAST AUM WHERE AUM.UPM_CD = AUT.UPM_CD AND  AUM.UPM_CD IN (8,17,18,19) AND AUT.APP_ID='"+APP_ID+"' ORDER BY AUT.UPLOAD_TIMESTAMP DESC ");
					...
					if (ext.equals("jpg")||ext.equals("jpeg")||ext.equals("pdf")) {
					 upBean.store(mrequest, "uploadfile");
						...
						pstm.clear();
							values.clear();	
							db.close();
							response.sendRedirect("upload_app_doc_mm.jsp?APP_ID="+APP_ID);
