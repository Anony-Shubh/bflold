...
String repnm="centerWiseEdp.jsp";
 ...
 urlrepE ="centerWiseEdpType.jsp?TODT="+ToDt+"&FROMDT="+FromDt+"&AGENCY="+vAgency+"&ZONECD="+vZonecd+"&STATEDC="+vStatedc+"&STATECD="+vStatecd+"&OFF_TYPE_CD=";
 String EDPOFF_CD="";
String STATE_NM="";
String OFF_NAME="";
String PREON_EDP="";
...
String   PENPOSTOF_EDP="";  
 ...
 //out.print(vDist);
 txtSearch=txtSearch+" AND MOM.AGENCY_TYPE LIKE '"+vAgency+"' AND ZM.ZONE_NM LIKE '"+vZonecd+"' AND MS.STATE_NM LIKE '"+vStatecd+"' AND DM.DISTRICT_NAME LIKE '"+vDist+"' AND MOM.OFF_NAME1 LIKE '"+vOffice+"' AND MS.STATE_NM LIKE '"+vStatedc+"' ";
...
qrysb.append("     "      );
qrysb.append("   FROM"      );
qrysb.append("     (SELECT A.OFF_NAME,A.OFF_TYPE_CD,"      );
...
qrysb.append("           ZONE_MAST ZM,MAS_OFF_TYPE_CDMAST MOC"      );
...
qrysb.append("         ) A"      );
qrysb.append("     GROUP BY A.OFF_NAME,A.OFF_TYPE_CD"      );
qrysb.append("     ) A"      );
qrysb.append("   WHERE A.EDP_COMP > 0"      );
qrysb.append("   GROUP BY ROLLUP(A.OFF_NAME)  "      );
qrysb.append("   ORDER BY EDP_COMP  DESC"      );
...
<%if (vfields.equals("3")) {
p3=vFurl+"&DISTCD="+vnmdw+"&STATECD="+firstcolold+"&STATEDC="+vStatedc+"&";
//out.print(vnmdw);
}else{
