...
out.print ("<center>PMEGP e-Tracking System</center>");
 ...
 sb.append("SELECT A.* ").
append(" FROM ( ").
append(" SELECT  NVL(DECODE(MS.STATE_NM,NULL,'Total',MS.STATE_NM),'Total') as bank_name").
...
append(",sum( nvl(case when BD.ACT_ID>=11 and ad.act_id=5 then 1 end ,0)) as bank_sanc ").
append(" ,sum( nvl(case when  bd.act_id>=11 and ad.act_id=5  then ROUND( MM_AMT_BANKSANC(BD.APP_ID)/100000,2) end ,0)) as bank_sanc_mm").
...
append(" ,sum( nvl(case when  bd.act_id=11 then ad.emp_env end ,0)) as emp "). 
 append(" ,sum( nvl(case when  bd.act_id=3 then round(mminvolve(bd.app_id)/100000,2) end ,0)) as bank_rej_mm").
 append(",sum( nvl(case when  bd.act_id=3 then 1 end ,0)) as bank_reject ").
 append(", sum( nvl(case when  nvl(bd.ACT_ID,0)  in (0,8) and ad.act_id=5 then 1 end ,0)) as pend_bank_sanc  ").
 append(" ,sum( nvl(case when  nvl(bd.ACT_ID,0)  in (0,8) and ad.act_id=5 then round(NVL(mminvolve(AD.app_id),0)/100000,2) end ,0)) as pend_bank_sanc_mm ").
  append(" ,sum( nvl(case when bd.MM_CLAIM_AMT>0 AND BD.LOAN_DOCFNAME IS NOT NULL and bd.act_id>=11 AND NVL(PACT_ID,0) <>34 then round(bd.mm_claim_amt/100000,2) end ,0)) as Pend_mmrel_amt ").
  append(" ,sum( nvl(case when bd.MM_CLAIM_AMT>0 AND BD.LOAN_DOCFNAME IS NOT NULL and bd.act_id>=11 AND BD.MM_REL_DT IS NULL  then 1 end ,0)) as Pend_mmrel ").
...
append("  AND ad.BANK_NAME LIKE 'CORPORATION BANK' AND TRUNC(ad.ONLINE_SUBDT) BETWEEN   '"+FromDt+"' and '"+ToDt+"' ").
append(" and ad.off_cd=mom.off_cd(+) AND MS.zone_cd=ZM.zone_cd group by ROLLUP( NVL(DECODE(MS.STATE_NM,NULL,'Total',MS.STATE_NM),'Total'))  )A WHERE A.BANK_FORD>0 ");
