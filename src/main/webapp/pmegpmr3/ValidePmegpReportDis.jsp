...
String Statep=request.getParameter("State")==null?"%%":(String) request.getParameter("State");
String vAgencytit="";
String vsZonetit="";
String vsStatetit="";
...
qrysb.append("   SELECT nvl(state_nm,'Grand Total') as ZONE_NM,nvl(district_name,'Total') as State_nm,COUNT(*) AS NOOFAPPREC,ROUND(SUM(mm_claim_amt)/10000000,2) AS MMCLAIMONREC ,"      );
qrysb.append("   ROUND(SUM(SANC)/10000000,2) AS LOANSANCONREC,"      );
qrysb.append("   SUM(DECODE(APPRV_YN,'A',1,0)) NO_APPRV,"      );
qrysb.append("   ROUND(SUM(DECODE(APPRV_YN,'A',mm_claim_amt,0))/10000000,2) AS MMCLAIMONAPPRV,"      );
qrysb.append("   ROUND(SUM(DECODE(APPRV_YN,'A',SANC,0))/10000000,2) AS SANCAPPRV"      );
qrysb.append("   FROM "      );
qrysb.append("   ("      );
qrysb.append("   SELECT"      );
...
qrysb.append("       nvl(bd.working_capital, 0) + nvl(bd.machinary_cost, 0) AS prj_cost,"      );
qrysb.append("       bd.mm_claim_amt,"      );
qrysb.append("      nvl( bd.ce_sanc_fb,0)+"      );
qrysb.append("      nvl( bd.wc_sanc_fb,0) as sanc,"      );
qrysb.append("      ZM.ZONE_CD,"      );
...
qrysb.append("     AND BD.PACT_ID=32 AND APPRV_YN<>'R' AND MS.STATE_NM like'"+Statep+"'  "      );
qrysb.append("   "      );
qrysb.append("    )"      );
qrysb.append("       GROUP BY ROLLUP(STATE_NM,DISTRICT_NAME)"      );
...
String ZONE_NM=rsinst.getString("ZONE_NM");
String State_nm=rsinst.getString("State_nm")==null?"Total":rsinst.getString("State_nm");
%>
...
<td>
  <% out.print(dfd.format(rsinst.getDouble("NOOFAPPREC")));%>
</td>
<td><div align="right">

      <% out.print(dfd.format(rsinst.getDouble("MMCLAIMONREC")));%>
...
<td >
    <div align="right">
      ...
      <% out.print(dfd.format(rsinst.getDouble("LOANSANCONREC")));%>
</div></td>

<td>
  <% out.print(dfd.format(rsinst.getDouble("NO_APPRV")));%>
</td>
<td> <% out.print(dfd.format(rsinst.getDouble("MMCLAIMONAPPRV")));%></td>
<td> <% out.print(dfd.format(rsinst.getDouble("SANCAPPRV")));%></td>
