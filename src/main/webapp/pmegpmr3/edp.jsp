...
String PENON_EDP="";
String PENOF_EDP="";
 urlrep ="edpAppdetail.jsp?TODT="+ToDt+"&FROMDT="+FromDt+"&STATECD="+STATE_CD+"&off_cd="+EDPOFF_CD;
String POSTON_EDP="";
String TOTON_EDP="";
String PREOF_EDP="";
String POSTOF_EDP="";
 ...
 qrysb.append("   SELECT  MS.STATE_NM,MS.STATE_CD,ED.EDPOFF_CD,NVL(mom.OFF_NAME,'Total') as OFF_NAME,"      );
qrysb.append("   "      );
...
qrysb.append("  and ed.EDPOFF_CD="+EDPOFF_CD+" AND MS.STATE_CD='"+STATE_CD+"' GROUP BY ROLLUP(mom.OFF_NAME),ms.STATE_NM,ed.EDPOFF_CD,ms.state_cd"      );
...
<td><div align="center"><%=OFF_NAME %></a></div> </td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=1"><%=PREON_EDP  %></a></td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=2"><%=POSTON_EDP  %></a>	</td>
<td  class="sum"><a href="<%=urlrep%>&QRYCODE=3"><%=TOTON_EDP  %></a></td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=4"><%=PREOF_EDP  %></a>	</td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=5"><%=POSTOF_EDP  %></a></td>
<td  class="sum"><a href="<%=urlrep%>&QRYCODE=6"><%=TOTOF_EDP  %></a>	</td>
<td  class="sum"><a href="<%=urlrep%>&QRYCODE=7"</a></td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=8"><%=COMON_EDP  %></a>	</td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=9"><%=COMOF_EDP  %></a>	</td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=10"><%=PENON_EDP  %></a>	</td>
<td class="sum"><a href="<%=urlrep%>&QRYCODE=11"><%=PENOF_EDP  %></a>	</td>
<%
 } //end of while
 rsMain.close();
 db.close();
...
<div class="footer"><B>DESIGNED AND MAINTAINED BY DIRECTORATE OF INFORMATION TECHNOLOGY, KVIC, MUMBAI</B></div>
