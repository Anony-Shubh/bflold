...
String vSTATE = request.getParameter("STATECD")==null||request.getParameter("STATECD")=="ALL"?"%%":(String) request.getParameter("STATECD"); 
...
sb.append("SELECT A.* ").
append(" FROM ( ").
append(" SELECT  AD.bank_name as bank_name,NVL(DECODE(dm.DISTRICT_NAME,NULL,'Total',dm.DISTRICT_NAME),'Total') as DISTRICT_NAME,MS.STATE_NM,MS.STATE_CD  ").
...
append(" ,sum( nvl(case when  bd.act_id=11 then ad.emp_env end ,0)) as emp "). 
 ...
 append(" ,sum( nvl(case when  nvl(bd.ACT_ID,0)  in (0,8) and  AD.ACT_YN='Y' and ad.act_id=5  AND TRUNC(ad.BANK_F_DATE) BETWEEN '"+FromDt+"' and '"+ToDt+"' then round(NVL(mminvolve(AD.app_id),0)/100000,2) end ,0)) as pend_bank_sanc_mm ").
...
append(" where ad.app_id=bd.app_id(+) and ad.unit_dist_cd=dm.district_cd and dm.state_cd=ms.state_cd and AD.BENF_CATAGORY_CD IN ('SC','ST')").
append("  "+txtSearch+"  ").
append(" and ad.off_cd=mom.off_cd(+) AND MS.zone_cd=ZM.zone_cd group by bank_name,ms.STATE_NM,MS.STATE_CD,  ROLLUP(DECODE(dm.DISTRICT_NAME,NULL,'Total',dm.DISTRICT_NAME))) A WHERE A.BANK_FORD>0  AND  A.BANK_NAME='"+vBank+"' AND A.STATE_NM LIKE '"+vSTATE+"'");
...
<div align="center"><span class="style5"><u><%= vBank %><span class="style5">(State Name:<%= wherecls %>)</u></span><br />
 </div>
<br />
...
p4=addUrl+"&BANKNAME="+rsinst.getString("bank_name")+"&STATE="+rsinst.getString("state_nm")+"&DISTCD="+rsinst.getString("DISTRICT_NAME");
 %>
...
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=5"%>">
...
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=11"%>">
...
<td nowrap="nowrap">
    <div align="right">
	<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=12"%>">
   ...
   <td nowrap="nowrap">
<div align="right"><a href="dwstatewiseSCST.jsp?<%=p4+"&amp;QRYCODE=27"%>">
<% out.print(dfd.format(rsinst.getDouble("TDR_UPDATE")));%>
</a>
</td>
<td nowrap="nowrap"><a href="dwstatewiseSCST.jsp?<%=p4+"&amp;QRYCODE=24"%>">
<% out.print(dfd.format(rsinst.getDouble("TDR_NOT_UPDATE")));%></a></td> 
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=7"%>">
...
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=40"%>">
...
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p3+"&QRYCODE=45"%>">
...
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=38"%>">
...
<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSCST.jsp?<%=p4+"&QRYCODE=16"%>">
