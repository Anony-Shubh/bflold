<%@ page  session="true" buffer="300kb" import="java.sql.*,java.util.*,java.text.*,java.util.Date,java.text.SimpleDateFormat,pkgPmegpNew.DBCon,java.text.NumberFormat"  %>
...
String ToDt=(request.getParameter("TODT")==null) ? "31-MAR-2017" :(String) request.getParameter("TODT");
...
QryCaption="Under Process/Pending at Agency";
  txtSearch = txtSearch + " and ad.act_id=1 and TRUNC(ad.Online_subdt) between '"+FromDt+"' and '"+ToDt+"' " ;
 ...
 // REJECTED BY DLTFC COMMITTEE
  txtSearch = txtSearch + " and TRUNC(ad.Online_subdt) between '"+FromDt+"' and '"+ToDt+"' and ad.act_id=7 ";
 ...
 // Sanctioned by Bank
  txtSearch = txtSearch + " and  TRUNC(bd.LOAN_SANC_DT) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11 ";
}else if (vQryCode.equals("7")){
 QryCaption="Rejected by Bank";
  txtSearch = txtSearch + " and  TRUNC(ad.Online_subdt) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=3  ";
 }else if (vQryCode.equals("8")){
 QryCaption="Own Contribution Deposited";
 // Own Contribution Deposited
  txtSearch = txtSearch + " and  BD.OWN_CONT_DT IS NOT NULL AND  TRUNC(BD.OWN_CONT_DT) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
 }else if (vQryCode.equals("9")){
 QryCaption="Loan Released";
 //  LOAN RELEASED
  txtSearch = txtSearch + " and  TRUNC(bd.ist_loan_dt) IS NOT NULL AND   TRUNC(bd.ist_loan_dt) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
  }else if (vQryCode.equals("10")){
  QryCaption="EDP Training Completed";
 // EDP TRANING COMPLETED
  txtSearch = txtSearch + " AND BD.EDP_CERT_DT IS NOT NULL and TRUNC(ad.Online_subdt) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
 }else if (vQryCode.equals("11")){
 QryCaption="Margin Money Claimed";
  txtSearch = txtSearch + " AND BD.MM_CLAIM_DT IS NOT NULL AND BD.MM_CLAIM_AMT>0 AND LOAN_DOCFNAME IS NOT NULL and TRUNC(BD.MM_CLAIM_DT) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11  ";
 }else if (vQryCode.equals("12")){
 QryCaption="Margin Money Released";
  txtSearch = txtSearch + " AND BD.MM_REL_DT IS NOT NULL and TRUNC(BD.MM_REL_DT) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=11 AND BD.PACT_ID=34 ";
 }else if (vQryCode.equals("13")){
 QryCaption="Physical Verification completed";
  txtSearch = txtSearch + " and TRUNC(ad.Online_subdt) between '"+FromDt+"' and '"+ToDt+"' and bd.act_id=30  ";
...
%>
  ...
  <tr>
    <td colspan="2" nowrap  scope="col">From: 
      <input name="FROMDT" type="text" id="FROMDT" value="<%= FromDt %>" size="11" maxlength="11" readonly="true">
     
      To:
      <input name="TODT" type="text" id="TODT" value="<%= ToDt %>" size="11" maxlength="11" readonly="true">
      Status:
      <select name="STATUSCODE" id="STATUSCODE">
        ...
        <option value="2"<% if (vQryCode.equals("2")){ out.println("Selected");}%>>Place Before Approver</option>
        <option value="3"<% if (vQryCode.equals("3")){ out.println("Selected");}%>>Returned by Approver</option>
        <option value="4"<% if (vQryCode.equals("4")){ out.println("Selected");}%>>Sanctioned by Approver</option>
        ...
        <option value="7"<% if (vQryCode.equals("7")){ out.println("Selected");}%>>Returned by Bank</option>
    ...
    <td rowspan="2" scope="col"><div align="center">
      <select name="COL1" id="COL1">
          ...
          <option value="ad.ind_type">Manufacturing/Service Ind</option>
  ...
  <tr>
    ...
    <td rowspan="2">
      ...
      <br><input name="chkRow" type="checkbox" value="edm.edu_desc">
       Qualification
      <input name="chkRow" type="checkbox" value="ad.ind_type">
...
qrysb.append("     m_district bend,"      );
qrysb.append("     education_mast edm,"      );
qrysb.append("     m_district udm,"      );
...
qrysb.append("     mas_off_mast edpom"      );
...
qrysb.append("   AND ad.APP_DIST_CD      = bend.DISTRICT_CD"      );
qrysb.append("   AND ad.BENF_TYPE_CD     = BTM.BENF_TYPE_CD"      );
qrysb.append("   AND ad.BENF_CATAGORY_CD = bcm.BENF_CATEGORY_CD"      );
qrysb.append("   AND ad.EDU_ID           = edm.EDU_ID"      );
qrysb.append("   AND ad.UNIT_DIST_CD     = udm.DISTRICT_CD"      );
qrysb.append("   AND bd.TR_ID            = edpom.OFF_CD(+)"      );
qrysb.append("   AND ad.OFF_CD           = mom.OFF_CD"      );
qrysb.append("   AND am.IND_GRP_CD       = igm.IND_GRP_CD"      );
qrysb.append("   AND ad.ACTIVITY_CD      = am.ACTIVITY_CD  "+txtSearch+" "      );
...
append(" SUM(CASE WHEN nvl(ad.workshed_cost,0)+nvl(ad.machinary_cost,0)+nvl(ad.pre_ope_exp,0)+ nvl(ad.working_capital,0) between 1500001 and 2500000 " ).
 append(" THEN 1 ELSE 0 END) \"15.00 to 25.00\", " ).
 append(" SUM(CASE WHEN nvl(ad.workshed_cost,0)+nvl(ad.machinary_cost,0)+nvl(ad.pre_ope_exp,0)+ nvl(ad.working_capital,0)>2500000 " ).
 append(" THEN 1 ELSE 0 END) \"Above 25.00\", COUNT(*) total " );
...
append(" SUM(CASE WHEN nvl(bd.workshed_cost,0)+nvl(bd.machinary_cost,0)+nvl(bd.pre_ope_cost,0)+ nvl(bd.working_capital,0)>0 THEN 1 ELSE 0 END) TOTAL " );
 ...
 totsb.append("select count(*) from   "+qrysb.toString()+"");
 ...
 append ("  from  "+qrysb.toString()+"  GROUP BY  "+col1+" ");
...
while (rscol1_hasData) {
	...
	if (Val1.equals("1")){
	fqry="SELECT  "+initStr+" "+row1+" , "+qry2+" COUNT( nvl("+col1+",0) ) TOTAL FROM "+qrysb.toString()+" GROUP BY rollup( "+initStr+" "+row1+" )" ;
	
	}else{
	fqry="SELECT  "+initStr+" "+row1+" , "+qry2+" SUM( nvl("+Val1+",0) ) TOTAL FROM "+qrysb.toString()+" GROUP BY rollup( "+initStr+" "+row1+" )" ;
	}//if end;
}
 ...
 if (col1.equals("2") ||col1.equals("3")) {
	...
	fqry="SELECT  "+initStr+"  ,"+prjszqry.toString()+"  FROM "+qrysb.toString()+" GROUP BY rollup( "+initStr+" )" ;
}
