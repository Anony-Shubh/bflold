...
sb.append(" SELECT NVL(DECODE(bank_name,NULL,'Total',BANK_NAME),'Total') as bank_name,COUNT(*) AS NO_OF_APP,").
append(" SUM(DECODE (TDR_YN,'Y',1,0)) AS TDR_UPDATED,").
...
append("  "+txtSearch+"  ").
append(" and ad.off_cd=mom.off_cd(+) AND MS.zone_cd=ZM.zone_cd   )group by ROLLUP(bank_name) ");
...
<td nowrap="nowrap"><a href="dwstatewiseSec.jsp?<%=p3+"&amp;QRYCODE=27"%>">
  <% out.print(dfd.format(rsinst.getDouble("TDR_UPDATED")));%>
</a></td>

<td nowrap="nowrap"><div align="right">
<a href="dwstatewiseSec.jsp?<%=p3+"&amp;QRYCODE=24"%>">
    
<% out.print(dfd.format(rsinst.getDouble("TDR_NOT_UPDATED")));%></a>
</div></td>
 <td><a href="stateWiseTdrDetailSecond.jsp?<%=p3%>">view </a>
 </td>
</tr>
<%
}
