...
qrysb.append("   SELECT nvl(ZONE_NM,'Grand Total') as ZONE_NM,nvl(state_nm,'Total') as State_nm,COUNT(*) AS NOOFAPPREC");
qrysb.append("   FROM "      );
qrysb.append("   ("      );
qrysb.append("   SELECT"      );
...
qrysb.append("     AND ad.RET_YN='Y' AND TRUNC(ad.ONLINE_SUBDT) BETWEEN '"+FromDt+"' and '"+ToDt+"' "      );
qrysb.append("   "      );
qrysb.append("    )"      );
...
<tr>
  <td colspan="10"><div align="center">PMEGP Portal Offline Performance Report </div></td>
</tr>
<tr>
<th>Agency</th>
<td><%=vAgencytit  %></td>
...
<%} else{%>
 <!--<a href="pmegpdashboardmrOffDis.jsp<%=urlrep%>&State=<%= State_nm%>">-->
      <%= State_nm%>

</a><%}%>
	  
	  </div></td>
<td> <a href="dwstatewiseoff.jsp<%=urlrep%>&<%=p3+"&QRYCODE=0"%>">
