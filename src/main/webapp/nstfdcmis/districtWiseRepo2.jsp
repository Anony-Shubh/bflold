...
String STATE_CD =(String) request.getParameter("state");
...
if(nm.equals("TOTAL")){
 ...
 if(!STATE_CD.equals("TOTAL")){
vcls=vcls+ "MS.STATE_CD='"+STATE_CD+"'";	 
	 
 }
 
 else{
	 vcls=vcls+ "MS.STATE_CD like '%%'";	
 }
...
qrysb.append(" SELECT NVL(dm.district_name,'TOTAL') AS STATE_NM,MAX(dm.DISTRICT_CD) state_cd,MAX(MS.STATE_CD) AS DIST_CD, ");
...
qrysb.append(" where ad.app_id=bd.app_id(+) AND  "+vcls+"  ");


qrysb.append(" and ad.unit_dist_cd=dm.district_cd and dm.state_cd=ms.state_cd  ");


qrysb.append(" and ad.off_cd=mom.off_cd and AD.BENF_CATAGORY_CD='ST' group by ROLLUP(dm.district_name)  ");
...
DIST_CD=rsinst.getString("DIST_CD")==null?"":rsinst.getString("DIST_CD");
...
<td><div align="right">
<a href="../jasperrop/nbfcGenReport.jsp?dist=<%=STATE_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=5&QRY=2">

      <% out.print(dfd.format(rsinst.getDouble("bank_ford")));%>
 </a></div></td>
  <td>
    <div align="right">
	<a href="../jasperrop/nbfcGenReport.jsp?dist=<%=STATE_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=6&QRY=2">
...
<td>
    <div align="right">
	<a href="../jasperrop/nbfcGenReport.jsp?dist=<%=STATE_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=12&QRY=2"%>
	<% out.print(dfd.format(rsinst.getDouble("mm_rel")));%>
</a></div></td><td><div align="right">
      <% out.print(dfd.format(rsinst.getDouble("mm_rel_amt")));%>
  </div></td>
  <td><div align="right">
<a href="../jasperrop/nbfcGenReport.jsp?dist=<%=STATE_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=7&QRY=2"%>
...
<%}else{%>
 ...
 <td><div align="center"><%=STATE_NM  %>	</div></td>
<td><div align="right">
<a href="../jasperrop/nbfcGenReport.jsp?dist=TOTAL&state=<%=DIST_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=5&QRY=2">

      <% out.print(dfd.format(rsinst.getDouble("bank_ford")));%>
 </a></div></td>
  <td>
    <div align="right"><a href="../jasperrop/nbfcGenReport.jsp?dist=TOTAL&state=<%=DIST_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=6&QRY=2"%>
      <% out.print(dfd.format(rsinst.getDouble("bank_sanc")));%>
</a></div></td><td><div align="right">
      <% out.print(dfd.format(rsinst.getDouble("bank_sanc_mm")));%>
  </div></td>
<td>
    <div align="right">
	<a href="../jasperrop/nbfcGenReport.jsp?dist=TOTAL&state=<%=DIST_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=12&QRY=2"%>
	<% out.print(dfd.format(rsinst.getDouble("mm_rel")));%>
</a></div></td><td><div align="right">
      <% out.print(dfd.format(rsinst.getDouble("mm_rel_amt")));%>
  </div></td>
  <td><div align="right">
<a href="../jasperrop/nbfcGenReport.jsp?dist=TOTAL&state=<%=DIST_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRYCODE=7&QRY=2"%>
<% out.print(dfd.format(rsinst.getDouble("bank_reject")));%>
</a></div></td>
  <td><div align="right">
