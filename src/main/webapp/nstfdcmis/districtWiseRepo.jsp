...
qrysb.append("   SELECT NVL(DISTRICT_NAME,'TOTAL') AS STATE_NM,MAX(DISTRICT_CD) state_cd,MAX(STATE_CD) AS DIST_CD,COUNT(*) as NO_OF_PRJ,SUM(MMINLAK) AS MMINLAK FROM("      );
qrysb.append("   SELECT "      );
...
qrysb.append("     BD.MM_REL_AMT,ROUND(NVL(BD.MM_REL_AMT,0)/100000,2) AS MMINLAK, "      );
qrysb.append("     BD.UTRN_NO,"      );
qrysb.append("     BD.PAY_STATUS,MD.DISTRICT_CD,"      );
...
qrysb.append("   AND BD.PACT_ID        = 34 AND "+vcls+" "      );
qrysb.append("   AND bd.mm_rel_dt BETWEEN '"+FromDt+"' AND '"+ToDt+"'   "      );
qrysb.append("   AND AD.BENF_CATAGORY_CD='ST') GROUP BY ROLLUP (DISTRICT_NAME)"      );
qrysb.append("   "      );
...
DIST_CD=rsMain.getString("DIST_CD")==null?"":rsMain.getString("DIST_CD");
srno=srno+1;	
%>	
...
<td ><a href="../jasperrop/nbfcGenReport.jsp?dist=<%=STATE_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRY=2"><%=NO_OF_PRJ  %></td>
<td ><%=MMINV  %>	</td>
</TR>
<%}
else{%>
 ...
 <td><div align="center"><%= srno 	 %></div></td>
 <td><div align="center"><%=STATE_NM  %>	</div></td>
<td ><a href="../jasperrop/nbfcGenReport.jsp?dist=TOTAL&state=<%=DIST_CD%>&FROMDT=<%=FromDt%>&TODT=<%=ToDt%>&QRY=2"><%=NO_OF_PRJ  %></td>
<td >	<%=MMINV  %>	</td>
</TR>
