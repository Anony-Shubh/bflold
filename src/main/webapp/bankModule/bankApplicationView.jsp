...
String ACT_ID="";
 ...
 String sancupload="";
		...
		av.append(" SELECT * FROM  (select  D.BR_UPDATE_YN, B.TR_ID, a.app_id,a.act_yn,B.act_id ,NVL(c.act_desc,'') AS CURRENT_STATUS, a.app_name,a.fh_name,TO_CHAR(a.bank_f_date,'DD-MON-YYYY') AS bank_f_date,case when trunc(a.online_subdt)> '31-MAY-2022' AND B.act_id=11 THEN 'Y' ELSE 'N' END  AS online_subdt,(select count(*) from  app_upload_trans where app_id=a.app_id and upm_cd=22 and doc_actyn='Y') as sancupload,  ").
		...
		append("  AS MM_CLAIM_DT,(SELECT COUNT(*) AS EDP_COUNT FROM ONLINE_EDP_DATA WHERE APPLICATION_ID=b.APP_ID) AS EDP_COUNT ,(select count(*) from  app_upload_trans where app_id=a.app_id and upm_cd=16 and doc_actyn='Y') as counsign from app_detail a,bank_dataentry b,bank_app_status c,BANK_BR_MAST D where  A.IFSC_CODE=D.BR_IFSL_CODE AND  a.app_id=b.app_id (+) AND A.ACT_ID=5  AND  b.act_id=c.act_id (+)   ").
		...
		append (" (  SELECT AUT.APP_ID,MAX(AUT.DOC_NAME) AS DOC_NAME FROM APP_UPLOAD_TRANS AUT, BANK_DATAENTRY BD WHERE AUT.APP_ID=BD.APP_ID  ").
        append ("  AND BD.IFSC_CODE=? AND AUT.UPM_CD=8  AND AUT.DOC_ACTYN='Y'").
...
<TABLE border="2" align="center"  >
  ...
  <TR >
	...
	<TH rowspan="2" align="left">Upload Sanction Letter</TH>
	 <TH rowspan="2" align="left">Upload</TH>
      <TH colspan="2" align="left"><DIV align="center">EDP Certificate</DIV></TH>
  ...
  EDP_COUNT=rsinst.getString("EDP_COUNT")==null?"0":rsinst.getString("EDP_COUNT");
  ...
  <TR>
	...
	<div align="center">
	<% if (ACT_ID.equals("11"))  { %>
	<!--<INPUT name="btnUpload" type="button" class="buttonMerun" style="color:#FF0000"  value="Upload Sanction Letter" onClick="uploadBanc('//= APP_ID %>')">-->
	...
	<DIV align="center">
	<%
			if ((DOC_NAME != "") && (ACT_ID.equals("11")) ) {			
			%>
	 Uploaded </a></center>
	
			<% } else if((ACT_ID.equals("11")) && (DOC_NAME.equals("")) ) {
	%>
	<INPUT name="btnUpload" type="button" class="buttonMerun" style="color:#FF0000"  value="Upload EDP Certificate" onClick="uploadCert('<%= APP_ID %>')">
	<%
	}
	else{%>
	-
  ...
  %></TD>
   <TD align="left" nowrap="nowrap"><%if (((DOC_NAME != "") && (ACT_ID.equals("11")) ) && (EDP_COUNT.equals("0"))) {%> 
   <INPUT name="btnSunction2" type="button" class="buttonOrange" style="color:#FF0000; background-color:#FFCC00;"  value="Remove EDP" onClick="RemoveDocument('<%= APP_ID %>')">
   <%} else if (!EDP_COUNT.equals("0")){%>
   Online EDP   
  <% }else{%>-<%}%></TD>
    ...
    <TD align="left" nowrap="nowrap">
	  ...
	  -
	  <%}else{%>
	  <A href="docview.jsp?docname=<%=LOAN_DOCFNAME%>" target="_blank" class="buttonGray">View</A>	 
	  <%}%>
	  </DIV></TD>
	...
	<INPUT name="btnSunction" type="button" class="buttonMerun" style="color:#FF0000" value=" Draft MM Claim" onClick="openmmReport('<%= APP_ID %>')">
	<%} else if (!MM_CLAIM_DT.equals("")) { %>
	<INPUT name="btnSunction" type="button"  class="buttonOrange" style="color:#FF0000; background-color:#FFCC00;" value="MM Claim letter (Final)" onClick="openmmReport('<%= APP_ID %>')">
	<%}%>	</TD>
 ...
 <%
  ...
  <TD align="left" nowrap="nowrap"><SPAN >(<%=srn%>)<%= APP_ID %></SPAN></TD>
    ...
    <TD align="left" nowrap="nowrap"><DIV align="center"><SPAN >
        <INPUT name="btnSunction" type="button" class="buttonOrange" style="background-color:#FF9900; color:#000000;" value="Documents" onClick="viewAppDocument('<%= APP_ID %>')">
    </SPAN></DIV></TD>
...
var url ='BankSanctionLetUpload.jsp?APP_ID='+pAppid;
 modalWin.ShowURL(url,500,700,'Upload PMEGP Sanction Letter',null,null);
...
function RemoveDocument(APP_ID){
var urlpar='RemoveEdp.jsp?&APP_ID='+APP_ID;
 modalWin.ShowURL(urlpar,600,1100,'Applicant Data View.',null,null);
 }//END OF FUNCTION
