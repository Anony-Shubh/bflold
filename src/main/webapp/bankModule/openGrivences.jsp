...
qrysb.append("  SELECT P.GR_ID,ad.off_cd, TO_CHAR(P.GR_TIMESTAMP,'DD-MON-RRRR') AS GR_TIMESTAMP, g.SUB_desc as gr_sub, ");qrysb.append(" P.EMAIL_FROM, P.EMAIL_TO, P.APP_ID, md.district_cd ,");
qrysb.append("  (SELECT ASM.ACT_DESC FROM APP_STATUS_MAST ASM,APP_DETAIL PD WHERE PD.ACT_ID=ASM.ACT_ID AND PD.APP_ID=AD.APP_ID) AS STATUS_DESC,");
qrysb.append("  (SELECT PD.ACT_ID FROM APP_STATUS_MAST ASM,APP_DETAIL PD WHERE PD.ACT_ID=ASM.ACT_ID AND PD.APP_ID=AD.APP_ID) AS APP_ACT_ID,");
qrysb.append(" (SELECT PD.IFSC_CODE FROM APP_DETAIL PD WHERE PD.APP_ID=AD.APP_ID) AS IFSC_CODE,");
qrysb.append(" DECODE(P.ACT_ID,0,'Online Submitted',1,'Under Process',2,'Replied',3,'Closed') as app_status ,");
qrysb.append(" P.ACT_ID,P.ASSIGN_TO,ad.APP_NAME FROM pmegp_grivences P,GRIV_sub_mast g,app_detail_online ad,m_district md,mas_off_mast mom ");
qrysb.append(" WHERE p.sub_id=g.sub_id    ");
qrysb.append(" and p.app_id=ad.app_id and ad.unit_dist_cd=md.district_cd and ad.off_cd=mom.off_cd   "); 
qrysb.append(" ORDER BY P.GR_TIMESTAMP DESC ) WHERE APP_ACT_ID=5 AND IFSC_CODE='"+bank_id+"' and ASSIGN_TO='BK' ");
