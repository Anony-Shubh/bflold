...
function abc(){
 ...
 var dtRegex = new RegExp("^([0]?[1-9]|[1-2]\\d|3[0-1])-(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)-[1-2]\\d{3}$", 'i');
		...
		if(NPA_DT=="" || NPA_DT=="0"){
	inlineMsg('NPA_DT',"Please Select Date",2);
    return false;
	}
	
		if  (!dtRegex.test(NPA_DT)){
	inlineMsg('NPA_DT',"WRONG DATE OR DATE FORMAT SHOULD BE (DD-MON-YYYY)",2);
    return false;
	  }
...
while(rsMain.next())
 ...
 %>
    ...
    <table width="100%">
    <tr>
      <th height="23" colspan="6" nowrap><div align="center" class="style29 style39 style3">NPA Details </div></th>
    ...
    <table width="100%">
      ...
      <tr >
        ...
        <th>NPA Date<span class="style10"> (*) </span></th>
        <td><input name="NPA_DT" id="NPA_DT" type="text"  placeholder="DD-MON-YYYY" size="20" maxlength="20" onClick="javascript:NewCssCal('NPA_DT','ddMMMyyyy')" value="<%= NPA_DT%>"></td>
      </tr>
      <tr >
        <th>3)</th>
        <th>Remarks (if any) </th>
        <td>
		<textarea name="REMARKS" cols="80" rows="2" id="REMARKS"><%= REMARKS %></textarea>		</td>
      </tr>
	 
	  <tr >
	    <th colspan="3"><div align="center">
	      <input name="btnFinalSave" type="button" class="button" id="btnFinalSave" tabindex="24" onClick="abc();"  value="Add NPA Details">
	      </div></th>
      </tr>
    </table>
...
NPA_DT=request.getParameter("NPA_DT")==null?"":request.getParameter("NPA_DT");
IFS_CODE=request.getParameter("IFS_CODE")==null?"":request.getParameter("IFS_CODE");
NPA_YN=request.getParameter("NPA_YN")==null?"":request.getParameter("NPA_YN");
...
StringBuffer sb = new StringBuffer();
		 ...
		 sb.append("   INSERT INTO   LOAN_NPA_DETAIL ( NPATR_ID, APP_ID, NPA_DT, IFSC_CODE, REMARKS,RTIMESTAMP,NPA_YN) VALUES (?,?,?,?,?,SYSDATE,?) ");           


	
  values.add(TR_ID);pstm.add("D");
  values.add(APP_ID);pstm.add("D");
  values.add(NPA_DT);pstm.add("D");
  values.add(IFS_CODE);pstm.add("D");
  values.add(REMARKS);pstm.add("D");
  values.add(NPA_YN);pstm.add("D");
...
response.sendRedirect("NpaDetailUpdation.jsp?APP_ID="+APP_ID);
    ...
    <thead>
      ...
      <tr>
        <td colspan="6"><div align="center" class="GreenLebel">NPA  Detail </div></td>
      </tr>
      <tr>
        <th><div align="left">Sr.No</div></th>
        <th><div align="left">NPA (YES/N0) </div></th>
        <th><div align="left">NPA Date </div></th>
...
int cnt=0;
 ...
 qry.append("  select  NPATR_ID,APP_ID,TO_CHAR(NPA_DT,'DD-MON-YYYY') NPA_DT,IFSC_CODE,REMARKS,DECODE(NPA_YN,'Y','YES','N','NO') AS NPA_YN ");
 qry.append("  from LOAN_NPA_DETAIL WHERE APP_ID=? ");
values.clear();	
 values.add (APP_ID); 
 ...
 ResultSet rsMaiNew = db.executeSQL(qry.toString(),values);
  ...
  {
  cnt=cnt+1;
TTR_ID=rsMaiNew.getString("NPATR_ID")==null?"":rsMaiNew.getString("NPATR_ID");
TAPP_ID=rsMaiNew.getString("APP_ID")==null?"":rsMaiNew.getString("APP_ID");
TNPA_DT=rsMaiNew.getString("NPA_DT")==null?"":rsMaiNew.getString("NPA_DT");
TIFSC_CODE=rsMaiNew.getString("IFSC_CODE")==null?"":rsMaiNew.getString("IFSC_CODE");
TNPA_YN=rsMaiNew.getString("NPA_YN")==null?"":rsMaiNew.getString("NPA_YN");
TREMARKS=rsMaiNew.getString("REMARKS")==null?"":rsMaiNew.getString("REMARKS");
	  ...
	  <tr>
        ...
        <td><div align="left"><%=TNPA_YN%></div></td>
        <td class="style9"><div align="left"><%=TNPA_DT%></div></td>
       
        <td><div align="left"><%= TREMARKS %></div></td>
		
        <td>
		<% if (!TTR_ID.equals("")){%>
		<input type="button" id="BUTTON"  class="button"  name="BUTTON" value="DELETE" onClick="DELETETRID('<%= TTR_ID %>');">
		<%}%>
		</td>
      </tr>
...
<script>
 ...
 modalWin.SetCloseButtonImagePath("../images/remove.gif");
  ...
  modalWin.ShowURL("DelNpaTrans.jsp?TR_ID="+TTR_ID,300,650,'Delete',null,null);
