...
<br> 
 <form name="form" id="form"  class="form" method="post">
		...
		/*
   qrysb.append("  SELECT MOM.OFF_NAME1,  MOM.AGENCY_TYPE, MD.DISTRICT_NAME,  AD.APP_ID, AD.APP_NAME,  AD.MOB_NO1,FGETNEFTIFSCODE_SEC(BD.APP_ID) AS IFSC_CODE," );
   ...
   qrysb.append("  DECODE(BD.TDR_ADJ_DT,NULL,'N','Y') AS TDR_YN,BD.PAY_STATUS,  BD.FB_ACNO,  BD.MM_REL_AMT,TO_CHAR(BD.MM_REL_DT,'DD-MON-YYYY') AS MM_REL_DT  , BD.PACT_ID,BD.APPRV_YN ," );
   qrysb.append("  CASE    WHEN BD.PACT_ID = 30  AND BD.APPRV_YN IN ('A','R','S','N')  THEN MRT.CO_REMARKS ELSE  BD.FAIL_REASON  END CO_REMARKS" );
   qrysb.append("  FROM APP_DETAIL_SECOND AD,  BANK_DATAENTRY_SECOND BD,  MM_REMARKS_TRANS_SECOND MRT,  MAS_OFF_MAST MOM,  M_DISTRICT MD,  RBIBANKLIST RBL" );
...
qrysb.append("  			SELECT MOM.OFF_NAME1,  MOM.AGENCY_TYPE, MD.DISTRICT_NAME,  AD.APP_ID, AD.APP_NAME,  AD.MOB_NO1,FGETNEFTIFSCODE_SEC(BD.APP_ID) AS IFSC_CODE, " );
qrysb.append("       RBL.BANK_NAME,   RBL.BRANCHNAME, BD.MM_CLAIM_AMT,TO_CHAR(BD.MM_CLAIM_DT,'DD-MON-YYYY') AS MM_CLAIM_DT  , " );
qrysb.append("       BD.PBATCH_ID,TO_CHAR( BD.BATCH_DATE,'DD-MON-YYYY') AS  BATCH_DATE  ,  BD.UTRN_NO, " );
qrysb.append("       DECODE(BD.TDR_ADJ_DT,NULL,'N','Y') AS TDR_YN,BD.PAY_STATUS,  BD.FB_ACNO,BRM.BR_UPDATE_YN,  " );
...
qrysb.append("       500 " );
qrysb.append("       ELSE " );
qrysb.append("       0 END APP_FEE , (select  count(*) from indbank_request where substr(txnid,3,length(txnid))=substr(AD.app_id,instr(AD.app_id,'-')+1,length(AD.app_id)) AND UPPER(STATUS) NOT IN ('NOT FOUND' ,'FAILURE')  and PRODUCTINFO='Margin Money Refund'  ) as verifyapp_cnt," );
qrysb.append("       TO_CHAR(BD.MM_REL_DT,'DD-MON-YYYY') AS MM_REL_DT  , BD.PACT_ID,BD.APPRV_YN , " );
qrysb.append("       CASE    WHEN BD.PACT_ID = 30  AND BD.APPRV_YN IN ('A')  THEN MRT.CO_REMARKS ELSE  BD.FAIL_REASON  END CO_REMARKS, " );

 
  qrysb.append("     FUN_VALIDAPPSFTPSEC(AD.APP_ID)                   AS SFTP_REM " );
qrysb.append("       FROM APP_DETAIL_SECOND AD,  BANK_DATAENTRY_SECOND BD,  MM_REMARKS_TRANS_SECOND MRT,  MAS_OFF_MAST MOM,  M_DISTRICT MD,  RBIBANKLIST RBL,BANK_BR_MAST BRM  " );
qrysb.append("       WHERE AD.APP_ID = BD.APP_ID AND AD.OFF_CD= MOM.OFF_CD AND MOM.DISTRICT_CD= MD.DISTRICT_CD and  BRM.BR_IFSL_CODE=RBL.IFSC_CODE " );
qrysb.append("       AND BD.REMTRID = MRT.REMTR_ID(+)AND AD.IFSC_CODE= RBL.IFSC_CODE AND BD.MM_CLAIM_AMT   > 0 " );
qrysb.append("       AND BD.MM_CLAIM_DT IS NOT NULL AND BD.LOAN_DOCFNAME IS NOT NULL AND RBL.IFSC_CODE='"+IFS_CODE+"' " );
...
String SFTP_REM="";
  ...
  {	 
	  ...
	  TDR_YN=rsMain.getString("TDR_YN")==null?"":rsMain.getString("TDR_YN");
	  APP_FEE=rsMain.getString("APP_FEE")==null?"":rsMain.getString("APP_FEE");
	   ...
	   BR_UPDATE_YN=rsMain.getString("BR_UPDATE_YN")==null?"":rsMain.getString("BR_UPDATE_YN");
	   SFTP_REM=rsMain.getString("SFTP_REM")==null?"":rsMain.getString("SFTP_REM");
	  edpcount=edpcount+1;
	  ...
	  <td>
	  <% if ((PACT_ID.equals("34")) && (APPRV_YN.equals("A")) && (TDR_YN.equals("N"))){ %>
	  <input name="btnBank1" id="btnBank1" type="button" class="button" onClick="ShowNewPage('<%=APP_ID%>','bankDetailUpdationsecond.jsp');"  value="UPDATE TDR DETAIL" />
	<%} else if ( PACT_ID.equals("30") && APPRV_YN.equals("A")  && (BR_UPDATE_YN.equals("Y")) ) {%>
	<a href="appidSnRefSec.jsp?APP_ID=<%= APP_ID %>" class="buttonGray"> Update  Referred Bank MM Claim For Second Loan	 </a>
	 <%}	else if ((PAY_STATUS.equals("FAILED")) && (PACT_ID !=("34")) && (APPRV_YN != ("A"))){%>
	FAILED
	<%} else if ((PACT_ID.equals("34")) && (APPRV_YN.equals("A")) && (TDR_YN.equals("Y"))){%>
	TDR Updated
	<%}	
	else {%>
	-
...
<A  href="BankPvGenCk.jsp?APP_ID=<%= APP_ID %>&TYPE=R&VERIFYC=<%=VERIFYAPP_CNT%>" class="greenButtonRound">Generate Challan For Refund</A>
	<%	} else  {%>
	 ...
	 <%} else{%>
      ...
      <td><%if ( PACT_ID.equals("30") && APPRV_YN.equals("A") ) {%>
	  <div align="left"><%= CO_REMARKS %><%=SFTP_REM%></div>
	  <% }else{%>-<%}%></td>	  
	 </tr>
	 <% 
...
db.close();
  ...
  <p>&nbsp;</p>
  <p class="mtextarea">For MM Claim related queries (PMEGP,MUMBAI) contact No: # 022-26713686 and send email to<span class="OrangeLebel"><em><bold> regpkvic@gmail.com</bold> </em></span></p>
  <p class="mtextarea">For Govt. Subsidy Release ( Corp. Bank) queries contact :022-22835701 ( cb0511@corpbank.co.in) </p>
  <p>&nbsp;
   </p>
  <p align="center"> 
