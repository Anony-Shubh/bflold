...
<br> 
 ...
 qrysb.append(" SELECT MOM.OFF_NAME1, (select  count(*) from indbank_request where substr(txnid,3,length(txnid))=substr(AD.app_id,instr(AD.app_id,'-')+1,length(AD.app_id)) AND UPPER(STATUS) NOT IN ('NOT FOUND' ,'FAILURE')  and PRODUCTINFO='Margin Money Refund'  ) as verifyapp_cnt, MOM.AGENCY_TYPE, MD.DISTRICT_NAME,BD.APPLOAN_ACCNO,  AD.APP_ID, AD.APP_NAME,  AD.MOB_NO1,FGETNEFTIFSCODE(BD.APP_ID) AS IFSC_CODE, BD.MM_CLAIM_AMT as MM_CLAIM_AMT ,(select count(*) from bankform_pv where app_id=ad.app_id) as bankpv_c, " );
 qrysb.append("      RBL.BANK_NAME,   RBL.BRANCHNAME,BRM.BR_UPDATE_YN, " );
 ...
 qrysb.append("     FUN_VALIDAPPSFTP(AD.APP_ID)                   AS SFTP_REM, " );
 
 
 qrysb.append(" CASE WHEN TRUNC(BD.IST_LOAN_DT) > '01-APR-2019' AND  TRUNC(months_between(sysdate,BD.IST_LOAN_DT))  > 33 THEN 'Y' ELSE 'N' END AS PV_YN ");
 qrysb.append("      FROM APP_DETAIL AD,  BANK_DATAENTRY BD,  MM_REMARKS_TRANS MRT,  MAS_OFF_MAST MOM,  M_DISTRICT MD,  RBIBANKLIST RBL,BANK_BR_MAST BRM " );
 qrysb.append("      WHERE AD.APP_ID = BD.APP_ID AND AD.OFF_CD= MOM.OFF_CD AND MOM.DISTRICT_CD= MD.DISTRICT_CD AND  AD.ACT_YN='Y' AND  " );
 qrysb.append("      BRM.BR_IFSL_CODE=RBL.IFSC_CODE AND BD.REMTRID = MRT.REMTR_ID(+)AND AD.IFSC_CODE= RBL.IFSC_CODE AND BD.MM_CLAIM_AMT   > 0  " );
 qrysb.append("      AND BD.MM_CLAIM_DT IS NOT NULL AND BD.LOAN_DOCFNAME IS NOT NULL AND RBL.IFSC_CODE='"+IFS_CODE+"'  " );
...
String APP_FEE="";
  ...
  <table border="1" align="center" cellpadding="1" cellspacing="1" bordercolor="#666666" class="bordered" id="example">
    ...
    <tr>
	  ...
	  <th><div align="center">Beneficiary Loan Account No </div></th>
  ...
  {	 
	  ...
	  APPRV_YN=rsMain.getString("APPRV_YN")==null?"":rsMain.getString("APPRV_YN");
	   ...
	   PV_YN=rsMain.getString("PV_YN")==null?"":rsMain.getString("PV_YN");
	   BANKPV_C=rsMain.getString("BANKPV_C")==null?"":rsMain.getString("BANKPV_C");
	  ...
	  edpcount=edpcount+1;
	// out.print("VERIFYAPP_CNT"+VERIFYAPP_CNT);
	  ...
	  <td><div align="left"><%=FB_ACNO %></div></td>
	   <td><div align="left"><%=APPLOAN_ACCNO%></div></td>
	  ...
	  <input name="btnBank1" id="btnBank1" type="button" class="button" onClick="ShowNewPage('<%=APP_ID%>','bankDetailUpdation.jsp');"  value="TDR DETAIL" />
		...
		<input name="btnBank1" id="btnBank1" type="button" class="button" onClick="ShowNewPage('<%=APP_ID%>','LoanInstDetail.jsp');"  value="Installment Details" />
	<%} else if ( PACT_ID.equals("30") && APPRV_YN.equals("A") && (BR_UPDATE_YN.equals("Y")) ) {%>
	<a href="appidSnRef.jsp?APP_ID=<%= APP_ID %>" class="buttonGray"> Update  Referred Bank MM Claim	 </a>
	<input name="btnBank1" id="btnBank1" type="button" class="button" onClick="ShowNewPage('<%=APP_ID%>','appRet.jsp');"  value="Cancel MM Claim" />
...
<A  href="BankPvGenCk.jsp?APP_ID=<%= APP_ID %>&TYPE=R&VERIFYC=<%=VERIFYAPP_CNT%>" class="greenButtonRound">Generate Challan For Refund</A>
	...
	<% if (PV_YN.equals("Y") && (!BANKPV_C.equals("1"))){%>
	<a href="bankform.jsp?APP_ID=<%=APP_ID%>" class="buttongreen"  value="Pv Form">Pv Form</a> 
	<%} if(BANKPV_C.equals("1")){ %>
	<a href="../jasperrop/genrep.jsp?APP_ID=<%=APP_ID%>" class="buttongreen"  value="View Report">View Report</a>
		<%}%>
		  </td>
      <td><div align="left"><%= CO_REMARKS %><%=SFTP_REM%></div></td>	  
	 </tr>
	 <% 
...
var callbackFunctionArray = new Array(refresh_data);
 ...
 modalWin.ShowURL(urlpar,550,800,'View Application',null,null);
 }//end of bank
  function ShowNewPage1(vappid,filename){
var urlpar=filename+'?APP_ID='+vappid;
 var callbackFunctionArray = new Array(refresh_data);

 modalWin.ShowURL(urlpar,750,800,'View Application',null,null);
 }//end of bank
