<%@ page session="true" buffer="500kb" import="java.sql.*,java.util.*,java.text.*,java.util.*,pkgPmegpNew.BankDataValidation,pkgPmegpNew.DBCon,pkgPmegpNew.LoginAttemptCache"  %>
<%
...
<% 
  ...
  ResultSet rsmon = db.executeSQL(sbTr.toString(),values);
    while (rsmon.next()) {
    LoginAttemptCache.resetFailedLogin(moduleName+ us);
  ...
  if (!rsmon.isBeforeFirst()) {
        ...
        LoginAttemptCache.trackFailedLogin(moduleName + uc,5,30);
	}

  while (rsmon.next()) {
    LoginAttemptCache.resetFailedLogin(moduleName+ uc);
    session.setAttribute("sponc_banknm",rsmon.getString(1));
