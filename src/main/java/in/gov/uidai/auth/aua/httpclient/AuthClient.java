...
public class AuthClient {
	...
	public AuthResponseDetails authenticate(Auth auth) {
		try {
			...
			String responseXML = webResource.header("REMOTE_ADDR", InetAddress.getLocalHost().getHostAddress()).type("application/xml").post(String.class,signedXML);
	...
	private String generateSignedAuthXML(Auth auth) throws JAXBException, Exception {
		...
		JAXBElement authElement = new JAXBElement(new QName("http://www.uidai.gov.in/authentication/uid-auth-request/1.0", "Auth"), Auth.class, auth);
